...
namespace RimWorld
{
	public class Precept_RoleSingle : Precept_Role
	{
		...
		public override void RecacheActivity()
		{
			...
			if (active && colonistBelieverCountCached <= def.deactivationBelieverCount && !def.leaderRole)
			{
				active = false;
				if (flag)
				{
			...
			if (!active && (colonistBelieverCountCached >= def.activationBelieverCount || def.leaderRole))
			{
				active = true;
		...
		public override void Assign(Pawn p, bool addThoughts)
		{
			...
			foreach (Ideo allIdeo in ofPlayer.ideos.AllIdeos)
			{
				foreach (Precept item in allIdeo.PreceptsListForReading)
				{
					...
					if (item != this && (precept_Role = item as Precept_Role) != null && precept_Role.def.leaderRole)
					{
