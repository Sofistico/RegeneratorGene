...
namespace RimWorld
{
	public static class TradeUtility
	{
		...
		public static IEnumerable<Pawn> AllSellableColonyPawns(Map map, bool checkAcceptableTemperatureOfAnimals = true)
		{
			...
			{
				if (item3.RaceProps.Animal && item3.HostFaction == null && !item3.InMentalState && !item3.Downed && (!checkAcceptableTemperatureOfAnimals || map.mapTemperature.SeasonAndOutdoorTemperatureAcceptableFor(item3.def)))
				{
					yield return item3;
				}
			}
