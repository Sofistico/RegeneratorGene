...
namespace RimWorld
{
	public class IdeoStyleTracker : IExposable
	{
		...
		public void RecalculateAvailableStyleItems()
		{
			...
			void SetupFrequency<T>() where T : StyleItemDef
			{
				foreach (T allDef in DefDatabase<T>.AllDefs)
				{
					...
					bool flag = allDef is TattooDef && ideo.classic;
		...
		public void ResetStylesForThingDef()
		{
			...
			styleForThingDef.RemoveAll((KeyValuePair<ThingDef, StyleCategoryPair> kvp) => !ideo.PreceptsListForReading.Any((Precept p) => (precept_ThingDef = p as Precept_ThingDef) != null && precept_ThingDef.ThingDef == kvp.Key) || !kvp.Key.canEditAnyStyle);
		}
