...
namespace RimWorld
{
	...
	public class Building_SubcoreScanner : Building_Enterable, IThingHolderWithDrawnPawn, IThingHolder
	{
		...
		public override AcceptanceReport CanAcceptPawn(Pawn selPawn)
		{
			...
			if (State != SubcoreScannerState.WaitingForOccupant)
			{
				...
				case SubcoreScannerState.WaitingForIngredients:
					return "SubcoreScannerRequiresIngredients".Translate();
		...
		{
			Building_SubcoreScanner building_SubcoreScanner;
			if ((building_SubcoreScanner = thing as Building_SubcoreScanner) != null && !building_SubcoreScanner.initScanner)
			{
				return true;
			}
			return false;
		...
		public override IEnumerable<Gizmo> GetGizmos()
		{
			...
			if (initScanner)
			{
				...
				command_Action3.action = EjectContents;
		...
		public override string GetInspectString()
		{
			...
			case SubcoreScannerState.WaitingForIngredients:
			{
				...
				for (int i = 0; i < def.building.subcoreScannerFixedIngredients.Count; i++)
				{
					...
					TaggedString labelCap = ingredientCount.FixedIngredient.LabelCap;
					int num = innerContainer.TotalStackCountOfDef(ingredientCount.FixedIngredient);
					labelCap += $" {num} / {ingredientCount.GetBaseCount()}";
					stringBuilder.AppendInNewLine(labelCap);
				}
				break;
			}
