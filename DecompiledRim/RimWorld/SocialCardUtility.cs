	...
	public static class SocialCardUtility
	{
		...
		public static void DrawSocialCard(Rect rect, Pawn pawn)
		{
			...
			if (ModsConfig.IdeologyActive && !pawn.Dead && pawn.Ideo != null)
			{
				...
				string label = ((precept_Role != null) ? precept_Role.LabelCap : ((string)"NoRoleAssigned".Translate()));
			...
			if (Mouse.IsOver(rect4))
			{
				...
				if (pawn.story.traits != null)
				{
					...
					foreach (Trait allTrait in pawn.story.traits.allTraits)
					{
						...
						List<MemeDef> affectedMemes = allTrait.CurrentData.GetAffectedMemes(allTrait.def, agreeable: true);
						for (int i = 0; i < affectedMemes.Count; i++)
						{
							if (pawn.Ideo.HasMeme(affectedMemes[i]))
							{
								tmpAgreeableMemeTraitPairs.Add(new Pair<Trait, MemeDef>(allTrait, affectedMemes[i]));
							}
						}
						List<MemeDef> affectedMemes2 = allTrait.CurrentData.GetAffectedMemes(allTrait.def, agreeable: false);
						for (int j = 0; j < affectedMemes2.Count; j++)
						{
							if (pawn.Ideo.HasMeme(affectedMemes2[j]))
							{
								tmpDisagreeableMemeTraitPairs.Add(new Pair<Trait, MemeDef>(allTrait, affectedMemes2[j]));
							}
						}
					}
