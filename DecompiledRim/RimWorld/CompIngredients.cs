...
namespace RimWorld
{
	public class CompIngredients : ThingComp
	{
		...
		public override bool AllowStackWith(Thing otherStack)
		{
			...
			if (!Props.performMergeCompatibilityChecks || !compIngredients.Props.performMergeCompatibilityChecks)
			{
				return true;
			}
			if (compIngredients != null && ModsConfig.BiotechActive && FoodUtility.GetFoodKindForStacking(parent) != FoodUtility.GetFoodKindForStacking(otherStack))
			{
				return false;
			}
			...
			MergeIngredients(tmpMergedIngredients, compIngredients.ingredients, out var lostImportantIngredients, parent.def);
			if (lostImportantIngredients)
			{
				return false;
			}
			ComputeTags(tmpMergedIngredientTags, tmpMergedIngredients);
			return tmpMergedIngredientTags.SetsEqual(MergeCompatibilityTags) && tmpMergedIngredientTags.SetsEqual(compIngredients.MergeCompatibilityTags);
		}
