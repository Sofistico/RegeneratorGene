...
namespace RimWorld
{
	public static class StatsReportUtility
	{
		...
		{
			return new StatDrawEntry(StatCategoryDefOf.BasicsImportant, "Description".Translate(), "", thing.DescriptionFlavor, 99999, null, Dialog_InfoCard.DefsToHyperlinks(thing.DescriptionHyperlinks));
		}
		...
		private static void DrawStatsWorker(Rect rect, Thing optionalThing, WorldObject optionalWorldObject)
		{
			...
			for (int i = 0; i < cachedDrawEntries.Count; i++)
			{
				...
				if ((string)ent.category.LabelCap != text)
				{
