...
namespace RimWorld
{
	...
	public class Dialog_Options : Window
	{
		...
		private bool simulateNotOwningIdeologyWhenOpened;

		private bool simulateNotOwningBiotechWhenOpened;
		...
		public override void PostOpen()
		{
			...
			simulateNotOwningIdeologyWhenOpened = Prefs.SimulateNotOwningIdology;
			simulateNotOwningBiotechWhenOpened = Prefs.SimulateNotOwningBiotech;
		...
		{
			if (listing.ButtonTextLabeledPct("ChooseLanguage".Translate(), LanguageDatabase.activeLanguage.DisplayName, 0.6f, TextAnchor.MiddleLeft))
			{
		...
		private void DoVideoOptions(Listing_Standard listing)
		{
			...
			Prefs.ScreenShakeIntensity = listing.SliderLabeled(string.Format("{0}: {1}", "ScreenShakeIntensity".Translate(), screenShakeIntensity.ToStringPercent()), screenShakeIntensity, 0f, 2f, 0.6f);
		...
		private void DoGameplayOptions(Listing_Standard listing)
		{
			...
			for (int i = 0; i < Prefs.PreferredNames.Count; i++)
			{
				...
				if (Widgets.ButtonImage(new Rect(rect.xMax - 24f, rect.y, 24f, 24f), TexButton.DeleteX, Color.white, GenUI.SubtleMouseoverColor))
				{
		...
		private void DoUIOptions(Listing_Standard listing)
		{
			...
			if (listing.ButtonTextLabeledPct("ShowAnimalNames".Translate(), Prefs.AnimalNameMode.ToStringHuman(), 0.6f, TextAnchor.MiddleLeft))
			{
				...
				{
					AnimalNameDisplayMode localMode2 = value2;
					list4.Add(new FloatMenuOption(localMode2.ToStringHuman(), delegate
					{
						Prefs.AnimalNameMode = localMode2;
					}));
				}
			...
			if (ModsConfig.BiotechActive && listing.ButtonTextLabeledPct("ShowMechNames".Translate(), Prefs.MechNameMode.ToStringHuman(), 0.6f, TextAnchor.MiddleLeft))
			{
				...
				{
					MechNameDisplayMode localMode = value3;
					list5.Add(new FloatMenuOption(localMode.ToStringHuman(), delegate
					{
						Prefs.MechNameMode = localMode;
					}));
				}
			...
			if (ModLister.AllExpansions.Where((ExpansionDef e) => !e.isCore && e.Status != ExpansionStatus.NotInstalled).Any() && listing.ButtonTextLabeledPct("SetBackgroundImage".Translate(), taggedString, 0.6f, TextAnchor.MiddleLeft))
			{
				...
				foreach (ExpansionDef allExpansion in ModLister.AllExpansions)
				{
					if (allExpansion.Status != ExpansionStatus.NotInstalled)
					{
						...
						list6.Add(new FloatMenuOption(allExpansion.label, delegate
						{
				...
				list6.Add(new FloatMenuOption("Random".Translate(), delegate
				{
			...
			bool checkOn2 = Prefs.HatsOnlyOnMap;
			listing.CheckboxLabeled("HatsShownOnlyOnMap".Translate(), ref checkOn2, null, 30f, 0.6f);
			if (checkOn2 != Prefs.HatsOnlyOnMap)
			{
				PortraitsCache.Clear();
			}
			Prefs.HatsOnlyOnMap = checkOn2;
			if (!SteamDeck.IsSteamDeck)
			{
				bool checkOn3 = Prefs.DisableTinyText;
				listing.CheckboxLabeled("DisableTinyText".Translate(), ref checkOn3, null, 30f, 0.6f);
				if (Prefs.DisableTinyText != checkOn3)
				{
					Prefs.DisableTinyText = checkOn3;
			...
			bool checkOn4 = !Prefs.CustomCursorEnabled;
			listing.CheckboxLabeled("CustomCursor".Translate(), ref checkOn4, null, 30f, 0.6f);
			Prefs.CustomCursorEnabled = !checkOn4;
		}
		...
		private void DoDevOptions(Listing_Standard listing)
		{
			...
			if (Current.ProgramState != ProgramState.Playing)
			{
				bool checkOn4 = Prefs.SimulateNotOwningRoyalty;
				listing.CheckboxLabeled("SimulateNotOwningRoyalty".Translate(), ref checkOn4, null, 30f, 0.6f);
				Prefs.SimulateNotOwningRoyalty = checkOn4;
				bool checkOn5 = Prefs.SimulateNotOwningIdology;
				listing.CheckboxLabeled("SimulateNotOwningIdeology".Translate(), ref checkOn5, null, 30f, 0.6f);
				Prefs.SimulateNotOwningIdology = checkOn5;
				bool checkOn6 = Prefs.SimulateNotOwningBiotech;
				listing.CheckboxLabeled("SimulateNotOwningBiotech".Translate(), ref checkOn6, null, 30f, 0.6f);
				Prefs.SimulateNotOwningBiotech = checkOn6;
			}
		...
		private void DoModOptions(Listing_Standard listing)
		{
			...
			foreach (Mod cachedModsWithSetting in cachedModsWithSettings)
			{
				if (cachedModsWithSetting.SettingsCategory().ToLower().Contains(modFilter.ToLower()) || cachedModsWithSetting.Content.Name.ToLower().Contains(modFilter.ToLower()))
				{
					if (num % 2 == 0)
					{
						rect2 = listing.GetRect(34f);
					}
					if (Widgets.ButtonText(((num % 2 == 0) ? rect2.LeftHalf() : rect2.RightHalf()).ContractedBy(2f), "  " + cachedModsWithSetting.SettingsCategory(), drawBackground: true, doMouseoverSound: true, active: true, TextAnchor.MiddleLeft))
					{
		...
		public override void PreClose()
		{
			...
			{
				ModsConfig.SetActive("ludeon.rimworld.royalty", active: false);
				ModsConfig.RestartFromChangedMods();
			}
			if (Prefs.SimulateNotOwningIdology && !simulateNotOwningIdeologyWhenOpened && ModsConfig.IdeologyActive)
			{
				ModsConfig.SetActive("ludeon.rimworld.ideology", active: false);
				ModsConfig.RestartFromChangedMods();
			}
			if (Prefs.SimulateNotOwningBiotech && !simulateNotOwningBiotechWhenOpened && ModsConfig.BiotechActive)
			{
				ModsConfig.SetActive("ludeon.rimworld.biotech", active: false);
				ModsConfig.RestartFromChangedMods();
			}
		}
