	...
	public class Pawn_StoryTracker : IExposable
	{
		...
		public void ExposeData()
		{
			...
			if (Scribe.mode == LoadSaveMode.PostLoadInit)
			{
				...
				if (ModsConfig.BiotechActive && pawn.DevelopmentalStage.Juvenile() && bodyType != BodyTypeDefOf.Child)
				{
					Log.Warning(string.Concat(pawn, " had body type set to ", bodyType?.label, " as a child."));
					bodyType = BodyTypeDefOf.Child;
				}
			}
			if (Scribe.mode == LoadSaveMode.PostLoadInit && childhood == null)
			{
				string childDefName = ((childhood != null) ? childhood.identifier : null);
				Scribe_Values.Look(ref childDefName, "childhood");
				if (Scribe.mode == LoadSaveMode.LoadingVars && !childDefName.NullOrEmpty() && DefDatabase<BackstoryDef>.AllDefs.FirstOrDefault((BackstoryDef b) => b.defName == childDefName) == null)
				{
					Log.Error("Couldn't load child backstory with identifier " + childDefName + ". Giving random.");
					childhood = DefDatabase<BackstoryDef>.AllDefs.Where((BackstoryDef bs) => bs.slot == BackstorySlot.Childhood).RandomElement();
				}
			}
			if (Scribe.mode != LoadSaveMode.PostLoadInit || adulthood != null)
			{
				return;
			}
			string adultDefName = ((adulthood != null) ? adulthood.identifier : null);
			Scribe_Values.Look(ref adultDefName, "adulthood");
			if (Scribe.mode == LoadSaveMode.LoadingVars && !adultDefName.NullOrEmpty() && DefDatabase<BackstoryDef>.AllDefs.FirstOrDefault((BackstoryDef b) => b.defName == adultDefName) == null)
			{
				Log.Error("Couldn't load adult backstory with identifier " + adultDefName + ". Giving random.");
				DefDatabase<BackstoryDef>.AllDefs.Where((BackstoryDef bs) => bs.slot == BackstorySlot.Adulthood).RandomElement();
			}
		}
