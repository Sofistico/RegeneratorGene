...
namespace RimWorld
{
	...
	public class HumanEmbryo : GeneSetHolderBase
	{
		...
		public override IEnumerable<Gizmo> GetGizmos()
		{
			...
			if (implantTarget == null)
			{
				...
				Command_Action command_Action2 = new Command_Action
				{
					...
					defaultDesc = "InsertEmbryoGrowthVatDesc".Translate(),
		...
		private void EnsureImplantTargetValid()
		{
			...
			Pawn pawn;
			Building_GrowthVat building_GrowthVat;
			if ((pawn = implantTarget as Pawn) != null)
			{
			...
			else if ((building_GrowthVat = implantTarget as Building_GrowthVat) != null && (building_GrowthVat.Destroyed || !ImplantVatValid(cancel: false)))
			{
				implantTarget = null;
			}
		}
		...
		private bool ImplantPawnValid(bool cancel)
		{
			...
			if ((pawn = implantTarget as Pawn) != null)
			{
				foreach (Bill item in pawn.BillStack)
				{
					...
					if ((bill_Medical = item as Bill_Medical) == null || bill_Medical.uniqueRequiredIngredients.NullOrEmpty())
					{
						continue;
					}
		...
		{
			Building_GrowthVat building_GrowthVat;
			if ((building_GrowthVat = implantTarget as Building_GrowthVat) != null)
			{
				if (cancel)
				{
		...
		private Building_GrowthVat BestAvailableGrowthVat()
		{
			...
			IEnumerable<Building> enumerable = base.MapHeld.listerBuildings.AllBuildingsColonistOfDef(ThingDefOf.GrowthVat);
			tmpEligibleVats.Clear();
			foreach (Building item in enumerable)
			{
				Building_GrowthVat building_GrowthVat = item as Building_GrowthVat;
				if (building_GrowthVat != null && !building_GrowthVat.Working && building_GrowthVat.selectedEmbryo == null && building_GrowthVat.SelectedPawn == null)
				{
