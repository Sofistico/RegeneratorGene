	...
	public class WorkGiver_DoBill : WorkGiver_Scanner
	{
		...
		public override bool ShouldSkip(Pawn pawn, bool forced = false)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				if ((billGiver = list[i] as IBillGiver) != null && billGiver != pawn && ThingIsUsableBillGiver(list[i]) && billGiver.BillStack.AnyShouldDoNow)
				{
					return false;
				}
			}
		...
		public override Job JobOnThing(Pawn pawn, Thing thing, bool forced = false)
		{
			...
			if (billGiver == null || !ThingIsUsableBillGiver(thing) || !billGiver.BillStack.AnyShouldDoNow || !billGiver.UsableForBillsAfterFueling() || !pawn.CanReserve(thing, 1, -1, null, forced) || thing.IsBurning() || thing.IsForbidden(pawn) || (thing.def.hasInteractionCell && !pawn.CanReserveSittableOrSpot(thing.InteractionCell, forced)))
			{
				return null;
			}
		...
		private Job StartOrResumeBillJob(Pawn pawn, IBillGiver giver)
		{
			...
			for (int i = 0; i < giver.BillStack.Count; i++)
			{
				...
				if ((bill_Medical = bill as Bill_Medical) != null)
				{
					...
					if (!pawn.CanReserve(bill_Medical.GiverPawn))
					{
				...
				Bill_Mech bill_Mech2;
				if ((bill_Mech2 = bill as Bill_Mech) != null && bill_Mech2.State != 0)
				{
					return WorkOnMechBill((Thing)giver, bill_Mech2);
				}
		...
		private static Job WorkOnMechBill(Thing giver, Bill_Mech bill)
		{
		...
		private static bool TryFindBestIngredientsHelper(Predicate<Thing> thingValidator, Predicate<List<Thing>> foundAllIngredientsAndChoose, List<IngredientCount> ingredients, Pawn pawn, Thing billGiver, List<ThingCount> chosen, float searchRadius)
		{
			...
			if ((building_MechGestator = billGiver as Building_MechGestator) != null)
			{
				relevantThings.AddRange(building_MechGestator.innerContainer);
				if (foundAllIngredientsAndChoose(relevantThings))
				{
					relevantThings.Clear();
					return true;
