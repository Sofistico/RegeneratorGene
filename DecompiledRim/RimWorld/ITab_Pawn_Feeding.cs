...
namespace RimWorld
{
	public class ITab_Pawn_Feeding : ITab
	{
		...
		public override bool IsVisible
		{
			get
			{
				if (ChildcareUtility.CanSuckle(base.SelPawn, out var _))
				{
					return ChildcareUtility.HasBreastfeedCompatibleFactions(Faction.OfPlayer, base.SelPawn);
				}
				return false;
			}
		}
		...
		protected override void FillTab()
		{
			...
			{
				FillTab(base.SelPawn, new Rect(Vector2.zero, size), ref scrollPositionAutoBreastfeed, ref scrollPositionBabyConsumables);
			}
		}
		...
		{
			Widgets.DropdownMenuElement<AutofeedMode> dropdownMenuElement = new Widgets.DropdownMenuElement<AutofeedMode>
			{
				option = GenerateFloatMenuOption(AutofeedMode.Never, pair),
				payload = AutofeedMode.Never
			};
			yield return dropdownMenuElement;
			dropdownMenuElement = new Widgets.DropdownMenuElement<AutofeedMode>
			{
