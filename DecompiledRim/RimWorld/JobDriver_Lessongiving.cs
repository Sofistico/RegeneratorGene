	...
	public class JobDriver_Lessongiving : JobDriver
	{
		...
		protected override IEnumerable<Toil> MakeNewToils()
		{
			...
			toil.tickAction = delegate
			{
				...
				JobDriver_Lessontaking jobDriver_Lessontaking;
				if (taughtSkill == null)
				{
					EndJobWith(JobCondition.Incompletable);
				}
				else if (isReadyToTeach && (jobDriver_Lessontaking = Student?.jobs?.curDriver as JobDriver_Lessontaking) != null && jobDriver_Lessontaking.isReadyToLearn)
				{
		...
		{
			string text = ((taughtSkill != null) ? (" (" + taughtSkill.defName + ")") : "");
