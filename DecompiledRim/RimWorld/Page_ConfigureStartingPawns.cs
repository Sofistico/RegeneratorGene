	...
	public class Page_ConfigureStartingPawns : Page
	{
		...
		private void DrawPawnList(Rect rect)
		{
			...
			for (int i = 0; i < Find.GameInitData.startingAndOptionalPawns.Count; i++)
			{
				...
				Widgets.Label(label: (nameTriple == null) ? pawn.LabelShort : (string.IsNullOrEmpty(nameTriple.Nick) ? nameTriple.First : nameTriple.Nick), rect: rect3.TopPart(0.5f).Rounded());
