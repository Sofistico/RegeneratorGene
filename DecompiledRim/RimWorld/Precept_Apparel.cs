	...
	public class Precept_Apparel : Precept
	{
		...
		public override void Init(Ideo ideo, FactionDef generatingFor = null)
		{
			...
			for (int i = 0; i < def.comps.Count; i++)
			{
				...
				if ((preceptComp_Apparel = def.comps[i] as PreceptComp_Apparel) != null)
				{
		...
		public override void Regenerate(Ideo ideo, FactionDef generatingFor = null)
		{
			...
			if (!def.canUseAlreadyUsedThingDef)
			{
				...
				foreach (Precept item in ideo.PreceptsListForReading)
				{
					...
					if ((precept_Apparel = item as Precept_Apparel) != null)
					{
		...
		public override bool CompatibleWith(Precept other)
		{
			...
			Precept_Apparel precept_Apparel;
			if ((precept_Apparel = other as Precept_Apparel) != null && def == other.def && (TargetGender == precept_Apparel.TargetGender || TargetGender == Gender.None || precept_Apparel.TargetGender == Gender.None) && !ApparelUtility.CanWearTogether(apparelDef, precept_Apparel.apparelDef, BodyDefOf.Human))
			{
				return false;
			}
		...
		public override bool GetPlayerWarning(out string shortText, out string description)
		{
			...
			{
				if (TargetGender != 0 && item.restrictToSupremeGender && ideo.SupremeGender != 0 && ideo.SupremeGender != TargetGender)
				{
					continue;
				}
