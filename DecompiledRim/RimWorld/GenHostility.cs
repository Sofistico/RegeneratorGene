...
namespace RimWorld
{
	public static class GenHostility
	{
		public static bool HostileTo(this Thing a, Thing b)
		{
			...
			if ((pawn != null && MechanitorUtility.IsOverseerSubject(pawn) && !pawn.IsColonyMechPlayerControlled) || (pawn2 != null && MechanitorUtility.IsOverseerSubject(pawn2) && !pawn2.IsColonyMechPlayerControlled))
			{
				return false;
			}
		...
		public static bool AnyHostileActiveThreatTo(Map map, Faction faction, out IAttackTarget threat, bool countDormantPawnsAsHostile = false, bool canBeFogged = false)
		{
			foreach (IAttackTarget item in map.attackTargetsCache.TargetsHostileToFaction(faction))
			{
				...
				Pawn pawn;
				if (countDormantPawnsAsHostile && item.Thing.HostileTo(faction) && (canBeFogged || !item.Thing.Fogged()) && !item.ThreatDisabled(null) && (pawn = item.Thing as Pawn) != null)
				{
