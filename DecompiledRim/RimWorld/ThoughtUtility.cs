...
namespace RimWorld
{
	public static class ThoughtUtility
	{
		...
		public static bool CanGetThought(Pawn pawn, ThoughtDef def, bool checkIfNullified = false)
		{
			try
			{
				...
				if (ModsConfig.BiotechActive && !def.requiredGenes.NullOrEmpty())
				{
					...
					for (int j = 0; j < def.requiredGenes.Count; j++)
					{
						if (!pawn.genes.HasGene(def.requiredGenes[j]))
						{
							return false;
						}
					}
		...
		public static string GenerateBabyTalk(string str)
		{
			...
			return GenText.CapitalizeSentences(Regex.Replace(str, "([A-Z]|[a-z]|')+", (Match word) => string.Join("", Enumerable.Repeat(syllables[Rand.RangeSeeded(0, syllables.Length, GenText.StableStringHash(word.ToString()))], (word.Length <= 6) ? 1 : 2))));
		}
	}
}
