...
namespace RimWorld
{
	public class Pawn_NeedsTracker : IExposable
	{
		...
		public bool NeedsInitialized => needsInitialized;
		...
		private bool ShouldHaveNeed(NeedDef nd)
		{
			...
			if (nd.playerMechsOnly && (!pawn.RaceProps.IsMechanoid || pawn.Faction != Faction.OfPlayer || pawn.TryGetComp<CompOverseerSubject>() == null))
			{
				return false;
			}
			...
			if (ModsConfig.BiotechActive && pawn.genes != null && pawn.genes.GenesListForReading.Any((Gene x) => x.def.disablesNeeds != null && x.def.disablesNeeds.Contains(nd)))
			{
				return false;
			}
			...
			if (nd.onlyIfCausedByGene && (pawn.genes == null || !pawn.genes.GenesListForReading.Any((Gene x) => x.def.causesNeed == nd)))
			{
				return false;
			}
		...
		{
			if (energy != null && DebugSettings.ShowDevGizmos)
			{
				Command_Action command_Action = new Command_Action();
				command_Action.defaultLabel = "DEV: Mech energy +5%";
				...
				command_Action2.defaultLabel = "DEv: Mech energy -5%";
