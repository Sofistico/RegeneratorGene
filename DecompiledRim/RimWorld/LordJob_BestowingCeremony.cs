...
namespace RimWorld
{
	public class LordJob_BestowingCeremony : LordJob_Ritual
	{
		...
		public override bool BlocksDrafting => lord.CurLordToil == ceremonyToil;
		...
		public override StateGraph CreateGraph()
		{
			...
			transition14.AddTrigger(new Trigger_Custom((TriggerSignal signal) => (signal.type == TriggerSignalType.Tick && !bestower.Dead && ((bestower.health.hediffSet.HasHediff(HediffDefOf.Hypothermia) && bestower.health.hediffSet.GetFirstHediffOfDef(HediffDefOf.Hypothermia).Severity >= 0.35f) || (bestower.health.hediffSet.HasHediff(HediffDefOf.Heatstroke) && bestower.health.hediffSet.GetFirstHediffOfDef(HediffDefOf.Heatstroke).Severity >= 0.35f))) ? true : false));
			...
			transition15.AddTrigger(new Trigger_PawnHarmed(1f, requireInstigatorWithFaction: true, base.Map.ParentFaction));
