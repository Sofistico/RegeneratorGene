	...
	public class Precept_Relic : Precept_ThingStyle
	{
		...
		private static ThingDef GenerateStuffFor(ThingDef thing, Ideo ideo = null)
		{
			...
			if (ideo != null)
			{
				...
				IEnumerable<ThingDef> alreadyUsedStuffs = (from p in ideo.PreceptsListForReading
					where (precept_Relic = p as Precept_Relic) != null && precept_Relic.ThingDef == thing
		...
		private IEnumerable<Quest> GetQuestsToDeactivateIfLost()
		{
			...
			for (int i = 0; i < allQuests.Count; i++)
			{
				...
				{
					QuestPart_SubquestGenerator_RelicHunt questPart_SubquestGenerator_RelicHunt;
					if ((questPart_SubquestGenerator_RelicHunt = partsListForReading[j] as QuestPart_SubquestGenerator_RelicHunt) != null && questPart_SubquestGenerator_RelicHunt.relic == this)
					{
