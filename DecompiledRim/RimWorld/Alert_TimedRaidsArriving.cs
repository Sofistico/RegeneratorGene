...
namespace RimWorld
{
	public class Alert_TimedRaidsArriving : Alert
	{
		...
		private void CalculateTargets()
		{
			...
			foreach (WorldObject allWorldObject in Find.WorldObjects.AllWorldObjects)
			{
				foreach (WorldObjectComp allComp in allWorldObject.AllComps)
				{
					...
					if ((timedDetectionRaids = allComp as TimedDetectionRaids) != null && ShouldAlertTimedRaids(timedDetectionRaids))
					{
		...
		{
			return "AlertTimedRaidsArrivingIn".Translate(timedRaidsArrivingSoon.MinBy((TimedDetectionRaids t) => t.TicksLeftToSendRaids).TicksLeftToSendRaids.ToStringTicksToPeriod());
		}

		public override TaggedString GetExplanation()
		{
			return "AlertTimedRaidsArrivingInDesc".Translate(timedRaidsArrivingSoon.Select((TimedDetectionRaids t) => "AlertTimedRaidsArrivingAt".Translate(t.parent.Label, t.TicksLeftToSendRaids.ToStringTicksToPeriod()).Resolve()).ToLineList("- "));
		}
