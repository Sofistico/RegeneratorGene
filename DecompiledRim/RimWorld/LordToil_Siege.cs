	...
	public class LordToil_Siege : LordToil
	{
		...
		public override void Init()
		{
			...
			foreach (Blueprint_Build item2 in SiegeBlueprintPlacer.PlaceBlueprints(lordToilData_Siege.siegeCenter, base.Map, lord.faction, lordToilData_Siege.blueprintPoints))
			{
				...
				if (thingDef != null)
				{
					ThingDef thingDef2 = TurretGunUtility.TryFindRandomShellDef(thingDef, allowEMP: false, allowToxGas: false, mustHarmHealth: true, lord.faction.def.techLevel, allowAntigrainWarhead: false, 250f, lord.faction);
					if (thingDef2 != null)
					{
		...
		public override void LordToilTick()
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				for (int j = 0; j < thingList.Count; j++)
				{
					...
					if (Find.Storyteller.difficulty.classicMortars || thingList[j].def.building == null || !thingList[j].def.building.IsMortar || thingList[j].Faction != lord.faction)
					{
						continue;
					}
