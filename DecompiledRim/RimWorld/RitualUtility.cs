	...
	public static class RitualUtility
	{
		...
		public static LordJob_Ritual TargetOfRitual(this Thing thing)
		{
			...
			foreach (Lord lord in thing.Map.lordManager.lords)
			{
				...
				if ((lordJob_Ritual = lord.LordJob as LordJob_Ritual) != null && lordJob_Ritual.selectedTarget.Thing == thing)
				{
					LordToilData_Gathering lordToilData_Gathering = lordJob_Ritual.lord.CurLordToil.data as LordToilData_Gathering;
					if (lordToilData_Gathering != null && lordToilData_Gathering.presentForTicks.Any())
					{
		...
		public static float CalculateQualityAbstract(Precept_Ritual ritual, TargetInfo ritualTarget, RitualRoleAssignments assignments, RitualObligation obligation = null)
		{
			...
			{
				ExpectedOutcomeDesc expectedOutcomeDesc = comp.GetExpectedOutcomeDesc(ritual, ritualTarget, obligation, assignments, null);
		...
		public static string QualityBreakdownAbstract(Precept_Ritual ritual, TargetInfo ritualTarget, RitualRoleAssignments assignments, RitualObligation obligation = null)
		{
			...
			foreach (RitualOutcomeComp comp in def.comps)
			{
				...
				if (expectedOutcomeDesc != null)
				{
					taggedString += "\n  - " + expectedOutcomeDesc.label;
					taggedString += ": " + comp.GetDescAbstract(expectedOutcomeDesc.positive, expectedOutcomeDesc.quality) + ".";
				}
			}
		...
		public static IEnumerable<LordJob_Ritual> GetActiveRitualsForPrecept(Precept_Ritual precept)
		{
			...
			for (int i = 0; i < maps.Count; i++)
			{
				...
				{
					LordJob_Ritual lordJob_Ritual;
					if ((lordJob_Ritual = lords[j].LordJob as LordJob_Ritual) != null && lordJob_Ritual.Ritual == precept)
					{
			...
			else if (newRole.def.leaderRole)
			{
				foreach (Ideo allIdeo in Faction.OfPlayer.ideos.AllIdeos)
				{
					foreach (Precept item in allIdeo.PreceptsListForReading)
					{
						...
						if (item != oldRole && (precept_Role = item as Precept_Role) != null)
						{
		...
		{
			LordJob_Ritual lordJob_Ritual = pawn.GetLord()?.LordJob as LordJob_Ritual;
			if (lordJob_Ritual != null && pawn.CurJob != null && pawn.CurJob.speechFaceSpectatorsIfPossible)
			{
