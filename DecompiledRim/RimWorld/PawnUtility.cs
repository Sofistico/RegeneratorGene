...
namespace RimWorld
{
	public static class PawnUtility
	{
		...
		public static bool IsInteractionBlocked(this Pawn pawn, InteractionDef interaction, bool isInitiator, bool isRandom)
		{
			...
			if ((lordJob_Ritual = pawn.GetLord()?.LordJob as LordJob_Ritual) != null && (lordJob_Ritual.RoleFor(pawn, includeForced: true)?.blocksSocialInteraction ?? false))
			{
				return true;
			}
			return false;
		...
		public static bool CanCasuallyInteractNow(this Pawn p, bool twoWayInteraction = false, bool canInteractWhileSleeping = false, bool canInteractWhileRoaming = false)
		{
			if (p.Drafted)
			{
				return false;
		...
		public static PawnPosture GetPosture(this Pawn p)
		{
			...
			PawnPosture pawnPosture = (p.Dead ? PawnPosture.LayingOnGroundNormal : ((!ModsConfig.BiotechActive || !p.IsCharging()) ? (p.Downed ? ((p.jobs == null || !p.jobs.posture.Laying()) ? PawnPosture.LayingOnGroundNormal : p.jobs.posture) : (((thingHolderWithDrawnPawn = p.ParentHolder as IThingHolderWithDrawnPawn) != null) ? thingHolderWithDrawnPawn.HeldPawnPosture : ((p.jobs != null) ? p.jobs.posture : PawnPosture.Standing))) : PawnPosture.Standing));
		...
		public static void ForceWait(Pawn pawn, int ticks, Thing faceTarget = null, bool maintainPosture = false, bool maintainSleep = false)
		{
			...
			{
				def = (pawn.IsSelfShutdown() ? JobDefOf.SelfShutdown : JobDefOf.Wait_Asleep);
			}
			...
			if (pawn.IsTeetotaler())
			{
				...
				if (ModsConfig.BiotechActive && pawn.genes != null)
				{
					foreach (Gene item in pawn.genes.GenesListForReading)
					{
						...
						if ((gene_ChemicalDependency = item as Gene_ChemicalDependency) != null && gene_ChemicalDependency.def.chemical == compProperties.chemical)
						{
							return true;
						}
					}
		...
		public static int GetPsylinkLevel(this Pawn pawn)
		{
			...
			foreach (Hediff hediff in pawn.health.hediffSet.hediffs)
			{
				...
				if ((hediff_Psylink = hediff as Hediff_Psylink) != null)
				{
		...
		{
			if (pawn.roping?.IsRopedToSpot ?? false)
			{
		...
		public static IEnumerable<PawnKindDef> GetCombatPawnKindsForPoints(Func<PawnKindDef, bool> selector, float points, Func<PawnKindDef, float> selectionWeight = null)
		{
			...
			selectionWeight = ((selectionWeight != null) ? selectionWeight : ((Func<PawnKindDef, float>)((PawnKindDef pk) => 1f)));
			PawnKindDef result;
		...
		{
			Corpse corpse;
			if (ModsConfig.IdeologyActive && (corpse = t as Corpse) != null && corpse.MapHeld != null)
			{
		...
		public static bool DutyActiveWhenDown(this Pawn pawn, bool onlyInBed = false)
		{
			...
			if ((lordJob_Ritual = pawn.GetLord()?.LordJob as LordJob_Ritual) != null && (ritualRole = lordJob_Ritual.assignments?.RoleForPawn(pawn)) != null && ritualRole.allowDowned)
			{
				return true;
			}
			return false;
