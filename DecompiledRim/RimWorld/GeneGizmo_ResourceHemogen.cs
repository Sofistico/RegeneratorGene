	...
	public class GeneGizmo_ResourceHemogen : GeneGizmo_Resource
	{
		...
		public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
		{
			...
			if (command_Ability != null && gene.Max != 0f)
			{
				...
				{
					CompAbilityEffect_HemogenCost compAbilityEffect_HemogenCost;
					if ((compAbilityEffect_HemogenCost = effectComp as CompAbilityEffect_HemogenCost) != null && compAbilityEffect_HemogenCost.Props.hemogenCost > float.Epsilon)
					{
		...
		protected override void DrawLabel(Rect labelRect, ref bool mouseOverAnyHighlightableElement)
		{
			...
			if ((gene.pawn.IsColonistPlayerControlled || gene.pawn.IsPrisonerOfColony) && (hemogenGene = gene as Gene_Hemogen) != null)
			{
				labelRect.xMax -= 24f;
				...
				if (Mouse.IsOver(rect))
				{
					...
					TooltipHandler.TipRegion(rect, () => "AutoTakeHemogenDesc".Translate(gene.pawn.Named("PAWN"), hemogenGene.targetValue.ToStringPercent().Named("MIN"), onOff.Named("ONOFF")).Resolve(), 828267373);
