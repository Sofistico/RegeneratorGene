...
namespace RimWorld
{
	public class Pawn_GeneTracker : IExposable
	{
		...
		public T GetFirstGeneOfType<T>() where T : Gene
		{
			...
			{
				T result;
				if (genesListForReading[i].Active && (result = genesListForReading[i] as T) != null)
				{
					return result;
				}
			}
		...
		public GeneDef GetMelaninGene()
		{
			...
			for (int i = 0; i < genesListForReading.Count; i++)
			{
				if (genesListForReading[i].def.endogeneCategory == EndogeneCategory.Melanin)
				{
		...
		public GeneDef GetHairColorGene()
		{
			...
			for (int i = 0; i < genesListForReading.Count; i++)
			{
				if (genesListForReading[i].def.endogeneCategory == EndogeneCategory.HairColor)
				{
					return genesListForReading[i].def;
				}
			}
		...
		public GeneDef GetFirstEndogeneByCategory(EndogeneCategory cat)
		{
			...
			for (int i = 0; i < genesListForReading.Count; i++)
			{
				if (genesListForReading[i].Active && genesListForReading[i].def.endogeneCategory == cat)
				{
					return genesListForReading[i].def;
				}
			}
		...
		public void RemoveGene(Gene gene)
		{
			...
			if (ModLister.BiotechInstalled)
			{
				...
				{
					SkillRecord skill = pawn.skills.GetSkill(gene.def.passionMod.skill);
					skill.passion = gene.def.passionMod.NewPassionForOnRemoval(skill);
				}
			}
			Notify_GenesChanged(gene.def);
			if (ModLister.BiotechInstalled)
		...
		private void Notify_GenesChanged(GeneDef addedOrRemovedGene)
		{
			...
			if (ModLister.BiotechInstalled && addedOrRemovedGene.graphicData != null && addedOrRemovedGene.graphicData.skinIsHairColor)
			{
				if (SelectGene((Gene x) => x.def.graphicData != null && x.def.graphicData.skinIsHairColor, out var _))
				{
			...
			if (addedOrRemovedGene.skinColorBase.HasValue)
			{
				if (SelectGene((Gene g) => g.def.skinColorBase.HasValue, out var chosen4))
				{
			...
			if (ModLister.BiotechInstalled)
			{
				...
				if (!addedOrRemovedGene.forcedHeadTypes.NullOrEmpty())
				{
					if (SelectGene((Gene g) => !g.def.forcedHeadTypes.NullOrEmpty(), out var chosen7))
					{
						...
						pawn.story.TryGetRandomHeadFromSet(chosen7.def.forcedHeadTypes);
					}
					else
					{
				...
				{
					if (SelectGene((Gene g) => g.def.graphicData?.fur != null, out var chosen8))
					{
						pawn.story.furDef = chosen8.def.graphicData.fur;
				...
				if (addedOrRemovedGene.RandomChosen && SelectGene((Gene g) => g.def.ConflictsWith(addedOrRemovedGene), out var chosen9))
				{
			...
			bool SelectGene(Predicate<Gene> validator, out Gene chosen)
			{
				...
				{
					if (validator(genes[i]))
					{
						genes[i].OverrideBy(null);
						tmpGenes.Add(genes[i]);
					}
				}
				if (tmpGenes.Where((Gene x) => IsXenogene(x)).TryRandomElement(out chosen))
				{
					tmpGenes.Clear();
					return true;
				}
		...
		public float FactorForDamage(DamageInfo dinfo)
		{
			...
			for (int i = 0; i < GenesListForReading.Count; i++)
			{
				if (GenesListForReading[i].def.damageFactors.NullOrEmpty())
				{
					continue;
				}
				for (int j = 0; j < GenesListForReading[i].def.damageFactors.Count; j++)
				{
					if (GenesListForReading[i].def.damageFactors[j].damageDef == dinfo.Def)
					{
						num *= GenesListForReading[i].def.damageFactors[j].factor;
					}
				}
			}
		...
		public float AddictionChanceFactor(ChemicalDef chemical)
		{
			...
			for (int i = 0; i < GenesListForReading.Count; i++)
			{
				if (GenesListForReading[i].def.chemical == chemical)
				{
		...
		public bool HediffGiversCanGive(HediffDef hediff)
		{
			...
			for (int i = 0; i < GenesListForReading.Count; i++)
			{
				if (GenesListForReading[i].def.hediffGiversCannotGive != null && GenesListForReading[i].def.hediffGiversCannotGive.Contains(hediff))
				{
					return false;
				}
			}
		...
		public SoundDef GetSoundOverrideFromGenes(Func<GeneDef, SoundDef> getter, SoundDef oldSound)
		{
			...
			for (int i = 0; i < genesListForReading.Count; i++)
			{
				if (!genesListForReading[i].Overridden)
				{
