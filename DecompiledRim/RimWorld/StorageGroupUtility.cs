	...
	public static class StorageGroupUtility
	{
		...
		public static IEnumerable<Gizmo> StorageGroupMemberGizmos(IStorageGroupMember member)
		{
			...
			foreach (object selectedObject in Find.Selector.SelectedObjects)
			{
				...
				if ((storageGroupMember = selectedObject as IStorageGroupMember) == null || !(storageGroupMember.StorageGroupTag == member.StorageGroupTag) || ((building_Storage = storageGroupMember as Building_Storage) != null && !building_Storage.StorageTabVisible) || storageGroupMember.Map != member.Map)
				{
					continue;
				}
			...
			Command_Action command_Action = new Command_Action
			{
				...
				{
					Thing thing;
					if (tmpMembers.Any() && (thing = tmpMembers[0] as Thing) != null)
					{
			...
			yield return new Command_Action
			{
				...
				action = delegate
				{
					if (member.Group != null)
					{
						...
						member.Group = null;
						Building_Storage building_Storage2;
						if ((building_Storage2 = member as Building_Storage) != null)
						{
			...
			yield return new Command_Action
			{
				...
				action = delegate
				{
					...
					{
						if (!Find.Selector.IsSelected(member2))
						{
		...
		public static void SetStorageGroup(this IStorageGroupMember member, StorageGroup newGroup)
		{
			if (member.Group == null || member.Group != newGroup)
			{
				...
				if ((storeSettingsParent = member as IStoreSettingsParent) != null)
				{
