...
namespace RimWorld
{
	public class Need_Food : Need
	{
		...
		public float FoodFallPerTickAssumingCategory(HungerCategory hunger, bool ignoreMalnutrition = false)
		{
			...
			float num = BaseHungerRate(pawn.ageTracker.CurLifeStage, pawn.def) * hunger.HungerMultiplier() * pawn.health.hediffSet.GetHungerRateFactor(ignoreMalnutrition ? null : HediffDefOf.Malnutrition) * (pawn.story?.traits?.HungerRateFactor ?? 1f) * (building_Bed?.GetStatValue(StatDefOf.BedHungerRateFactor) ?? 1f);
		...
		{
			return lifeStage.hungerRateFactor * pawnDef.race.baseHungerRate * 2.66666666E-05f;
		}
	}
}
