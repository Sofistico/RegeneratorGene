...
namespace RimWorld
{
	public static class RestUtility
	{
		...
		public static bool CanUseBedNow(Thing bedThing, Pawn sleeper, bool checkSocialProperness, bool allowMedBedEvenIfSetToNoCare = false, GuestStatus? guestStatusOverride = null)
		{
			...
			if ((building_Bed = bedThing as Building_Bed) == null)
			{
				return false;
			}
			...
			if (sleeper.IsColonist && !(sleeper.CurJob?.ignoreForbidden ?? false) && !sleeper.Downed && building_Bed.IsForbidden(sleeper))
			{
				return false;
			}
			return true;
