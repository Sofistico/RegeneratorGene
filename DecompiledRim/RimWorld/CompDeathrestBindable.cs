...
namespace RimWorld
{
	public class CompDeathrestBindable : ThingComp
	{
		...
		public override void CompTickRare()
		{
			...
			{
				PowerTraderComp.PowerOutput = 0f - ((presenceTicks > 0) ? Props.powerUsageDeathresting : Props.powerUsageNotDeathresting);
			}
		}
		...
		{
			if (!Mathf.Approximately(Props.deathrestEffectivenessFactor, 1f))
			{
				yield return new StatDrawEntry(StatCategoryDefOf.Building, "StatsReport_DeathrestEffectiveness".Translate(), Props.deathrestEffectivenessFactor.ToStringPercent(), "StatsReport_DeathrestEffectiveness_Desc".Translate(), 900);
			}
			if (!Props.mustBeLayingInToBind)
			{
				yield return new StatDrawEntry(StatCategoryDefOf.Building, "StatsReport_DeathrestConnectionLimit".Translate(), (Props.stackLimit <= 0) ? "Unlimited".Translate().ToString() : Props.stackLimit.ToString(), "StatsReport_DeathrestConnectionLimit_Desc".Translate(), 910);
			}
		}
