	...
	public static class GeneUIUtility
	{
		...
		private static void DoDebugButton(Rect buttonRect, Thing target, GeneSet genesOverride)
		{
			...
			if (sourcePawn != null)
			{
				...
				list.Add(new FloatMenuOption("Add all genes", delegate
				{
					sourcePawn.genes.Debug_AddAllGenes();
				}));
		...
		public static void DrawGeneDef(GeneDef gene, Rect geneRect, GeneType geneType, string extraTooltip, bool doBackground = true, bool clickable = true, bool overridden = false)
		{
		...
		public static void DrawBiostats(int gcx, int met, int arc, ref float curX, float curY, float margin = 6f)
		{
			...
			float num2 = 0f;
			float iconWidth = Text.LineHeightOf(GameFont.Small);
			Rect iconRect2 = new Rect(curX, curY + margin + num2, iconWidth, iconWidth);
			DrawStat(iconRect2, GeneUtility.GCXTex, gcx.ToString());
			Rect rect = new Rect(curX, iconRect2.y, 38f, iconWidth);
			if (Mouse.IsOver(rect))
			{
				Widgets.DrawHighlight(rect);
			...
			if (met != 0)
			{
				Rect iconRect3 = new Rect(curX, curY + margin + num2, iconWidth, iconWidth);
				DrawStat(iconRect3, GeneUtility.METTex, met.ToStringWithSign());
				Rect rect2 = new Rect(curX, iconRect3.y, 38f, iconWidth);
				if (Mouse.IsOver(rect2))
				{
					Widgets.DrawHighlight(rect2);
					TooltipHandler.TipRegion(rect2, "Metabolism".Translate().Colorize(ColoredText.TipSectionTitleColor) + "\n\n" + "MetabolismDesc".Translate());
				}
				num2 += num;
			}
			if (arc > 0)
			{
				Rect iconRect4 = new Rect(curX, curY + margin + num2, iconWidth, iconWidth);
				DrawStat(iconRect4, GeneUtility.ARCTex, arc.ToString());
				Rect rect3 = new Rect(curX, iconRect4.y, 38f, iconWidth);
				if (Mouse.IsOver(rect3))
				{
					Widgets.DrawHighlight(rect3);
					TooltipHandler.TipRegion(rect3, "ArchitesRequired".Translate().Colorize(ColoredText.TipSectionTitleColor) + "\n\n" + "ArchitesRequiredDesc".Translate());
				}
			}
			curX += 34f;
			void DrawStat(Rect iconRect, CachedTexture icon, string stat)
			{
