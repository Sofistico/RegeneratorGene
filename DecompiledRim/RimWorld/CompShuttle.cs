	...
	public class CompShuttle : ThingComp
	{
		...
		public void SendLaunchedSignals()
		{
			...
			for (int i = 0; i < transportersInGroup.Count; i++)
			{
				...
				{
					Pawn pawn;
					if ((pawn = transportersInGroup[i].innerContainer[j] as Pawn) != null && pawn.IsColonist && !requiredPawns.Contains(pawn))
					{
						list.Add(pawn);
					}
				}
		...
		public virtual bool IsAllowed(Thing t)
		{
			...
			{
				Pawn pawn = t as Pawn;
				if (pawn != null && (pawn.IsColonist || (pawn.IsPrisonerOfColony && acceptColonyPrisoners) || (!onlyAcceptColonists && pawn.RaceProps.Animal && pawn.Faction == Faction.OfPlayer)) && (!pawn.IsSlave || allowSlaves) && (!onlyAcceptColonists || !pawn.IsQuestLodger()) && (!onlyAcceptHealthy || PawnIsHealthyEnoughForShuttle(pawn)) && pawn.ageTracker.AgeBiologicalYearsFloat >= minAge)
				{
					return true;
				}
			}
			Pawn pawn2;
			if (!acceptChildren && (pawn2 = t as Pawn) != null && pawn2.RaceProps.Humanlike && !pawn2.DevelopmentalStage.Adult())
			{
				return false;
			}
			if (permitShuttle)
