	...
	public class Alert_Boredom : Alert
	{
		...
		private List<Pawn> BoredPawns
		{
			get
			{
				...
				{
					if (item.needs.joy != null && (item.needs.joy.CurLevelPercentage < 426f / (565f * (float)Math.PI) || item.GetTimeAssignment() == TimeAssignmentDefOf.Joy) && item.needs.joy.tolerances.BoredOfAllAvailableJoyKinds(item))
					{
