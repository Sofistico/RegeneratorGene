...
namespace RimWorld
{
	public class JobDriver_DeliverPawnToCell : JobDriver
	{
		...
		protected override IEnumerable<Toil> MakeNewToils()
		{
			...
			yield return Toils_Goto.GotoThing(TargetIndex.A, PathEndMode.Touch).FailOnDespawnedOrNull(TargetIndex.A).FailOn(() => !pawn.CanReach(job.GetTarget(TargetIndex.B), PathEndMode.Touch, Danger.Deadly))
			...
			yield return Toils_General.Do(delegate
			{
				...
				{
					(Takee.GetLord()?.LordJob as LordJob_Ritual)?.AddTagForPawn(Takee, job.ritualTag);
