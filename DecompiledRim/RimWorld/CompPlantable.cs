...
namespace RimWorld
{
	...
	public class CompPlantable : ThingComp
	{
		...
		private void BeginTargeting()
		{
			...
			{
				if (CanPlantAt(t.Cell, parent.Map).Accepted)
				{
		...
		public override void PostDrawExtraSelectionOverlays()
		{
			for (int i = 0; i < plantCells.Count; i++)
			{
				if (plantCells[i].IsValid)
				{
					...
					GenDraw.DrawLineBetween(parent.TrueCenter(), intVec.ToVector3Shifted(), AltitudeLayer.Blueprint.AltitudeFor());
		...
		{
			AcceptanceReport acceptanceReport = CanPlantAt(target.Cell, parent.Map);
			if (!acceptanceReport.Accepted)
			{
