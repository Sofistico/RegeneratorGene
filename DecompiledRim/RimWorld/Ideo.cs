	...
	public class Ideo : IExposable, ILoadReferenceable
	{
		...
		public Texture2D Icon
		{
			get
			{
				if (icon == null)
				{
					icon = ContentFinder<Texture2D>.Get((iconDef != null) ? iconDef.iconPath : BaseContent.BadTexPath);
				}
				return icon;
			}
		}
		...
		public Color Color
		{
			get
			{
				if (!hiddenIdeoMode)
				{
					Color? color = primaryFactionColor;
					if (!color.HasValue)
					{
						if (colorDef == null)
						{
			...
			get
			{
				if (cachedVeneratedAnimals == null)
				{
					...
					for (int i = 0; i < precepts.Count; i++)
					{
						...
						if ((precept_Animal = precepts[i] as Precept_Animal) != null)
						{
		...
		public string KeyDeityName
		{
			get
			{
				...
				if ((ideoFoundation_Deity = foundation as IdeoFoundation_Deity) != null && ideoFoundation_Deity.DeitiesListForReading.Any())
				{
		...
		public ThingDef RitualSeatDef
		{
			get
			{
				foreach (Precept item in PreceptsListForReading)
				{
					...
					if ((precept_RitualSeat = item as Precept_RitualSeat) != null)
					{
		...
		public void ExposeData()
		{
			...
			Scribe_Values.Look(ref classic, "classic", defaultValue: false);
			Scribe_Values.Look(ref hiddenIdeoMode, "hiddenIdeoMode", defaultValue: false);
			...
			if (ModsConfig.IdeologyActive && !hiddenIdeoMode && RitualSeatDef == null && DefDatabase<PreceptDef>.AllDefs.Where((PreceptDef x) => x.issue == IssueDefOf.IdeoRitualSeat).TryRandomElementByWeight((PreceptDef x) => x.selectionWeight, out var result))
			{
		...
		public void Notify_MemberGenerated(Pawn member, bool newborn)
		{
			for (int i = 0; i < precepts.Count; i++)
			{
				try
				{
					precepts[i].Notify_MemberGenerated(member, newborn);
				}
				catch (Exception ex)
				{
		...
		public void Notify_RelicSeenByPlayer(Thing relic)
		{
			...
			{
				TaggedString taggedString = (hiddenIdeoMode ? "LetterTextRelicsCollectedClassic".Translate() : "LetterTextRelicsCollected".Translate(this));
		...
		private bool AllRelicsNewlyCollected()
		{
			...
			for (int i = 0; i < precepts.Count; i++)
			{
				...
				if ((precept_Relic = precepts[i] as Precept_Relic) != null && !precept_Relic.RelicInPlayerPossession)
				{
					return false;
				}
			}
		...
		public Tuple<Precept_Ritual, List<string>> FirstRitualMissingTarget()
		{
			foreach (Precept precept in precepts)
			{
				...
				if ((precept_Ritual = precept as Precept_Ritual) != null && precept_Ritual.obligationTargetFilter != null)
				{
		...
		public Precept_Building FirstConsumableBuildingMissingRitual()
		{
			foreach (Precept precept in precepts)
			{
				...
				if ((precept_Building = precept as Precept_Building) != null && precept_Building.ThingDef.ritualFocus != null && precept_Building.ThingDef.ritualFocus.consumable && !HasRequiredRitualForBuilding(precept_Building))
				{
		...
		public void RemovePrecept(Precept precept, bool replacing = false)
		{
			...
			if ((precept_Role = precept as Precept_Role) != null)
			{
		...
		public bool MembersCanBuild(Thing thing)
		{
			...
			if ((buildingDef = buildableDef as ThingDef) != null)
			{
				...
				{
					Precept_RitualSeat precept_RitualSeat;
					if ((precept_RitualSeat = precepts[i] as Precept_RitualSeat) != null && precept_RitualSeat.ThingDef == buildableDef)
					{
						return true;
					}
				}
		...
		public bool IsVeneratedAnimal(ThingDef thingDef)
		{
			...
			{
				Precept_Animal precept_Animal;
				if ((precept_Animal = precept as Precept_Animal) != null && thingDef == precept_Animal.ThingDef)
				{
					return true;
				}
			}
		...
		public IdeoWeaponDisposition GetDispositionForWeapon(ThingDef td)
		{
			...
			foreach (Precept precept in precepts)
			{
				...
				if ((precept_Weapon = precept as Precept_Weapon) != null)
				{
		...
		public T GetFirstPreceptOfType<T>() where T : class
		{
			...
			{
				T result;
				if ((result = precepts[i] as T) != null)
				{
					return result;
				}
			}
		...
		private void RecachePossibleSituationalThoughts()
		{
			...
			for (int i = 0; i < precepts.Count; i++)
			{
				...
				{
					PreceptComp_SituationalThought preceptComp_SituationalThought;
					if ((preceptComp_SituationalThought = precepts[i].def.comps[j] as PreceptComp_SituationalThought) != null && !cachedPossibleSituationalThoughts.Contains(preceptComp_SituationalThought.thought))
					{
		...
		private void RecachePossibleGoodwillSituations()
		{
			...
			for (int i = 0; i < precepts.Count; i++)
			{
				for (int j = 0; j < precepts[i].def.comps.Count; j++)
				{
					...
					if ((preceptComp_GoodwillSituation = precepts[i].def.comps[j] as PreceptComp_GoodwillSituation) != null && !cachedPossibleGoodwillSituations.Contains(preceptComp_GoodwillSituation.goodwillSituation))
					{
		...
		public void RecachePossibleBuildings()
		{
			...
			for (int i = 0; i < precepts.Count; i++)
			{
				...
				if ((item = precepts[i] as Precept_Building) != null)
				{
		...
		public void RecachePossibleRoles()
		{
			...
			{
				Precept_Role item;
				if ((item = precepts[i] as Precept_Role) != null)
				{
		...
		public void RegenerateAllApparelRequirements(FactionDef generatingFor = null)
		{
			...
			{
				Precept_Role precept_Role;
				if ((precept_Role = item as Precept_Role) != null)
				{
		...
		public StyleCategoryPair GetStyleAndCategoryFor(ThingDef thingDef)
		{
			...
			for (int i = 0; i < precepts.Count; i++)
			{
				...
				if ((precept_ThingDef = precepts[i] as Precept_ThingDef) != null && precept_ThingDef.ThingDef == thingDef)
				{
