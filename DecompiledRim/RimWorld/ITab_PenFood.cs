...
namespace RimWorld
{
	public class ITab_PenFood : ITab_PenBase
	{
		...
		private void DrawTopPane(ref float curY, float width, PenFoodCalculator calc)
		{
			...
			if (flag)
			{
				DrawStatLine(value: Mathf.FloorToInt(calc.sumStockpiledNutritionAvailableNow / num).ToString(), label: "PenFoodTab_StockpileEmptyDays".Translate(), curY: ref curY, width: width, toolipGetter: () => "PenFoodTab_StockpileEmptyDaysDescription".Translate(), valueColor: Color.red);
			}
		}
		...
		private void DrawAnimalPane(ref float curYOuter, float width, float height, PenFoodCalculator calc, Map map)
		{
			...
			Widgets.Dropdown(rect3.LeftPart(0.35f), calc, (PenFoodCalculator calculator) => null, MenuGenerator, "PenFoodTab_AddAnimal".Translate());
			curY = rect3.yMax;
			curYOuter = curY;
			void DrawCell(string label, ref float x, float cellWidth, float cellHeight, TextAnchor anchor, Color? color, Func<string> tooltip)
			{
				if (label != null)
				{
					Rect rect4 = new Rect(x, curY, cellWidth, cellHeight);
					Text.Anchor = anchor;
					if (color.HasValue)
					{
			...
			{
				if (Widgets.ButtonImage(new Rect(x, curY, cellWidth, cellHeight), TexButton.DeleteX, Color.white, GenUI.SubtleMouseoverColor))
				{
			...
			void DrawIconCell(ThingDef thingDef, ref float x, float cellWidth, float cellHeight)
			{
				if (thingDef != null)
				{
					Rect rect5 = new Rect(x, curY, 27f, 27f);
					Widgets.ThingIcon(rect5, thingDef);
					rect5.x += 29f;
					Widgets.InfoCardButton(rect5.x, rect5.y + 2f, thingDef);
				}
				x += cellWidth + 4f;
			}
