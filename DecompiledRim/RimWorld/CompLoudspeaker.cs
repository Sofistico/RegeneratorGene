	...
	public class CompLoudspeaker : CompAutoPowered
	{
		...
		private void UpdateInRitual()
		{
			...
			foreach (Lord lord in parent.Map.lordManager.lords)
			{
				...
				if ((lordJob_Ritual = lord.LordJob as LordJob_Ritual) != null && lordJob_Ritual.selectedTarget.Thing != null && lordJob_Ritual.selectedTarget.Thing.def == ThingDefOf.LightBall && parent.GetRoom() == lordJob_Ritual.selectedTarget.Thing.GetRoom())
				{
					inRitual = true;
					break;
				}
