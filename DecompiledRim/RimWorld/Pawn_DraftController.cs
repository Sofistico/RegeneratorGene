		...
		public bool Drafted
		{
			...
			set
			{
				...
				if (pawn.jobs.curJob != null && pawn.jobs.IsCurrentJobPlayerInterruptible())
				{
					...
					if ((jobDriver_Ingest = pawn.jobs.curDriver as JobDriver_Ingest) != null && jobDriver_Ingest.EatingFromInventory)
					{
					...
					{
						Pawn baby;
						if ((baby = pawn.carryTracker.CarriedThing as Pawn) != null && ChildcareUtility.CanSuckle(baby, out reason))
						{
					...
					{
						Pawn baby2;
						if ((baby2 = pawn.carryTracker.CarriedThing as Pawn) != null && ChildcareUtility.CanSuckle(baby2, out reason))
						{
