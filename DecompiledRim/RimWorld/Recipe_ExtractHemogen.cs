...
namespace RimWorld
{
	...
	{
		private const float BloodlossSeverityIndustrial = 0.45f;

		private const float BloodlossSeverityHerbal = 0.65f;
		...
		{
			Pawn pawn;
			if ((pawn = thing as Pawn) != null && pawn.genes != null && pawn.genes.HasGene(GeneDefOf.Hemogenic))
			{
				return false;
			}
		...
		public override void ApplyOnPawn(Pawn pawn, BodyPartRecord part, Pawn billDoer, List<Thing> ingredients, Bill bill)
		{
			...
			float severity = 0.45f;
			for (int i = 0; i < ingredients.Count; i++)
			{
				if (ingredients[i].GetStatValue(StatDefOf.MedicalPotency) < 1f)
				{
					severity = 0.65f;
					break;
				}
