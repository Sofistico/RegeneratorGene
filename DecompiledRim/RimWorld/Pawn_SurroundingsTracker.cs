...
namespace RimWorld
{
	public class Pawn_SurroundingsTracker : IExposable
	{
		...
		public void SurroundingsTrackerTick()
		{
			...
			SkullspikeSighting item;
			for (int num = skullspikeSightings.Count - 1; num >= 0; num--)
			{
				item = skullspikeSightings[num];
				if (item.TicksSinceSighting > 1800)
				{
			...
			for (int i = 0; i < num2; i++)
			{
				...
				if (!flag)
				{
					List<SkullspikeSighting> list = skullspikeSightings;
					item = new SkullspikeSighting
					{
