...
namespace RimWorld
{
	public class Pawn_IdeoTracker : IExposable
	{
		...
		{
			if (!pawn.Destroyed && !pawn.InMentalState && ideo != null && !Find.IdeoManager.classicMode)
			{
		...
		{
			if (!ModsConfig.BiotechActive || !ModsConfig.IdeologyActive || ideo == null || Find.IdeoManager.classicMode || !pawn.DevelopmentalStage.Baby())
			{
				return;
			}
		...
		{
			if (pawn.Faction != null && pawn.Faction.ideos.PrimaryIdeo != excludeIdeo && pawn.Faction.ideos.PrimaryIdeo != null)
			{
