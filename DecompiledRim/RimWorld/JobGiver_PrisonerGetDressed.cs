			...
			if (room.IsPrisonCell)
			{
				foreach (IntVec3 cell in room.Cells)
				{
					...
					for (int i = 0; i < thingList.Count; i++)
					{
						...
						if (apparel != null && apparel.def.apparel.bodyPartGroups.Contains(bodyPartGroupDef) && pawn.CanReserveAndReach(apparel, PathEndMode.OnCell, pawn.NormalMaxDanger()) && !apparel.IsBurning() && (!CompBiocodable.IsBiocoded(apparel) || CompBiocodable.IsBiocodedFor(apparel, pawn)) && ApparelUtility.HasPartsToWear(pawn, apparel.def))
						{
							return apparel;
						}
					}
		...
		private Apparel FindGarmentSatisfyingTitleRequirement(Pawn pawn, ApparelRequirement req)
		{
			...
			if (room.IsPrisonCell)
			{
				foreach (IntVec3 cell in room.Cells)
				{
					...
					{
						Apparel apparel = thingList[i] as Apparel;
						if (apparel != null && req.ApparelMeetsRequirement(thingList[i].def, allowUnmatched: false) && pawn.CanReserveAndReach(apparel, PathEndMode.OnCell, pawn.NormalMaxDanger()) && !apparel.IsBurning() && (!CompBiocodable.IsBiocoded(apparel) || CompBiocodable.IsBiocodedFor(apparel, pawn)) && ApparelUtility.HasPartsToWear(pawn, apparel.def))
						{
							return apparel;
						}
					}
