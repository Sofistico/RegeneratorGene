					...
					if (!hediffCompProperties_VerbGiver.verbs.NullOrEmpty())
					{
						...
						if (!verb.IsMeleeAttack)
						{
							if (verb.defaultProjectile != null)
							{
								...
								yield return new StatDrawEntry(valueString: verb.defaultProjectile.projectile.GetDamageAmount(null, stringBuilder).ToString(), category: StatCategoryDefOf.Basics, label: "Damage".Translate() + extraLabelPart, reportText: stringBuilder.ToString(), displayPriorityWithinCategory: 5500);
