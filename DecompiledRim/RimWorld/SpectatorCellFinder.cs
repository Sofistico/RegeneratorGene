			...
			if (ritual != null && ritual.ideo.RitualSeatDef != null)
			{
					...
					select t)
				{
					...
					foreach (IntVec3 item2 in item.OccupiedRect())
					{
						...
						if (!((float)intVec.DistanceToSquared(item2) > 210.25f) && p.CanReach(item2, PathEndMode.OnCell, Danger.Deadly) && p.CanReserveSittableOrSpot(item2) && GenSight.LineOfSight(intVec, item2, map, skipFirstCell: true))
						{
							cell = item2;
							return true;
						}
