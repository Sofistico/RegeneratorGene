	...
	public class SymbolResolver_WorkSite_Mining : SymbolResolver_WorkSite
	{
		...
		public override void Resolve(ResolveParams rp)
		{
			...
			if (rp.workSitePoints >= 500f && (int)rp.faction.def.techLevel >= (int)techLevel)
			{
				...
				for (int i = 0; i < num; i++)
				{
					SymbolStack symbolStack = BaseGen.symbolStack;
					resolveParams = new ResolveParams
					{
						rect = rect3,
						singleThingDef = ThingDefOf.DeepDrill
					};
					symbolStack.Push("thing", resolveParams);
				}
				SymbolStack symbolStack2 = BaseGen.symbolStack;
				resolveParams = new ResolveParams
				{
					rect = rect2,
					singleThingDef = ThingDefOf.GroundPenetratingScanner
				};
				symbolStack2.Push("thing", resolveParams);
			}
			...
			foreach (IntVec3 start in GenRadial.RadialCellsAround(centerCell, 30f, useCenter: false))
			{
				...
				PawnPath pawnPath = map.pathFinder.FindPath(end, centerCell, TraverseParms.For(TraverseMode.PassDoors));
				PawnPath pawnPath2 = map.pathFinder.FindPath(start, centerCell, TraverseParms.For(TraverseMode.PassDoors));
				if (pawnPath.Found || pawnPath2.Found)
				{
					float num3 = -1f;
					num3 = ((pawnPath.Found && !pawnPath2.Found) ? pawnPath.TotalCost : ((!pawnPath2.Found || pawnPath.Found) ? Mathf.Min(pawnPath.TotalCost, pawnPath2.TotalCost) : pawnPath2.TotalCost));
					TunnelCandidate tunnelCandidate = default(TunnelCandidate);
					tunnelCandidate.start = start;
					tunnelCandidate.end = end;
					tunnelCandidate.pathCost = num3;
