...
namespace RimWorld
{
	public class MechanitorControlGroupGizmo : Gizmo
	{
		...
		{
			MechanitorControlGroupGizmo mechanitorControlGroupGizmo;
			if ((mechanitorControlGroupGizmo = other as MechanitorControlGroupGizmo) == null)
			{
				return false;
			}
		...
		{
			MechanitorControlGroupGizmo mechanitorControlGroupGizmo;
			if ((mechanitorControlGroupGizmo = gizmo as MechanitorControlGroupGizmo) == null)
			{
		...
		public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
		{
			...
			for (int k = 0; k < num2; k++)
			{
				for (int l = 0; l < num2; l++)
				{
					...
					RenderTexture image = PortraitsCache.Get(mechsForReading[num7], rect7.size, Rot4.East, default(Vector3), mechsForReading[num7].kindDef.controlGroupPortraitZoom);
			...
			{
				string tip = ((string)("ControlGroup".Translate() + " #") + controlGroup.Index).Colorize(ColoredText.TipSectionTitleColor) + "\n\n";
				tip = tip + ("CurrentMechWorkMode".Translate() + ": " + controlGroup.WorkMode.LabelCap).Colorize(ColoredText.TipSectionTitleColor) + "\n" + controlGroup.WorkMode.description + "\n\n";
				IEnumerable<string> entries = controlGroup.MechsForReading.Select((Pawn m) => (m.LabelCap + " (" + m.needs.energy.CurLevelPercentage.ToStringPercent() + " " + "EnergyLower".Translate() + ")").Resolve());
				tip = tip + "AssignedMechs".Translate().Colorize(ColoredText.TipSectionTitleColor) + "\n" + entries.ToLineList(" - ");
				if (disabled && !disabledReason.NullOrEmpty())
				{
					tip += ("\n\n" + "DisabledCommand".Translate() + ": " + disabledReason).Colorize(ColorLibrary.RedReadable);
				}
				TooltipHandler.TipRegion(rect, () => tip, 2545872);
			}
