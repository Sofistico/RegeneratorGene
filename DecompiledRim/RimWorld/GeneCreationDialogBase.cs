...
namespace RimWorld
{
	...
	public abstract class GeneCreationDialogBase : Window
	{
		...
		public override void DoWindowContents(Rect rect)
		{
			...
			string text = "XenotypeName".Translate() + ":";
		...
		protected virtual void OnGenesChanged()
		{
			...
			for (int i = 0; i < selectedGenes.Count; i++)
			{
				...
				for (int j = 0; j < selectedGenes.Count; j++)
				{
					if (selectedGenes[i] != selectedGenes[j] && selectedGenes[i].ConflictsWith(selectedGenes[j]))
					{
			...
			{
				if (selectedGenes[k].RandomChosen || cachedOverriddenGenes.Contains(selectedGenes[k]))
				{
					continue;
				}
			...
			foreach (GeneDef item in tmpGenesWithType.NonOverriddenGenes())
			{
				gcx += item.biostatCpx;
				met += item.biostatMet;
				arc += item.biostatArc;
