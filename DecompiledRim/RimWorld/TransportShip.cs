...
namespace RimWorld
{
	public class TransportShip : IExposable, ILoadReferenceable
	{
		...
		private bool CanDispose
		{
			get
			{
				TransportPodsArrivalAction_TransportShip transportPodsArrivalAction_TransportShip;
				if (!started || curJob != null || shipThing.Spawned || shipJobs.Any((ShipJob x) => x.def.blocksDisposalIfQueuedUnspawned) || Find.World.worldObjects.TravelingTransportPods.Any((TravelingTransportPods x) => (transportPodsArrivalAction_TransportShip = x.arrivalAction as TransportPodsArrivalAction_TransportShip) != null && transportPodsArrivalAction_TransportShip.transportShip == this) || Find.QuestManager.IsReservedByAnyQuest(this))
				{
					return false;
				}
				return true;
