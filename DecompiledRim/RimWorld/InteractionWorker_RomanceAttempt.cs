...
namespace RimWorld
{
	public class InteractionWorker_RomanceAttempt : InteractionWorker
	{
		...
		public static string RomanceFactors(Pawn romancer, Pawn romanceTarget)
		{
			...
			if (ModsConfig.BiotechActive)
			{
				if (romancer.genes != null)
				{
					...
					{
						if (genesListForReading[i].Active && genesListForReading[i].def.missingGeneRomanceChanceFactor != 1f && (romanceTarget.genes == null || !romanceTarget.genes.HasGene(genesListForReading[i].def)))
						{
				...
				if (romanceTarget.genes != null)
				{
					...
					{
						if (genesListForReading2[j].Active && genesListForReading2[j].def.missingGeneRomanceChanceFactor != 1f && (romancer.genes == null || !romancer.genes.HasGene(genesListForReading2[j].def)))
						{
		...
		public override void Interacted(Pawn initiator, Pawn recipient, List<RulePackDef> extraSentencePacks, out string letterText, out string letterLabel, out LetterDef letterDef, out LookTargets lookTargets)
		{
			...
			if (Rand.Value < SuccessChance(initiator, recipient))
			{
				...
				if (CanCreatePsychicBondBetween(initiator, recipient))
				{
					...
					else if (gene_PsychicBonding2 != null && gene_PsychicBonding2.CanBondToNewPawn)
					{
		...
		private static bool CanCreatePsychicBondBetween(Pawn initiator, Pawn recipient)
		{
			if (!ModsConfig.BiotechActive)
			{
				return false;
