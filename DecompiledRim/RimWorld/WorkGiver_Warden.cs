...
namespace RimWorld
{
	public abstract class WorkGiver_Warden : WorkGiver_Scanner
	{
		...
		protected bool ShouldTakeCareOfSlave(Pawn warden, Thing slave, bool forced = false)
		{
			...
			if (pawn == null || !pawn.IsSlaveOfColony || !pawn.guest.SlaveIsSecure || !pawn.Spawned || pawn.InAggroMentalState || pawn.IsForbidden(warden) || pawn.IsFormingCaravan() || !warden.CanReserveAndReach(pawn, PathEndMode.Touch, warden.NormalMaxDanger(), 1, -1, null, forced))
			{
				return false;
			}
			return true;
