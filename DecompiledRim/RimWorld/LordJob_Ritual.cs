...
namespace RimWorld
{
	public class LordJob_Ritual : LordJob_Joinable_Gathering
	{
		...
		private static List<Pawn> tmpParticipants = new List<Pawn>(8);
		...
		public virtual bool BlocksDrafting => true;
		...
		{
			if (reason == PawnLostCondition.Incapped && (assignments.RoleForPawn(p)?.allowDowned ?? false))
			{
				return false;
			}
			...
			if (stages != null)
			{
				for (int i = 0; i < stages.Count; i++)
				{
					...
					foreach (Pawn participant in assignments.Participants)
					{
						...
						ritualStagePositions[i].positions.Add(participant, pawnPosition);
					}
		...
		public override StateGraph CreateGraph()
		{
			...
			for (int j = 0; j < stages.Count; j++)
			{
				...
				lordToil_Ritual2.startAction = delegate
				{
					...
					foreach (KeyValuePair<Pawn, PawnStagePosition> position in ritualStagePositions[iCapture].positions)
					{
						if (position.Value.highlight)
						{
							TryAddPosHighlight(position.Value.cell);
						}
					}
				...
				if (!stage.failTriggers.NullOrEmpty())
				{
					foreach (StageFailTrigger f in stage.failTriggers)
					{
						...
						{
							ApplyOutcome(Progress(), showFinishedMessage: true, showFailedMessage: false);
						}));
		...
		{
			return BehaviorFor(pawn, includeForced: true)?.GetJobReportOverride(pawn) ?? pawn.GetJobReport();
		}
			...
			if (RitualFinished(progress, cancelled))
			{
				...
				if (Ritual != null && Ritual.ideo != null)
				{
					foreach (Precept item in Ritual.ideo.PreceptsListForReading)
					{
						...
						if ((precept_Ritual = item as Precept_Ritual) == null || precept_Ritual.obligationTriggers.NullOrEmpty())
						{
							continue;
						}
		...
		protected override bool ShouldBeCalledOff()
		{
			...
			foreach (Pawn item2 in assignments.ExtraRequiredPawnsForReading)
			{
				...
				Pawn pawn2 = thingHolder as Pawn;
				if (pawn2 != null && pawn2.GetLord() != lord)
				{
					return true;
				}
			}
		...
		{
			if (p != organizer && !assignments.ExtraRequiredPawnsForReading.Contains(p))
			{
				Command_Action command_Action = new Command_Action();
				command_Action.defaultLabel = "CommandLeaveRitual".Translate(ritual.Named("RITUAL"));
				command_Action.defaultDesc = "CommandLeaveRitualDesc".Translate(ritual.Named("RITUAL"));
		...
		public override void LordJobTick()
		{
			...
			if (ritual != null)
			{
				...
				if ((lordToil_Ritual = lord.CurLordToil as LordToil_Ritual) != null && lordToil_Ritual.stage != null)
				{
			...
			if ((lordToil_Ritual2 = lord.CurLordToil as LordToil_Ritual) != null)
			{
		...
		public override void Notify_PawnLost(Pawn p, PawnLostCondition condition)
		{
			...
			LordToil_Ritual lordToil_Ritual;
			if ((lordToil_Ritual = lord.CurLordToil as LordToil_Ritual) != null)
			{
		...
		public PawnStagePosition PawnPositionForStage(Pawn pawn, RitualStage stage)
		{
			...
			Dictionary<Pawn, PawnStagePosition> positions = ritualStagePositions[num].positions;
		...
		public override void ExposeData()
		{
			...
			Scribe_Collections.Look(ref pawnsDeathIgnored, true, "pawnsDeathIgnored", LookMode.Reference);
			Scribe_Collections.Look(ref perPawnTags, "perPawnTags", LookMode.Reference, LookMode.Deep, ref tmpTagPawns, ref tmpTags);
