	...
	public class Dialog_CreateXenotype : GeneCreationDialogBase
	{
		...
		protected override bool CanAccept()
		{
			...
			for (int i = 0; i < selectedGenes.Count; i++)
			{
				...
				{
					Messages.Message("MessageGeneMissingPrerequisite".Translate(selectedGenes[i].label).CapitalizeFirst() + ": " + selectedGenes[i].prerequisite.LabelCap, null, MessageTypeDefOf.RejectInput, historical: false);
					return false;
				}
			}
		...
		private IEnumerable<string> GetWarnings()
		{
			if (ignoreRestrictions)
			{
				...
				{
					yield return "XenotypeBreaksLimits_Exceeds".Translate("Metabolism".Translate().ToLower().Named("STAT"), met.Named("VALUE"), GeneTuning.BiostatRange.TrueMax.Named("MAX"));
				}
				else if (met < GeneTuning.BiostatRange.TrueMin)
				{
					yield return "XenotypeBreaksLimits_Below".Translate("Metabolism".Translate().ToLower().Named("STAT"), met.Named("VALUE"), GeneTuning.BiostatRange.TrueMin.Named("MIN"));
				}
			}
		}
