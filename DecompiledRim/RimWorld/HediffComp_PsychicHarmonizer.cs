...
namespace RimWorld
{
	public class HediffComp_PsychicHarmonizer : HediffComp
	{
		...
		private void AffectPawns(Pawn p, List<Pawn> pawns)
		{
			...
			{
				Pawn pawn = pawns[i];
				if (p == pawn || !p.RaceProps.Humanlike || pawn == null || pawn.needs == null || pawn.needs.mood == null || pawn.needs.mood.thoughts == null || pawn.Position.DistanceTo(p.Position) > Props.range || pawn.health.hediffSet.HasHediff(HediffDefOf.PsychicHarmonizer))
				{
					continue;
				}
				bool flag = false;
