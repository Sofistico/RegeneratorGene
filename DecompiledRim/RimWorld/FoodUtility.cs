...
namespace RimWorld
{
	public static class FoodUtility
	{
		...
		{
			if (!p.WillEat(food.def, getter, careIfNotAcceptableForTitle))
			{
				return false;
			}
			if (IsVeneratedAnimalMeatOrCorpseOrHasIngredients(food, p))
			{
				return false;
			}
			return true;
		...
		public static bool WillEat(this Pawn p, ThingDef food, Pawn getter = null, bool careIfNotAcceptableForTitle = true)
		{
			...
			if (p.foodRestriction != null)
			{
				...
				else
				{
					...
					if (currentRespectedRestriction != null && !currentRespectedRestriction.Allows(food) && Dialog_ManageFoodRestrictions.FoodGlobalFilter.Allows(food))
					{
						return false;
					}
				}
			}
			if (IsVeneratedAnimalMeatOrCorpse(food, p))
			{
				return false;
			}
			...
			Thing FirstFoodInClosestPackAnimalInventory(FoodPreferability foodPref)
			{
				...
				if (canUseInventory && canUsePackAnimalInventory && canManipulateTools && eater.IsColonist && getter.IsColonist && getter.Map != null)
				{
					Thing thing3 = null;
					{
						...
						{
							thing3 = BestFoodInInventory(spawnedColonyAnimal, eater, foodPref);
		...
		public static Thing BestFoodInInventory(Pawn holder, Pawn eater = null, FoodPreferability minFoodPref = FoodPreferability.NeverForNutrition, FoodPreferability maxFoodPref = FoodPreferability.MealLavish, float minStackNutrition = 0f, bool allowDrug = false)
		{
			...
			for (int i = 0; i < innerContainer.Count; i++)
			{
				...
				if (thing.def.IsNutritionGivingIngestible && thing.IngestibleNow && eater.WillEat(thing, holder) && (int)thing.def.ingestible.preferability >= (int)minFoodPref && (int)thing.def.ingestible.preferability <= (int)maxFoodPref && (allowDrug || !thing.def.IsDrug) && NutritionForEater(eater, thing) * (float)thing.stackCount >= minStackNutrition)
				{
					return thing;
				}
			}
		...
		{
			if (!foodDef.IsDrug && !foodDef.IsProcessedFood && foodDef.ingestible.drugCategory != DrugCategory.Medical)
			{
				return foodDef.ingestible.CachedNutrition <= 0f;
			}
			return true;
		}
