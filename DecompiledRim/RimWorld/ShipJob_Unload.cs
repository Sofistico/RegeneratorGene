	...
	public class ShipJob_Unload : ShipJob
	{
		...
		private void Drop()
		{
			...
			if (thingToDrop != null)
			{
				...
				if (!transportShip.TransporterComp.innerContainer.TryDrop(thingToDrop, dropLoc, map, ThingPlaceMode.Near, out var _, null, delegate(IntVec3 c)
				{
					...
					Pawn pawn2;
					return ((pawn2 = thingToDrop as Pawn) == null || !pawn2.Downed || c.GetFirstPawn(map) == null) ? true : false;
				}, !(thingToDrop is Pawn)))
				{
					return;
				}
		...
		private float GetDropPriority(Thing t)
		{
			...
			if ((p = t as Pawn) != null)
			{
				...
				if (lord?.CurLordToil != null && (lordToil_EnterShuttleOrLeave = lord.CurLordToil as LordToil_EnterShuttleOrLeave) != null && lordToil_EnterShuttleOrLeave.shuttle == ShipThing)
				{
					return 0f;
				}
