	...
	public class QuestNode_Root_Mission_AncientComplex : QuestNode_Root_AncientComplex
	{
		...
		protected override void RunInt()
		{
			if (ModLister.CheckRoyaltyAndIdeology("Ancient Complex mission"))
			{
				...
				int population = (slate.Exists("population") ? slate.Get("population", 0) : map.mapPawns.FreeColonists.Where(IsPawnAllowed).Count());
				...
				quest.Signal(text7, delegate
				{
					...
					quest.AddShipJob_WaitSendable(transportShip, map.Parent, leaveImmeiatelyWhenSatisfied: true);
					quest.AddShipJob(transportShip, ShipJobDefOf.Unload);
		...
		{
			if (!pawn.IsSlave && !pawn.IsQuestLodger() && !pawn.Downed)
			{
