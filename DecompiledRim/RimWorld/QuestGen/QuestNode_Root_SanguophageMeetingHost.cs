...
namespace RimWorld.QuestGen
{
	public class QuestNode_Root_SanguophageMeetingHost : QuestNode
	{
		...
		private static readonly List<CountChance> SanguophageCountChances;

		private static readonly IntRange MeetingDurationRange;

		private static readonly IntRange EventDelayRange;

		private static readonly IntRange ArrivalDelayRange;

		private static readonly SimpleCurve RewardValueCurve;
		...
		protected override void RunInt()
		{
			...
			Action argumentEvent = delegate
			{
				quest.InnerFactionFight(gatherSpot, map.Parent, sanguophages, faction, FactionDefOf.Sanguophages, delegate
				{
					quest.SendSignals(Gen.YieldSingle(meetingCompletedSignal));
				}, null, null, factionBecameHostileSignal);
				quest.Letter(LetterDefOf.ThreatSmall, null, null, null, null, useColonistsFromCaravanArg: false, QuestPart.SignalListenMode.OngoingOnly, sanguophages, filterDeadPawnsFromLookTargets: true, "[argumentLetterText]", null, "[argumentLetterLabel]");
			};
		...
		static QuestNode_Root_SanguophageMeetingHost()
		{
			List<CountChance> list = new List<CountChance>();
			CountChance item = new CountChance
			{
				count = 2,
				chance = 0.45f
			};
			list.Add(item);
			item = new CountChance
			{
				count = 3,
				chance = 0.25f
			};
			...
			list.Add(item);
			SanguophageCountChances = list;
			MeetingDurationRange = new IntRange(10000, 20000);
			EventDelayRange = new IntRange(5000, 7500);
			ArrivalDelayRange = new IntRange(2500, 7500);
			RewardValueCurve = new SimpleCurve
			{
