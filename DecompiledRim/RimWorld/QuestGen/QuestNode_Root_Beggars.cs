...
namespace RimWorld.QuestGen
{
	public class QuestNode_Root_Beggars : QuestNode
	{
		...
		protected override void RunInt()
		{
			...
			int num2 = Mathf.Max(Mathf.RoundToInt(LodgerCountBasedOnColonyPopulationFactorRange.RandomInRange * (float)num), 1);
			...
			if (Find.Storyteller.difficulty.ChildrenAllowed)
			{
				...
				{
					(0, 0.4f),
					(Rand.Range(1, Mathf.Max(1, num2 / 2)), 0.4f),
					(num2, 0.2f)
				}.TryRandomElementByWeight(((int, float) p) => p.Item2, out var result);
				num3 = result.Item1;
				var = num2 == num3;
			}
			slate.Set("childCount", num3);
			slate.Set("allChildren", var);
			List<Pawn> pawns = new List<Pawn>();
			for (int i = 0; i < num2; i++)
			{
				DevelopmentalStage developmentalStages = ((i >= num2 - num3) ? DevelopmentalStage.Child : DevelopmentalStage.Adult);
			...
			string pawnLabelSingleOrPlural = ((num2 > 1) ? faction.def.pawnsPlural : faction.def.pawnSingular);
