	...
	public class QuestNode_Root_WorkSite : QuestNode
	{
		...
		public static Site GenerateSite(float points, int aroundTile)
		{
			...
			if (faction == null)
			{
				...
				FactionGeneratorParms parms = new FactionGeneratorParms(DefDatabase<FactionDef>.AllDefsListForReading.Where((FactionDef def) => FactionDefUseable(def)).RandomElement(), default(IdeoGenerationParms), true);
			...
			{
				if (f.ideos != null && f.ideos.PrimaryIdeo != null && sitePartWorker.DisallowedPrecepts.Any((PreceptDef p) => f.ideos.PrimaryIdeo.PreceptsListForReading.Any((Precept p2) => p2.def == p)))
				{
					return false;
				}
