...
namespace RimWorld
{
	public class QuestManager : IExposable
	{
		...
		public bool IsReservedByAnyQuest(Pawn p)
		{
			...
			{
				if (activeQuests[i].QuestReserves(p))
				{
					return true;
				}
			}
		...
		public bool IsReservedByAnyQuest(Faction f)
		{
			for (int i = 0; i < activeQuests.Count; i++)
			{
				if (activeQuests[i].QuestReserves(f))
				{
					return true;
				}
			}
		...
		public bool IsReservedByAnyQuest(TransportShip ship)
		{
			for (int i = 0; i < activeQuests.Count; i++)
			{
				if (activeQuests[i].QuestReserves(ship))
				{
					return true;
				}
			}
		...
		private void AddToCache(Quest quest)
		{
			...
			for (int i = 0; i < quest.PartsListForReading.Count; i++)
			{
				...
				QuestPart_ExtraFaction item;
				if ((item = quest.PartsListForReading[i] as QuestPart_ExtraFaction) != null)
				{
					cachedExtraFactionQuestParts.Add(item);
				}
			}
		}
		...
		private void RemoveFromCache(Quest quest)
		{
			...
			for (int i = 0; i < quest.PartsListForReading.Count; i++)
			{
				...
				if ((item = quest.PartsListForReading[i] as QuestPart_ExtraFaction) != null)
				{
					cachedExtraFactionQuestParts.Remove(item);
				}
			}
		}
