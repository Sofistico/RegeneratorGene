	...
	public class JobGiver_GetHemogen : ThinkNode_JobGiver
	{
		...
		public static float HemogenPackHemogenGain
		{
			get
			{
				if (!cachedHemogenPackHemogenGain.HasValue)
				{
					...
					{
						IngestionOutcomeDoer_OffsetHemogen ingestionOutcomeDoer_OffsetHemogen = ThingDefOf.HemogenPack.ingestible?.outcomeDoers?.FirstOrDefault((IngestionOutcomeDoer x) => x is IngestionOutcomeDoer_OffsetHemogen) as IngestionOutcomeDoer_OffsetHemogen;
		...
		protected override Job TryGiveJob(Pawn pawn)
		{
			...
			if (gene_Hemogen.hemogenPacksAllowed)
			{
				...
				if (hemogenPack != null)
				{
					...
					job.count = Mathf.Min(hemogenPack.stackCount, Mathf.CeilToInt((gene_Hemogen.Max - gene_Hemogen.Value) / HemogenPackHemogenGain));
					return job;
				}
			}
			return null;
		...
		private Pawn GetPrisoner(Pawn pawn)
		{
			return (Pawn)GenClosest.ClosestThing_Global_Reachable(pawn.Position, pawn.Map, pawn.Map.mapPawns.PrisonersOfColonySpawned, PathEndMode.OnCell, TraverseParms.For(pawn), 9999f, delegate(Thing t)
			{
				...
				return pawn2 != null && CanFeedOnPrisoner(pawn, pawn2).Accepted;
			});
		}
	}
}
