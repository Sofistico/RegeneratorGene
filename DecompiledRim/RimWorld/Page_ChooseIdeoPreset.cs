...
namespace RimWorld
{
	...
	public class Page_ChooseIdeoPreset : Page
	{
		...
		private void DrawCategory(IdeoPresetCategoryDef category, ref float curY)
		{
			...
			if ((string)category.LabelCap != category.groupLabel)
			{
		...
		private void DrawStructureAndStyleSelection(Rect rect)
		{
			...
			void FillAllAvailableStyles(ref List<FloatMenuOption> opts, int forIndex)
			{
				...
				foreach (StyleCategoryDef s in DefDatabase<StyleCategoryDef>.AllDefs.Where((StyleCategoryDef x) => !selectedStyles.Contains(x)))
				{
		...
		private void DoPreset()
		{
			...
			else if (DefDatabase<MemeDef>.AllDefsListForReading.Where((MemeDef m) => m.category == MemeCategory.Structure && IdeoUtility.IsMemeAllowedFor(m, Find.Scenario.playerFaction.factionDef)).TryRandomElement(out result))
			{
				list.Add(result);
			}
		...
		private void ApplySelectedStylesToIdeo(Ideo ideo)
		{
			...
			for (int num = finalizedStyles.Count - 1; num >= 0; num--)
			{
				...
				{
					if (DefDatabase<StyleCategoryDef>.AllDefs.Where((StyleCategoryDef x) => !finalizedStyles.Contains(x)).TryRandomElement(out var result))
					{
