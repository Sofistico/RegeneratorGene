...
namespace RimWorld
{
	public class Faction : IExposable, ILoadReferenceable, ICommunicable
	{
		...
		public void ExposeData()
		{
			...
			Scribe_Values.Look(ref mechColor, "mechColor");
		...
		public bool TryAffectGoodwillWith(Faction other, int goodwillChange, bool canSendMessage = true, bool canSendHostilityLetter = true, HistoryEventDef reason = null, GlobalTargetInfo? lookTarget = null)
		{
			...
			if (canSendMessage && num != num4 && !sentLetter && !sentLetter2 && Current.ProgramState == ProgramState.Playing && (IsPlayer || other.IsPlayer))
			{
				...
				string text = ((reason == null) ? ((string)"MessageGoodwillChanged".Translate(faction.name, num.ToString("F0"), num4.ToString("F0"))) : ((string)"MessageGoodwillChangedWithReason".Translate(faction.name, num.ToString("F0"), num4.ToString("F0"), reason.LabelCap)));
		...
		public void Notify_MemberDied(Pawn member, DamageInfo? dinfo, bool wasWorldPawn, bool wasGuilty, Map map)
		{
			...
			if (!wasWorldPawn && !PawnGenerator.IsBeingGenerated(member) && Current.ProgramState == ProgramState.Playing && map != null && !member.IsSlaveOfColony && map.IsPlayerHome && !this.HostileTo(OfPlayer))
			{
		...
		private void CheckForPrisonersAssignedToBloodfeederInteractionMode(Map map, Pawn member)
		{
			...
			{
				if (item.IsPrisonerOfColony && item.guest.interactionMode != null && item.guest.interactionMode.hideIfNoBloodfeeders)
				{
			...
			{
				tmpPrisonersRequiringBloodfeeder.guest.interactionMode = PrisonerInteractionModeDefOf.NoInteraction;
			}
			tmpPrisonersRequiringBloodfeeders.Clear();
		}
		...
		{
			if (free && (member.HostFaction == OfPlayer || member.Faction == OfPlayer))
			{
				bool isHealthy;
				bool isInMentalState;
				int goodwillGainForPrisonerRelease = GetGoodwillGainForPrisonerRelease(member, out isHealthy, out isInMentalState);
				if (member.mindState.AvailableForGoodwillReward)
				{
					OfPlayer.TryAffectGoodwillWith(this, goodwillGainForPrisonerRelease, canSendMessage: true, !temporary, HistoryEventDefOf.MemberExitedMapHealthy);
				}
			}
