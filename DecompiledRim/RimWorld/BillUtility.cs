...
namespace RimWorld
{
	public static class BillUtility
	{
		...
		public static ThingOwner<Thing> GetBillGiverContainer(Bill bill)
		{
			...
			if ((building_MechGestator = bill.billStack.billGiver as Building_MechGestator) != null)
			{
				return (ThingOwner<Thing>)building_MechGestator.innerContainer;
			}
			return null;
		}
		...
		{
			recipeTooltipLayout.Reset(4f);
			tmpSpecialProducts.Clear();
			if (!recipe.skillRequirements.NullOrEmpty())
			{
				recipeTooltipLayout.Label("Requires".Translate() + ": " + recipe.skillRequirements.Select((SkillRequirement x) => x.Summary).ToCommaList(), draw);
			}
			if (!recipe.ingredients.NullOrEmpty())
			{
				...
				foreach (IngredientCount ingredient in recipe.ingredients)
				{
					...
					IEnumerable<ThingDef> enumerable = ingredient.filter.AllowedThingDefs.Where((ThingDef x) => Widgets.GetIconFor(x) != BaseContent.BadTex && (recipe.fixedIngredientFilter.AllowedDefCount == 0 || (recipe.fixedIngredientFilter.Allows(x) && !recipe.fixedIngredientFilter.IsAlwaysDisallowedDueToSpecialFilters(x))));
					GetDistinctProductsFromIngredients(recipe, enumerable, tmpSpecialProducts);
					IEnumerable<TextureAndColor> enumerable2 = enumerable.Select((ThingDef x) => new TextureAndColor(Widgets.GetIconFor(x), x.uiIconColor)).Distinct();
					if (!enumerable2.Any())
					{
						continue;
					}
					if (!flag)
					{
						...
						recipeTooltipLayout.Label("Ingredients".Translate() + ": ", draw);
						flag = true;
					}
					recipeTooltipLayout.Newline();
					DisplayIngredientIconRow(enumerable2, draw);
				}
			}
			if (!recipe.products.NullOrEmpty() || tmpSpecialProducts.Count > 0)
			{
				...
				recipeTooltipLayout.Label("Products".Translate() + ": ", draw);
				recipeTooltipLayout.Newline();
				if (!recipe.products.NullOrEmpty())
				{
					foreach (ThingDefCountClass product in recipe.products)
					{
						...
						recipeTooltipLayout.Label(product.count + "x ", draw);
						recipeTooltipLayout.Icon(Widgets.GetIconFor(product.thingDef), product.thingDef.uiIconColor, Text.LineHeightOf(GameFont.Small), draw);
				...
				if (tmpSpecialProducts.Count > 0)
				{
					DisplayIngredientIconRow(tmpSpecialProducts.Select((ThingDef x) => new TextureAndColor(Widgets.GetIconFor(x), x.uiIconColor)).Distinct(), draw);
				}
			}
			if (!recipeTooltipLayout.Empty)
			{
				recipeTooltipLayout.Expand(4f, 4f);
			}
		}
		...
		private static void DisplayIngredientIconRow(IEnumerable<TextureAndColor> icons, bool draw)
		{
