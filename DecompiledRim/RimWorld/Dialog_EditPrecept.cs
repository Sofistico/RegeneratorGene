...
namespace RimWorld
{
	public class Dialog_EditPrecept : Window
	{
		...
		private void UpdateWindowHeight()
		{
			...
			if ((precept_Relic = precept as Precept_Relic) != null && precept_Relic.ThingDef.MadeFromStuff)
			{
				windowHeight += 10f + EditFieldHeight;
			}
		...
		public override void DoWindowContents(Rect rect)
		{
			...
			if ((relic = precept as Precept_Relic) != null && relic.ThingDef.MadeFromStuff)
			{
				...
				Widgets.DefIcon(rect10, relic.stuff);
				TaggedString taggedString3 = "ChooseStuffForRelic".Translate() + "...";
				if (Widgets.ButtonText(new Rect(width: Text.CalcSize(taggedString3).x + 20f, x: rect10.xMax + 4f, y: num3, height: EditFieldHeight), taggedString3))
				{
			...
			if (Widgets.ButtonText(new Rect(rect.width / 2f - ButSize.x / 2f, rect.height - ButSize.y, ButSize.x, ButSize.y), "Randomize".Translate()))
			{
				...
				if ((precept_Ritual = precept as Precept_Ritual) != null)
				{
				...
				Precept_Building building2;
				if ((building2 = precept as Precept_Building) != null)
				{
					selectedStyle = StylesForBuilding(building2).RandomElement();
				}
				Precept_Relic precept_Relic;
				if ((precept_Relic = precept as Precept_Relic) != null)
				{
		...
		private void ApplyChanges()
		{
			...
			Precept_Building precept_Building;
			if ((precept_Building = precept as Precept_Building) != null && selectedStyle != null)
			{
		...
		{
			if (building.ThingDef?.canEditAnyStyle ?? false)
			{
