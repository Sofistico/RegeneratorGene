...
namespace RimWorld
{
	public abstract class Precept_Role : Precept
	{
			...
			if (!allApparelRequirementLabelsCached.TryGetValue(gender, out var value))
			{
				...
				if (apparelRequirements != null)
				{
					for (int i = 0; i < apparelRequirements.Count; i++)
					{
						...
						{
							value.Add(requirement.groupLabel);
						}
						else
						{
							value.Add(requirement.AllRequiredApparel(Gender.Male).First().LabelCap.Resolve());
						}
					}
				}
			...
			if (apparelRequirements != null)
			{
				...
				for (int j = 0; j < apparelRequirements.Count; j++)
				{
					...
					if (forPawn != null && !ApparelUtility.IsRequirementActive(requirement2, ApparelRequirementSource.Role, forPawn, out var disabledByLabel))
					{
						...
						string text = value[j];
						text = (value[j] = text + (" [" + "ApparelRequirementDisabledLabel".Translate() + ": " + disabledByLabel + "]"));
					}
				}
			}
			return value;
		...
		public string LabelForPawn(Pawn p)
		{
			if (def.leaderRole)
			{
				if (p.gender != Gender.Female || ideo.leaderTitleFemale.NullOrEmpty())
				{
					return ideo.leaderTitleMale;
				}
				return ideo.leaderTitleFemale;
			}
			return name;
		}
			...
			if (!desperate)
			{
				for (int num = apparelRequirementPool.Count - 1; num >= 0; num--)
				{
					foreach (Precept item in ideo.PreceptsListForReading)
					{
						...
						if ((precept_Apparel = item as Precept_Apparel) != null && !apparelRequirementPool[num].CanWearTogetherWith(precept_Apparel.apparelDef))
						{
		...
		public override List<PreceptApparelRequirement> GenerateNewApparelRequirements(FactionDef generatingFor = null)
		{
			...
			List<BodyPartGroupDef> bodypartsAlreadyCovered = new List<BodyPartGroupDef>();
				...
				if (apparelRequirements.Count < apparelRequirementCount)
				{
					...
					for (int i = 0; i < list.Count; i++)
					{
						if (list[i].CanAddRequirement(this, apparelRequirements, out var cannotAddReason, generatingFor) && !list[i].RequirementOverlapsOther(apparelRequirements, out cannotAddReason))
						{
							...
							bodypartsAlreadyCovered.AddRange(list[i].requirement.bodyPartGroupsMatchAny);
							if (apparelRequirements.Count >= apparelRequirementCount)
							{
								break;
							}
		...
		public override void Init(Ideo ideo, FactionDef generatingFor = null)
		{
			...
			{
				RegenerateName();
				if (!ideo.classic && !ideo.hiddenIdeoMode)
				{
					apparelRequirements = GenerateNewApparelRequirements(generatingFor);
				}
				allApparelRequirementLabelsCached.Clear();
		...
		public override string GetTip()
		{
			if (tipCached.NullOrEmpty())
			{
				...
				if (!def.grantedAbilities.NullOrEmpty())
				{
					...
					{
						CompProperties_AbilityStartRitual compProperties_AbilityStartRitual = def.grantedAbilities[i].comps.FirstOrDefault((AbilityCompProperties comp) => comp is CompProperties_AbilityStartRitual) as CompProperties_AbilityStartRitual;
						if (compProperties_AbilityStartRitual != null)
						{
							foreach (Precept item in ideo.PreceptsListForReading)
							{
								Precept_Ritual precept_Ritual;
								if ((precept_Ritual = item as Precept_Ritual) != null && precept_Ritual.def == compProperties_AbilityStartRitual.ritualDef)
								{
				...
				foreach (Precept item2 in ideo.PreceptsListForReading)
				{
					...
					if ((precept_Ritual2 = item2 as Precept_Ritual) != null && item2.def.listedForRoles && precept_Ritual2?.behavior?.def.stages != null && precept_Ritual2.behavior.def.roles != null && !list2.Contains(precept_Ritual2.LabelCap) && precept_Ritual2.behavior.def.roles.Any((RitualRole r) => r.precept == def))
					{
		...
		protected override void PostDrawBox(Rect rect, out bool anyTooltipActive)
		{
			...
			if (!apparelRequirements.NullOrEmpty())
			{
				...
				num += (list.Count - 1) * 2;
				Rect rect2 = new Rect(rect.xMax - (float)num, rect.yMin, num, 24f);
				_ = rect2.xMin;
				GenUI.DrawElementStack(rect2, 24f, list, delegate(Rect r, ThingDef apparel)
				{
					if (Mouse.IsOver(r))
					{
		...
		public override List<Thought_Situational> SituationThoughtsToAdd(Pawn pawn, List<Thought_Situational> activeThoughts)
		{
			...
			if (!def.leaderRole && def.createsRoleEmptyThought && !ChosenPawns().Any() && !activeThoughts.Any((Thought_Situational t) => (thought_IdeoRoleEmpty2 = t as Thought_IdeoRoleEmpty) != null && thought_IdeoRoleEmpty2.Role == this))
			{
			...
			if (IsAssigned(pawn) && apparelRequirements != null && apparelRequirements.Count > 0 && !pawn.IsQuestLodger() && !activeThoughts.Any((Thought_Situational t) => (thought_IdeoRoleApparelRequirementNotMet2 = t as Thought_IdeoRoleApparelRequirementNotMet) != null && thought_IdeoRoleApparelRequirementNotMet2.Role == this))
			{
