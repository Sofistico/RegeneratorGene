		...
		public static void DrawDebug()
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				if (currentViewRect.Contains(list[i].Position))
				{
					...
					{
						CellRenderer.RenderCell(mat: SolidColorMaterials.SimpleSolidColorMaterial((!building_Door.FreePassage) ? new Color(1f, 0f, 0f, 0.5f) : new Color(0f, 1f, 0f, 0.5f)), c: building_Door.Position);
					}
				}
			}
		}
