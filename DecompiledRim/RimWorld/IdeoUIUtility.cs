...
namespace RimWorld
{
	...
	public static class IdeoUIUtility
	{
		...
		public static void DoStyles(ref float curY, ref float curX, float width, Ideo ideo, IdeoEditMode editMode, int styleIconSize = 28)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				if (i < tmpStyleCategories.Count)
				{
					...
					foreach (StyleCategoryDef s2 in DefDatabase<StyleCategoryDef>.AllDefs.Where((StyleCategoryDef x) => !ideo.thingStyleCategories.Any((ThingStyleCategoryWithPriority y) => y.category == x)))
					{
				...
				else
				{
					...
					foreach (StyleCategoryDef s in DefDatabase<StyleCategoryDef>.AllDefs.Where((StyleCategoryDef x) => !ideo.thingStyleCategories.Any((ThingStyleCategoryWithPriority y) => y.category == x)))
					{
		...
		{
			string text = "\n\n" + ((ideo != null && !ideo.hiddenIdeoMode) ? "StyleCategoryDescStyleDominance".Translate(ideo.Named("IDEO")) : "StyleCategoryDescStyleDominanceAbstract".Translate()).Resolve();
