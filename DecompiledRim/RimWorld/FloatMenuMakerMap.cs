	...
	public static class FloatMenuMakerMap
	{
		...
		private static void AddHumanlikeOrders(Vector3 clickPos, Pawn pawn, List<FloatMenuOption> opts)
		{
			...
			foreach (Thing thing8 in c.GetThingList(pawn.Map))
			{
				...
				if ((!t2.def.IsDrug || !ModsConfig.IdeologyActive || new HistoryEvent(HistoryEventDefOf.IngestedDrug, pawn.Named(HistoryEventArgsNames.Doer)).Notify_PawnAboutToDo(out var opt, text)) && (!t2.def.IsNonMedicalDrug || !ModsConfig.IdeologyActive || new HistoryEvent(HistoryEventDefOf.IngestedRecreationalDrug, pawn.Named(HistoryEventArgsNames.Doer)).Notify_PawnAboutToDo(out opt, text)) && (!t2.def.IsDrug || !ModsConfig.IdeologyActive || t2.def.ingestible.drugCategory != DrugCategory.Hard || new HistoryEvent(HistoryEventDefOf.IngestedHardDrug, pawn.Named(HistoryEventArgsNames.Doer)).Notify_PawnAboutToDo(out opt, text)))
				{
			...
			if (pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation))
			{
				...
				foreach (LocalTargetInfo item15 in GenUI.TargetsAt(clickPos, TargetingParameters.ForRescue(pawn), thingsOnly: true))
				{
					...
					if (!victim3.IsPrisonerOfColony && !victim3.IsSlaveOfColony && (!victim3.InMentalState || victim3.health.hediffSet.HasHediff(HediffDefOf.Scaria)) && !victim3.IsColonyMech && (victim3.Faction == Faction.OfPlayer || victim3.Faction == null || !victim3.Faction.HostileTo(Faction.OfPlayer)))
					{
						...
						{
							if (ChildcareUtility.CanSuckle(victim3, out var _))
							{
			...
			if (ModsConfig.BiotechActive && pawn.IsBloodfeeder() && pawn.genes?.GetFirstGeneOfType<Gene_Hemogen>() != null)
			{
				...
				{
					Pawn targPawn4 = (Pawn)item23.Thing;
					if (!pawn.CanReach(targPawn4, PathEndMode.ClosestTouch, Danger.Deadly))
					{
						opts.Add(new FloatMenuOption("CannotBloodfeedOn".Translate(targPawn4.Named("PAWN")) + ": " + "NoPath".Translate().CapitalizeFirst(), null));
						continue;
					}
					AcceptanceReport acceptanceReport3 = JobGiver_GetHemogen.CanFeedOnPrisoner(pawn, targPawn4);
					if (acceptanceReport3.Accepted)
					{
						opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("BloodfeedOn".Translate(targPawn4.Named("PAWN")), delegate
						{
							Job job18 = JobMaker.MakeJob(JobDefOf.PrisonerBloodfeed, targPawn4);
							pawn.jobs.TryTakeOrderedJob(job18, JobTag.Misc);
						}), pawn, targPawn4));
					}
					else if (!acceptanceReport3.Reason.NullOrEmpty())
					{
						opts.Add(new FloatMenuOption("CannotBloodfeedOn".Translate(targPawn4.Named("PAWN")) + ": " + acceptanceReport3.Reason.CapitalizeFirst(), null));
					}
				}
			}
			if (ModsConfig.BiotechActive && pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation))
			{
				...
				{
					Pawn targPawn3 = (Pawn)item24.Thing;
					if (targPawn3.InBed())
					{
						continue;
					}
					...
					{
						opts.Add(new FloatMenuOption("CannotCarry".Translate(targPawn3) + ": " + "NoPath".Translate().CapitalizeFirst(), null));
						continue;
					}
					Thing bestBedOrCasket = GenClosest.ClosestThingReachable(targPawn3.PositionHeld, pawn.Map, ThingRequest.ForDef(ThingDefOf.DeathrestCasket), PathEndMode.ClosestTouch, TraverseParms.For(pawn), 9999f, (Thing casket) => casket.Faction == Faction.OfPlayer && RestUtility.IsValidBedFor(casket, targPawn3, pawn, checkSocialProperness: true, allowMedBedEvenIfSetToNoCare: false, ignoreOtherReservations: false, targPawn3.GuestStatus));
					if (bestBedOrCasket == null)
					{
						bestBedOrCasket = RestUtility.FindBedFor(targPawn3, pawn, checkSocialProperness: false);
					}
					if (bestBedOrCasket != null)
					{
						...
						{
							Job job17 = JobMaker.MakeJob(JobDefOf.DeliverToBed, targPawn3, bestBedOrCasket);
							job17.count = 1;
							pawn.jobs.TryTakeOrderedJob(job17, JobTag.Misc);
						}, MenuOptionPriority.RescueOrCapture, null, targPawn3), pawn, targPawn3));
					}
					else
					{
						opts.Add(new FloatMenuOption("CannotCarry".Translate(targPawn3) + ": " + "NoCasketOrBed".Translate(), null));
					}
				}
			}
			if (ModsConfig.BiotechActive && pawn.genes != null)
			{
				...
				{
					Pawn targPawn2 = (Pawn)item25.Thing;
					if (!pawn.CanReserveAndReach(targPawn2, PathEndMode.OnCell, Danger.Deadly, 1, -1, null, ignoreOtherReservations: true))
					{
						continue;
					}
					FloatMenuOption item5 = (pawn.IsQuestLodger() ? new FloatMenuOption("CannotAbsorbXenogerm".Translate(targPawn2.Named("PAWN")) + ": " + "TemporaryFactionMember".Translate(pawn.Named("PAWN")), null) : ((targPawn2.genes.Xenotype == pawn.genes.Xenotype) ? new FloatMenuOption("CannotAbsorbXenogerm".Translate(targPawn2.Named("PAWN")) + ": " + "SameXenotype".Translate(pawn.Named("PAWN")), null) : ((!targPawn2.health.hediffSet.HasHediff(HediffDefOf.XenogermLossShock)) ? FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("AbsorbXenogerm".Translate(targPawn2.Named("PAWN")), delegate
					{
						if (targPawn2.IsPrisonerOfColony && !targPawn2.Downed)
						{
							Messages.Message("MessageTargetMustBeDownedToForceReimplant".Translate(targPawn2.Named("PAWN")), targPawn2, MessageTypeDefOf.RejectInput, historical: false);
						}
						else if (GeneUtility.PawnWouldDieFromReimplanting(targPawn2))
						{
							Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation("WarningPawnWillDieFromReimplanting".Translate(targPawn2.Named("PAWN")), delegate
							{
								GiveReimplantJob(targPawn2);
							}, destructive: true));
						}
						else
						{
					...
					}), pawn, targPawn2) : new FloatMenuOption("CannotAbsorbXenogerm".Translate(targPawn2.Named("PAWN")) + ": " + "XenogermLossShockPresent".Translate(targPawn2.Named("PAWN")), null))));
					opts.Add(item5);
				}
			}
			if (ModsConfig.BiotechActive && ChildcareUtility.CanBreastfeed(pawn, out var reason) && !pawn.Downed && !pawn.Drafted)
			{
			...
			foreach (LocalTargetInfo item28 in GenUI.TargetsAt(clickPos, TargetingParameters.ForStrip(pawn), thingsOnly: true))
			{
				...
				FloatMenuOption item7 = (pawn.CanReach(stripTarg, PathEndMode.ClosestTouch, Danger.Deadly) ? ((stripTarg.Pawn == null || !stripTarg.Pawn.HasExtraHomeFaction()) ? FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("Strip".Translate(stripTarg.Thing.LabelCap, stripTarg.Thing), delegate
				{
			...
			void GiveReimplantJob(Pawn targPawn)
			{
