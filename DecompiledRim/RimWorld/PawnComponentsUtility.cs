...
namespace RimWorld
{
	public class PawnComponentsUtility
	{
		public static void CreateInitialComponents(Pawn pawn)
		{
			...
			if ((pawn.RaceProps.Humanlike || pawn.RaceProps.IsMechanoid) && pawn.abilities == null)
			{
		...
		public static void AddComponentsForSpawn(Pawn pawn)
		{
			...
			if (((int)pawn.RaceProps.intelligence <= 1 || (pawn.genes != null && pawn.genes.ShouldHaveCallTracker)) && pawn.caller == null)
			{
		...
		public static void AddAndRemoveDynamicComponents(Pawn pawn, bool actAsIfSpawned = false)
		{
			...
			if (ModsConfig.BiotechActive && MechanitorUtility.IsOverseerSubject(pawn))
			{
				if (pawn.relations == null)
				{
					pawn.relations = new Pawn_RelationsTracker(pawn);
