...
namespace RimWorld
{
	...
	public static class SkillUI
	{
		...
		public static void DrawSkill(SkillRecord skill, Rect holdingRect, SkillDrawMode mode, string tooltipPrefix = "")
		{
			...
			if (!skill.TotallyDisabled)
			{
				...
				if (ModsConfig.BiotechActive)
				{
					if (skill.Aptitude > 0)
					{
						fillTex = SkillBarAptitudePositiveTex;
					}
					else if (skill.Aptitude < 0)
					{
						fillTex = SkillBarAptitudeNegativeTex;
					}
				}
		...
		private static string GetSkillDescription(SkillRecord sk)
		{
			...
			stringBuilder.AppendLineTagged(("Level".Translate().CapitalizeFirst() + ": ").AsTipTitle() + sk.GetLevelForUI() + " - " + sk.LevelDescriptor);
			...
			if (!sk.TotallyDisabled)
			{
				...
				float num2 = 1f;
				float num3 = statValue * sk.passion.GetLearningFactor();
				if (sk.def == SkillDefOf.Animals)
				{
					num2 = sk.Pawn.GetStatValue(StatDefOf.AnimalsLearningFactor);
					num3 *= num2;
				}
				stringBuilder.AppendLine();
				stringBuilder.AppendLineTagged(("LearningSpeed".Translate() + ": ").AsTipTitle() + num3.ToStringPercent());
				...
				{
					stringBuilder.AppendLine("  - " + StatDefOf.AnimalsLearningFactor.LabelCap + ": x" + num2.ToStringPercent());
				}
