...
namespace RimWorld
{
	public class Dialog_StylingStation : Window
	{
		...
		private List<Color> AllHairColors
		{
			get
			{
				if (allHairColors == null)
				{
					...
					DefDatabase<ColorDef>.AllDefsListForReading.Select((ColorDef ic) => ic.color).ToList();
		...
		private void DrawHairColors(Rect rect)
		{
			...
			y += colorsHeight;
			if (desiredHairColor != pawn.story.HairColor)
			{
		...
		private void DrawBottomButtons(Rect inRect)
		{
			...
			if (pawn.story.hairDef != initialHairDef || pawn.style.beardDef != initialBeardDef || pawn.style.FaceTattoo != initialFaceTattoo || pawn.style.BodyTattoo != initialBodyTattoo || desiredHairColor != pawn.story.HairColor)
			{
