...
namespace RimWorld
{
	...
	public class Building_GrowthVat : Building_Enterable, IStoreSettingsParent, IThingHolderWithDrawnPawn, IThingHolder
	{
		...
		private const int EmbryoGestationTicks = 540000;

		private const int EmbryoLateStageGraphicTicksRemaining = 540000;

		private const float WorkingPowerUsage = 200f;
		...
		public override void Tick()
		{
			...
			{
				PowerTraderComp.PowerOutput = 0f - (base.Working ? 200f : base.PowerComp.Props.PowerConsumption);
			}
			if ((selectedPawn?.Destroyed ?? false) || (selectedEmbryo?.Destroyed ?? false))
			{
				OnStop();
			}
			foreach (Thing item in (IEnumerable<Thing>)innerContainer)
			{
				...
				if ((humanEmbryo = item as HumanEmbryo) != null && humanEmbryo != selectedEmbryo)
				{
			...
			if (base.Working)
			{
				if (selectedPawn != null)
				{
					...
					{
						selectedPawn.ageTracker.Notify_TickedInGrowthVat(20);
						VatLearning?.Tick();
		...
		private void EmbryoBirth()
		{
			if (selectedEmbryo != null && innerContainer.Contains(selectedEmbryo) && startTick <= Find.TickManager.TicksGame)
			{
				...
				{
					Corpse corpse;
					Pawn pawn = (((corpse = thing as Corpse) != null) ? corpse.InnerPawn : ((Pawn)thing));
		...
		public override IEnumerable<Gizmo> GetGizmos()
		{
			...
			else
			{
				...
				if (selectedEmbryo == null && Find.Storyteller.difficulty.ChildrenAllowed)
				{
					...
					command_Action4.defaultDesc = "InsertEmbryoGrowthVatDesc".Translate();
		...
		public override void Draw()
		{
			...
			if (base.Working)
			{
				...
				else if (selectedEmbryo != null && innerContainer.Contains(selectedEmbryo))
				{
