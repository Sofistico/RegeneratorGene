...
namespace RimWorld
{
	...
	public static class PortraitsCache
	{
		...
		private readonly struct PortraitParams
		{
			...
			{
				object obj2;
				if ((obj2 = obj) is PortraitParams)
				{
					PortraitParams other = (PortraitParams)obj2;
					return Equals(other);
				}
				return false;
			}
			...
			{
				if (other.size == size && other.cameraOffset == cameraOffset && other.cameraZoom == cameraZoom && other.rotation == rotation && other.renderHeadgear == renderHeadgear && other.renderClothes == renderClothes && other.stylingStation == stylingStation && other.overrideHairColor == overrideHairColor && other.overrideHealthState == overrideHealthState)
				{
			...
			public void RenderPortrait(Pawn pawn, RenderTexture renderTexture)
			{
				...
				PawnHealthState pawnHealthState = overrideHealthState ?? pawn.health.State;
				if (pawnHealthState == PawnHealthState.Down && !HealthAIUtility.WantsToBeRescuedIfDowned(pawn))
				{
					pawnHealthState = PawnHealthState.Mobile;
				}
				if (pawnHealthState != PawnHealthState.Mobile)
				{
					angle = 85f;
					positionOffset.x -= 0.18f;
					positionOffset.z -= 0.18f;
				}
				Find.PawnCacheRenderer.RenderPawn(pawn, renderTexture, cameraOffset, cameraZoom, angle, rotation, pawn.health.hediffSet.HasHead, renderBody: true, renderHeadgear, renderClothes, portrait: true, positionOffset, overrideApparelColors, overrideHairColor, stylingStation);
			}
		}
