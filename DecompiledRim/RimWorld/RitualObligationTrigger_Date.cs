	...
	public class RitualObligationTrigger_Date : RitualObligationTrigger
	{
		...
		public string DateString
		{
			get
			{
				...
				return GenDate.QuadrumDateStringAt(triggerDaysSinceStartOfYear * 60000, (map != null) ? Find.WorldGrid.LongLatOf(map.Tile).x : 0f);
			}
		}
		...
		public int RandomDate()
		{
			...
			foreach (Precept item in ritual.ideo.PreceptsListForReading)
			{
				...
				if (item == ritual || (precept_Ritual = item as Precept_Ritual) == null)
				{
					continue;
				}
				foreach (RitualObligationTrigger obligationTrigger in precept_Ritual.obligationTriggers)
				{
					...
					if ((ritualObligationTrigger_Date = obligationTrigger as RitualObligationTrigger_Date) != null)
					{
