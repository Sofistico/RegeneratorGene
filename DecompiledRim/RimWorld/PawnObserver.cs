	...
	public class PawnObserver
	{
		...
		private void ObserveSurroundingThings()
		{
			...
			RegionTraverser.BreadthFirstTraverse(pawn.Position, pawn.Map, (Region from, Region to) => pawn.Position.InHorDistOf(to.extentsClose.ClosestCellTo(pawn.Position), 5f), delegate(Region reg)
			{
				...
				foreach (Thing item2 in reg.ListerThings.ThingsInGroup(ThingRequestGroup.BuildingArtificial))
				{
			...
			foreach (Thought_MemoryObservationTerror item3 in TerrorUtility.TakeTopTerrorThoughts(terrorThoughts))
			{
				pawn.needs.mood.thoughts.memories.TryGainMemory(item3);
			}
			terrorThoughts.Clear();
			...
			void TryCreateObservedThought(Thing thing)
			{
				...
				{
					Thought_Memory thought_Memory = observedThoughtGiver2.GiveObservedThought(pawn);
