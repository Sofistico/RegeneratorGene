...
namespace RimWorld
{
	public class WorkGiver_Replant : WorkGiver_ConstructDeliverResources
	{
		...
		public override Job JobOnThing(Pawn pawn, Thing t, bool forced = false)
		{
			...
			Blueprint_Install blueprint_Install = t as Blueprint_Install;
			ThingDef thingDef;
			if (blueprint_Install == null || (thingDef = blueprint_Install.def.entityDefToBuild as ThingDef) == null || thingDef.plant == null)
			{
				return null;
			}
			...
			Thing blockingThing;
			AcceptanceReport acceptanceReport = ((ThingDef)t.def.entityDefToBuild).CanEverPlantAt(t.Position, t.Map, out blockingThing, canWipePlantsExceptTree: true);
			if (!acceptanceReport)
			{
				JobFailReason.Is(acceptanceReport.Reason);
				return null;
			}
			if (!((ThingDef)t.def.entityDefToBuild).CanNowPlantAt(t.Position, t.Map, canWipePlantsExceptTree: true))
			{
				return null;
			}
			if (((ThingDef)t.def.entityDefToBuild).plant.interferesWithRoof && t.Position.Roofed(t.Map))
			{
			...
			job.plantDefToSow = (ThingDef)blueprint_Install.def.entityDefToBuild;
			job.count = 1;
