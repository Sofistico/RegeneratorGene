...
namespace RimWorld
{
	public abstract class IdeoFoundation : IExposable
	{
		...
		public virtual void RandomizePrecepts(bool init, IdeoGenerationParms parms)
		{
			...
			{
				PreceptDef result2;
				if (allDefsListForReading.Where((PreceptDef x) => x.issue == allIssueDefs[i] && x.associatedMemes.Any((MemeDef y) => ideo.memes.Contains(y)) && CanAddForFaction(x, parms.forFaction, parms.disallowedPrecepts, checkDuplicates: true, ignoreMemeRequirements: false, ignoreConflictingMemes: false, parms.classic)).TryRandomElementByWeight((PreceptDef x) => x.defaultSelectionWeight, out var result))
				{
					ideo.AddPrecept(PreceptMaker.MakePrecept(result));
				}
				else if (allDefsListForReading.Where((PreceptDef x) => x.issue == allIssueDefs[i] && CanAddForFaction(x, parms.forFaction, parms.disallowedPrecepts, checkDuplicates: true, ignoreMemeRequirements: false, ignoreConflictingMemes: false, parms.classic)).TryRandomElementByWeight((PreceptDef x) => x.defaultSelectionWeight, out result2))
				{
			...
			{
				PreceptDef preceptDef = allDefsListForReading[k];
				if (preceptDef.countsTowardsPreceptLimit || !preceptDef.canGenerateAsSpecialPrecept || !CanAddForFaction(preceptDef, parms.forFaction, parms.disallowedPrecepts, checkDuplicates: true, ignoreMemeRequirements: false, ignoreConflictingMemes: false, parms.classic))
				{
					continue;
				}
			...
			IEnumerable<PreceptDef> source = DefDatabase<PreceptDef>.AllDefs.Where((PreceptDef x) => x.preceptClass == typeof(Precept_RoleMulti) && CanAddForFaction(x, parms.forFaction, parms.disallowedPrecepts, checkDuplicates: true, ignoreMemeRequirements: false, ignoreConflictingMemes: false, parms.classic));
			if (source.Any())
			{
				for (int m = 0; m < 2; m++)
				{
			...
			if (!parms.classic)
			{
				int num2 = InitialVeneratedAnimalsCountRange.RandomInRange;
				for (int n = 0; n < ideo.memes.Count; n++)
				{
					if (ideo.memes[n].veneratedAnimalsCountOverride >= 0)
					{
						num2 = ideo.memes[n].veneratedAnimalsCountOverride;
						break;
					}
					num2 += ideo.memes[n].veneratedAnimalsCountOffset;
				}
				for (int num3 = 0; num3 < Mathf.Min(num2, PreceptDefOf.AnimalVenerated.maxCount); num3++)
				{
			...
			void AddPreceptsOfImpact(PreceptImpact impact, int countToAdd)
			{
				...
				for (int num4 = 0; num4 < tmpMemes.Count; num4++)
				{
					if (tmpMemes[num4].ritualsToMake != IntRange.zero)
					{
						tmpMemesNumRitualsToMake.Add(tmpMemes[num4], tmpMemes[num4].ritualsToMake.RandomInRange);
					}
				}
				int num5 = 0;
				for (int num6 = 0; num6 < countToAdd; num6++)
				{
					bool flag2 = false;
					int num7 = num5;
					for (int num8 = num5 + tmpMemes.Count; num7 < num8; num7++)
					{
						MemeDef memeToTry = tmpMemes[num7 % tmpMemes.Count];
						if (DefDatabase<PreceptDef>.AllDefsListForReading.Where((PreceptDef x) => x.countsTowardsPreceptLimit && x.impact == impact && x.associatedMemes.Contains(memeToTry) && CanAddForFaction(x, parms.forFaction, parms.disallowedPrecepts, checkDuplicates: true, ignoreMemeRequirements: false, ignoreConflictingMemes: false, parms.classic)).TryRandomElementByWeight((PreceptDef x) => x.selectionWeight, out var result4))
						{
		...
		private void AddRequiredPreceptsForMemes(List<MemeDef> memes, IdeoGenerationParms parms, bool generateWeaponPrecept, List<Precept> initializedPreceptsOut, List<IdeoWeaponClassPair> weaponClassPairsOut = null, Action<PreceptDef> preceptGenerated = null)
		{
			for (int i = 0; i < memes.Count; i++)
			{
				if (memes[i].selectOneOrNone != null && Rand.Value >= memes[i].selectOneOrNone.noneChance && memes[i].selectOneOrNone.preceptThingPairs.Where((PreceptThingPair x) => CanAddForFaction(x.precept, parms.forFaction, parms.disallowedPrecepts, checkDuplicates: true, ignoreMemeRequirements: false, ignoreConflictingMemes: false, parms.classic)).TryRandomElementByWeight((PreceptThingPair x) => x.precept.selectionWeight, out var result))
				{
				...
				if (memes[i].requiredRituals != null && !parms.classic)
				{
				...
				if (memes[i].requireOne != null)
				{
					...
					{
						if (memes[i].requireOne[k].Where((PreceptDef x) => CanAddForFaction(x, parms.forFaction, parms.disallowedPrecepts, checkDuplicates: true, ignoreMemeRequirements: false, ignoreConflictingMemes: false, parms.classic)).TryRandomElementByWeight((PreceptDef x) => x.selectionWeight, out var result2))
						{
		...
		private bool ConflictsWithNewMemes(Precept precept, List<MemeDef> newMemes)
		{
			for (int i = 0; i < newMemes.Count; i++)
			{
				...
				for (int j = 0; j < newMemes[i].requireOne.Count; j++)
				{
					...
					{
						if (list[k].issue == precept.def.issue)
						{
							num++;
						}
					}
			...
			void AddAndInitPrecepts()
			{
				...
				foreach (Precept item6 in ideo.PreceptsListForReading)
				{
					if (!initedPrecepts.Contains(item6) && (initializedPrecepts == null || !initializedPrecepts.Contains(item6)))
					{
						...
						if ((ritual = item6 as Precept_Ritual) != null && !filledPrecepts.Contains(item6))
						{
