	...
	public class Selector
	{
			...
			if (Event.current.type == EventType.MouseDown)
			{
				...
				if (Event.current.button == 1 && selected.Count > 0)
				{
					if (selected.Count == 1)
					{
						...
						else if ((selectedThing = selected[0] as Thing) != null)
						{
		...
		private void SelectInternal(object obj, bool playSound = true, bool forceDesignatorDeselect = true)
		{
			...
			for (int num = selected.Count - 1; num >= 0; num--)
			{
				...
				if (((thing2 != null) ? thing2.MapHeld : ((Zone)selected[num]).Map) != map)
				{
			...
			else
			{
				...
				if (obj3 != null)
				{
					if (!ShiftIsHeld)
					{
						...
						Thing thing;
						if ((thing = obj3 as Thing) != null && thing.def.category == ThingCategory.Item && thing.Spawned && thing.Position.GetItemCount(thing.Map) >= 2 && list.All(delegate(object x)
						{
							Thing thing3;
							if ((thing3 = x as Thing) != null && thing3.def.category != ThingCategory.Item)
							{
								Building obj5 = thing3 as Building;
								if (obj5 == null)
								{
									return false;
								}
								return obj5.MaxItemsInCell > 1;
							}
							return true;
						}))
						{
							...
							else if ((building = list[list.Count - 1] as Building) != null && building.MaxItemsInCell > 1)
							{
								ClearSelection();
				...
				Thing thing2;
				if (list.Count >= 2 && (thing2 = list[0] as Thing) != null && thing2.def.category == ThingCategory.Item && thing2.Spawned)
				{
					Building building2;
					object obj4 = list.FirstOrDefault((object x) => (building2 = x as Building) != null && building2.MaxItemsInCell > 1);
					if (obj4 != null)
					{
						Select(obj4);
					}
		...
		private void SelectAllMatchingObjectUnderMouseOnScreen()
		{
			...
			if (clickedThing == null)
			{
				...
				{
					Thing thing2;
					if ((thing2 = o as Thing) == null)
					{
						return false;
					}
					return (!thing2.def.neverMultiSelect) ? true : false;
				}) as Thing;
			}
			...
			if (edifice != null && (building_Storage = edifice as Building_Storage) != null && building_Storage.GetSlotGroup().HeldThings.Contains(clickedThing))
			{
				Deselect(edifice);
			}
			...
			Predicate<Thing> predicate = delegate(Thing t)
			{
				...
				if (t.def != innerIfMinified.def || t.Faction != innerIfMinified.Faction || IsSelected(t))
				{
					return false;
				}
		...
		private void MassTakeFirstAutoTakeableOptionOrGoto()
		{
			...
			{
				if (!FloatMenuMakerMap.InvalidPawnForMultiSelectOption(item) && !TakeFirstNonGotoAutoTakeableOption(item, intVec) && mouseCell.IsValid && item.Drafted)
				{
					gotoController.AddPawn(item);
				}
			}
		}
		...
		private static bool TakeFirstNonGotoAutoTakeableOption(Pawn pawn, IntVec3 dest)
		{
			...
			foreach (FloatMenuOption item in FloatMenuMakerMap.ChoicesAtFor(dest.ToVector3Shifted(), pawn, suppressAutoTakeableGoto: true))
			{
