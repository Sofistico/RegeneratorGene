...
namespace RimWorld
{
	public static class RCellFinder
	{
		...
		{
			if (mode == TraverseMode.PassAllDestroyableThings && !pawn.Map.reachability.CanReachMapEdge(pawn.Position, TraverseParms.For(pawn, Danger.Deadly, TraverseMode.ByPawn, canBash)))
			{
		...
		public static bool TryFindSkygazeCell(IntVec3 root, Pawn searcher, out IntVec3 result)
		{
			...
			if (!CellFinder.TryFindClosestRegionWith(root.GetRegion(searcher.Map), traverseParms, validator, 300, out var result2))
			{
				result = root;
				return false;
			}
		...
		public static IntVec3 SpotToStandDuringJob(Pawn pawn, Predicate<IntVec3> extraValidator = null, Region targetRegion = null)
		{
			...
			for (int i = 0; i < 30; i++)
			{
				...
				IntVec3 intVec = SpotToStandDuringJobInRegion(targetRegion ?? CellFinder.RandomRegionNear(region, maxRegions, TraverseParms.For(pawn)), pawn, maxDistance, desperate, ignoreDanger, extraValidator);
				if (intVec.IsValid)
				{
		...
		public static IntVec3 FindSiegePositionFrom(IntVec3 entrySpot, Map map, bool allowRoofed = false, bool errorOnFail = true)
		{
		...
		public static bool TryFindSkydreamingSpotOutsideColony(IntVec3 root, Pawn searcher, out IntVec3 result, int minDistanceFromColonyThing = 35, int maxDistanceFromColonyThing = int.MaxValue, int maxRegions = 300, Predicate<Region> extraRegionValidator = null)
		{
