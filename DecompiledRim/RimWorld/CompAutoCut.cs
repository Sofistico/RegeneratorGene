...
namespace RimWorld
{
	public abstract class CompAutoCut : ThingComp
	{
		...
		{
			base.CompTickLong();
			if (autoCut && CheckIfMidnight(parent, lastCheckedAutoCutTick))
			{
				lastCheckedAutoCutTick = Find.TickManager.TicksGame;
		...
		private static bool CheckIfMidnight(Thing thing, int? lastCheckTicks)
		{
			...
			if (lastCheckTicks.HasValue)
			{
				return lastCheckTicks + 30000 < Find.TickManager.TicksGame;
			}
			return true;
		}
		...
		public void DesignatePlantsToCut()
		{
			...
			foreach (IntVec3 autoCutCell in GetAutoCutCells())
			{
				...
				{
					Plant plant;
					if ((plant = thingList[i] as Plant) != null && CanDesignatePlant(plant))
					{
