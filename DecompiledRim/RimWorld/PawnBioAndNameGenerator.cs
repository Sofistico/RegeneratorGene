...
namespace RimWorld
{
	public static class PawnBioAndNameGenerator
	{
		...
		public static void GiveAppropriateBioAndNameTo(Pawn pawn, string requiredLastName, FactionDef factionType, bool forceNoBackstory = false, bool newborn = false, XenotypeDef xenotype = null)
		{
			List<BackstoryCategoryFilter> backstoryCategoryFiltersFor = GetBackstoryCategoryFiltersFor(pawn, factionType);
			if (!forceNoBackstory && !pawn.DevelopmentalStage.Baby() && (Rand.Value < 0.25f || pawn.kindDef.factionLeader) && TryGiveSolidBioTo(pawn, requiredLastName, backstoryCategoryFiltersFor))
			{
				return;
			}
			GiveShuffledBioTo(pawn, factionType, requiredLastName, backstoryCategoryFiltersFor, forceNoBackstory, forceNoNick: false, xenotype);
			NameTriple nameTriple;
			if (newborn && (nameTriple = pawn.Name as NameTriple) != null)
			{
		...
		private static void GiveShuffledBioTo(Pawn pawn, FactionDef factionType, string requiredLastName, List<BackstoryCategoryFilter> backstoryCategories, bool forceNoBackstory = false, bool forceNoNick = false, XenotypeDef xenotype = null)
		{
			if (!forceNoBackstory)
			{
				bool flag = pawn.ageTracker.AgeBiologicalYearsFloat >= 20f;
				FillBackstorySlotShuffled(pawn, BackstorySlot.Childhood, backstoryCategories, factionType, flag ? new BackstorySlot?(BackstorySlot.Adulthood) : null);
				if (flag)
				{
		...
		public static Name GeneratePawnName(Pawn pawn, NameStyle style = NameStyle.Full, string forcedLastName = null, bool forceNoNick = false, XenotypeDef xenotype = null)
		{
			...
			{
			case NameStyle.Full:
				return GenerateFullPawnName(pawn.def, pawn.kindDef.GetNameMaker(pawn.gender), pawn.story, xenotype, pawn.RaceProps.GetNameGenerator(pawn.gender), pawn.Faction?.ideos?.PrimaryCulture, pawn.gender, pawn.RaceProps.nameCategory, forcedLastName, forceNoNick);
		...
		public static Name GenerateFullPawnName(ThingDef genFor, RulePackDef pawnKindNameMaker = null, Pawn_StoryTracker story = null, XenotypeDef xenotype = null, RulePackDef nameGenner = null, CultureDef primaryCulture = null, Gender gender = Gender.None, PawnNameCategory nameCategory = PawnNameCategory.HumanStandard, string forcedLastName = null, bool forceNoNick = false)
		{
			if (ModsConfig.BiotechActive && xenotype != null)
			{
				...
				{
					return NameResolvedFrom(nameMaker, forcedLastName, forceNoNick);
				}
			}
			if (pawnKindNameMaker != null)
			{
				return NameResolvedFrom(pawnKindNameMaker, forcedLastName, forceNoNick);
			}
			if (story != null)
			{
				if (story.Childhood?.nameMaker != null)
				{
					return NameResolvedFrom(story.Childhood.nameMaker, forcedLastName, forceNoNick);
				}
				if (story.Adulthood?.nameMaker != null)
				{
					return NameResolvedFrom(story.Adulthood.nameMaker, forcedLastName, forceNoNick);
				}
			}
			if (nameGenner != null)
			{
			...
			if (primaryCulture != null)
			{
				...
				{
					return NameResolvedFrom(pawnNameMaker, forcedLastName, forceNoNick);
				}
			}
			if (nameCategory != 0)
			{
		...
		private static Name NameResolvedFrom(RulePackDef nameMaker, string forcedLastName, bool forceNoNick = false)
		{
			return NameTriple.FromString(NameGenerator.GenerateName(nameMaker, (string x) => !NameTriple.FromString(x).UsedThisGame), forceNoNick);
		}
