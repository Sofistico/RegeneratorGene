	...
	public class Need_Rest : Need
	{
		...
		public override void NeedInterval()
		{
			if (!IsFrozen)
			{
				...
				{
					float num2 = RestFallPerTick * 150f;
					if (ModsConfig.BiotechActive)
					{
						num2 *= pawn.GetStatValue(StatDefOf.RestFallRateFactor);
					}
					CurLevel -= num2;
				}
			}
			...
			pawn.jobs.StartJob(job, JobCondition.InterruptForced, null, resumeCurJobAfterwards: false, cancelBusyStances: true, null, JobTag.SatisfyingNeeds);
			if (pawn.InMentalState && pawn.MentalStateDef.recoverFromCollapsingExhausted)
			{
				pawn.mindState.mentalStateHandler.CurState.RecoverFromState();
			}
			if (pawn.ageTracker.CurLifeStage?.involuntarySleepIsNegativeEvent ?? true)
			{
		...
		private bool ShouldInvoluntarySleepFromMTB()
		{
			...
			if (ticksAtZero > 1000)
			{
				num = ((ticksAtZero < 15000) ? 0.25f : ((ticksAtZero < 30000) ? 0.125f : ((ticksAtZero >= 45000) ? 0.0625f : 0.0833333358f)));
			}
