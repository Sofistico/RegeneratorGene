...
namespace RimWorld
{
	...
	public class Gizmo_GrowthTier : Gizmo
	{
		...
		{
			this.child = child;
			barTex = SolidColorMaterials.NewSolidColorTexture(new Color(32f / 255f, 46f / 85f, 0f));
			Order = -100f;
		}
		...
		private string GrowthTierTooltip(Rect rect, int tier)
		{
			...
			{
				taggedString += ("ProgressToNextGrowthTier".Translate() + ": ").AsTipTitle() + child.ageTracker.growthPoints.ToString("F0") + " / " + GrowthUtility.GrowthTierPointsRequirements[tier + 1] + " (+" + "PerDay".Translate(child.ageTracker.GrowthPointsPerDay.ToStringByStyle(ToStringStyle.FloatMaxTwo)) + ")";
			}
		...
		private void DrawGrowthTier(Rect rect)
		{
			...
			string label2 = (child.ageTracker.AtMaxGrowthTier ? (num + " / " + num) : (child.ageTracker.growthPoints.ToString("F0") + " / " + GrowthUtility.GrowthTierPointsRequirements[growthTier + 1]));
