	...
	public class Dialog_ReformIdeo : Window
	{
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			if (stage == IdeoReformStage.MemesAndStyles)
			{
				...
				Widgets.Label(new Rect(viewRect.x, num, viewRect.width, Text.LineHeight), (ideo.memes.Where((MemeDef m) => m.category == MemeCategory.Normal).Count() <= 1) ? "ReformIdeoAddMeme".Translate() : "ReformIdeoAddOrRemoveMeme".Translate());
				num += Text.LineHeight + 10f;
