	...
	public static class HealthCardUtility
	{
			...
			else
			{
				Action action = delegate
				{
					...
					if (medPawn != null)
					{
						...
						{
							Find.WindowStack.Add(new Dialog_SelectXenogerm(pawn.Map, null, delegate(Xenogerm x)
							{
		...
		public static Bill_Medical CreateSurgeryBill(Pawn medPawn, RecipeDef recipe, BodyPartRecord part, List<Thing> uniqueIngredients = null, bool sendMessages = true)
		{
			...
			if (sendMessages)
			{
				...
				if (!mapHeld.mapPawns.FreeColonists.Any((Pawn col) => recipe.PawnSatisfiesSkillRequirements(col)))
				{
		...
		{
			Pawn pawn;
			if ((pawn = giver as Pawn) == null || pawn.playerSettings == null)
			{
				return true;
			}
		...
		public static string GetPawnCapacityTip(Pawn pawn, PawnCapacityDef capacity)
		{
			...
			if (list.Count > 0)
			{
				...
				{
					PawnCapacityUtility.CapacityImpactorHediff capacityImpactorHediff;
					if ((capacityImpactorHediff = list[i] as PawnCapacityUtility.CapacityImpactorHediff) != null && !tmpHediffImpactors.Contains(capacityImpactorHediff.hediff))
					{
				...
				{
					PawnCapacityUtility.CapacityImpactorBodyPartHealth capacityImpactorBodyPartHealth;
					if ((capacityImpactorBodyPartHealth = list[j] as PawnCapacityUtility.CapacityImpactorBodyPartHealth) != null && !tmpBodyPartImpactors.Contains(capacityImpactorBodyPartHealth.bodyPart))
					{
				...
				{
					PawnCapacityUtility.CapacityImpactorGene capacityImpactorGene;
					if ((capacityImpactorGene = list[k] as PawnCapacityUtility.CapacityImpactorGene) != null && !tmpGeneImpactors.Contains(capacityImpactorGene.gene))
					{
