		...
		public IEnumerable<Thing> Things
		{
			...
			set
			{
				...
				foreach (Thing item2 in value)
				{
					if (item2.Destroyed)
					{
						Log.Error("Tried to add a destroyed thing to QuestPart_DropPods: " + item2.ToStringSafe());
						continue;
					}
					...
					items.Add(item2);
					ThingOwner thingOwner = item2.TryGetInnerInteractableThingOwner();
					if (thingOwner == null)
					{
						continue;
					}
