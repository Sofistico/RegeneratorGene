	...
	public class Xenogerm : GeneSetHolderBase
	{
		...
		public void SetTargetPawn(Pawn newTarget)
		{
			...
			Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation(text, delegate
			{
				...
				if (bill != null)
				{
					newTarget.BillStack.Delete(bill);
				}
			...
			if (targetPawn == null)
			{
				yield return new Command_Action
				{
					...
					action = delegate
					{
						...
						{
							Pawn pawn = item;
							if (!pawn.IsQuestLodger() && pawn.genes != null && (pawn.IsColonistPlayerControlled || pawn.IsPrisonerOfColony || pawn.IsSlaveOfColony))
							{
								int num = GeneUtility.MetabolismAfterImplanting(pawn, geneSet);
								if (num < GeneTuning.BiostatRange.TrueMin)
								{
									list.Add(new FloatMenuOption((string)(pawn.LabelShortCap + ": " + "ResultingMetTooLow".Translate() + " (") + num + ")", null));
								}
								else
								{
