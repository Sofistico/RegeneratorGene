...
namespace RimWorld
{
	public static class ChildcareUtility
	{
		...
		public static bool CanSuckleNow(Pawn baby, out BreastfeedFailReason? reason)
		{
			...
			if (baby.needs.food.CurLevelPercentage >= 0.66f)
			{
		...
		public static IEnumerable<Pawn> CanBreastfeedMothers(Pawn baby, bool includeAutoBreastfeedOff = false)
		{
			...
			foreach (Pawn freeColonistsAndPrisoner in baby.Map.mapPawns.FreeColonistsAndPrisoners)
			{
		...
		public static Job MakeAutofeedBabyJob(Pawn feeder, Pawn baby, Thing foodSource)
		{
			...
			Pawn pawn;
			if ((pawn = foodSource as Pawn) != null)
			{
				if (feeder == pawn)
				{
		...
		public static bool HasBreastfeedCompatibleFactions(Faction faction, Pawn baby)
		{
			if (baby.Faction != faction)
			{
				if (faction != null)
				{
					return baby.HostFaction == faction;
				}
				return false;
			}
			return true;
		...
		{
			if (!FoodUtility.TryFindBestFoodSourceFor(feeder, baby, baby.needs.food.CurCategory == HungerCategory.Starving, out var foodSource, out var _, canRefillDispenser: false, canUseInventory: true, canUsePackAnimalInventory: false, allowForbidden: false, allowCorpse: true, allowSociallyImproper: false, allowHarvest: false, forceScanWholeMap: false, ignoreReservations: false, calculateWantedStackCount: false, FoodPreferability.RawBad))
			{
				return null;
			}
			return foodSource;
		...
		public static bool BabyNeedsMovingForTemperatureReasons(Pawn baby, Pawn hauler, out Region preferredRegion, IntVec3? positionOverride = null)
		{
			...
			Region region = JobGiver_SeekSafeTemperature.ClosestRegionWithinTemperatureRange(root, baby.MapHeld, tempRange, TraverseParms.For(hauler, Danger.Some));
			if (region != null)
			{
			...
			Region region2 = JobGiver_SeekSafeTemperature.ClosestRegionWithinTemperatureRange(root, baby.MapHeld, tempRange2, TraverseParms.For(hauler, Danger.Some));
			if (region2 != null)
			{
		...
		public static Pawn FindUnsafeBaby(Pawn mom, AutofeedMode priorityLevel)
		{
			...
			foreach (Pawn item in mom.MapHeld.mapPawns.FreeHumanlikesSpawnedOfFaction(mom.Faction))
			{
				...
				if ((building_Bed = localTargetInfo.Thing as Building_Bed) != null)
				{
