...
namespace RimWorld
{
	public class Pawn_MechanitorTracker : IExposable
	{
			...
			get
			{
				...
				for (int i = 0; i < maps.Count; i++)
				{
					...
					for (int j = 0; j < list.Count; j++)
					{
						...
						if ((building_MechGestator = list[j] as Building_MechGestator) != null && building_MechGestator.ActiveBill != null && building_MechGestator.ActiveBill.BoundPawn == pawn)
						{
							activeMechBills.Add(building_MechGestator.ActiveBill);
						}
					}
				}
		...
		{
			if (subject.TryGetComp<CompOverseerSubject>() != null)
			{
		...
		public void Notify_BandwidthChanged()
		{
			...
			for (int num2 = tmpMechsInAssignedOrder.Count - 1; num2 >= 0; num2--)
			{
				...
				{
					tmpMechsInAssignedOrder[num2].TryGetComp<CompOverseerSubject>()?.Notify_DisconnectedFromOverseer();
				}
			}
			tmpPrevControlledMechs.Clear();
			tmpMechsInAssignedOrder.Clear();
