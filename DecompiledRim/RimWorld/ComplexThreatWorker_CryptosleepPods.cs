...
namespace RimWorld
{
	public class ComplexThreatWorker_CryptosleepPods : ComplexThreatWorker
	{
		...
		{
			if (base.CanResolveInt(parms) && ComplexUtility.TryFindRandomSpawnCell(ThingDefOf.AncientCryptosleepPod, parms.room.SelectMany((CellRect r) => r.Cells), parms.map, out var _) && parms.points >= PawnKindDefOf.AncientSoldier.combatPower)
			{
		...
		private List<Thing> SpawnCasketsWithHostiles(List<CellRect> room, float threatPoints, string openSignal, Map map)
		{
			...
			for (int i = 0; i < num; i++)
			{
				if (!ComplexUtility.TryFindRandomSpawnCell(ThingDefOf.AncientCryptosleepPod, room.SelectMany((CellRect r) => r.Cells), map, out var spawnPosition))
				{
					break;
				}
