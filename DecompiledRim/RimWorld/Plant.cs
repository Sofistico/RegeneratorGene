...
namespace RimWorld
{
	...
	public class Plant : ThingWithComps
	{
		...
		private static readonly FloatRange PollutionDamagePerTickRange = new FloatRange(1.66666669E-05f, 0.000166666665f);
		...
		public float GrowthRateFactor_Light
		{
			get
			{
				float glow = base.Map.glowGrid.GameGlowAt(base.Position);
		...
		public Thing TrySpawnStump(PlantDestructionMode treeDestructionMode)
		{
			...
			if (thingDef != null)
			{
				...
				DeadPlant deadPlant;
				if ((deadPlant = thing as DeadPlant) != null)
				{
		...
		public bool DeliberatelyCultivated()
		{
			...
			if ((zone_Growing = base.Map.zoneManager.ZoneAt(base.Position) as Zone_Growing) != null && zone_Growing.GetPlantDefToGrow() == def)
			{
				return true;
			}
			Building edifice = base.Position.GetEdifice(base.Map);
		...
		public override IEnumerable<Gizmo> GetGizmos()
		{
			...
			if (Blighted)
			{
				...
				if (designation == null || designation.def != DesignationDefOf.CutPlant)
				{
					...
					command_Action.defaultLabel = "CutAllBlight".Translate();
					command_Action.defaultDesc = "CutAllBlightDesc".Translate();
