...
namespace RimWorld
{
	...
	public class Designator_Dropdown : DesignatorWithEyedropper
	{
		...
		{
			Designator_Place designator_Place;
			BuildableDef placingDef;
			Window window = (elements.Any((Designator x) => (designator_Place = x as Designator_Place) != null && (placingDef = designator_Place.PlacingDef) != null && placingDef.designatorDropdown.useGridMenu) ? SetupGridMenu(ev) : SetupFloatMenu(ev));
		...
		private Window SetupGridMenu(Event ev)
		{
			...
			Designator_Place designator_Place3;
			TerrainDef terrainDef2;
			if (elements.Any((Designator x) => (designator_Place3 = x as Designator_Place) != null && (terrainDef2 = designator_Place3.PlacingDef as TerrainDef) != null && terrainDef2.designatorDropdown.includeEyeDropperTool))
			{
				if (eyedropper == null)
				{
					eyedropper = new Designator_Eyedropper(delegate(ColorDef newCol)
					{
						for (int j = 0; j < elements.Count; j++)
						{
							...
							Designator_Place designator_Place2;
							TerrainDef terrainDef;
							if ((designator_Place2 = designator2 as Designator_Place) != null && (terrainDef = designator_Place2.PlacingDef as TerrainDef) != null && terrainDef.colorDef == newCol)
							{
			...
			for (int i = 0; i < elements.Count; i++)
			{
				...
				Designator_Place designator_Place;
				BuildableDef placingDef;
				if ((designator_Place = designator as Designator_Place) != null && (placingDef = designator_Place.PlacingDef) != null)
				{
