	...
	public class PreceptWorker_Apparel : PreceptWorker
	{
		...
		public override bool ShouldSkipThing(Ideo ideo, ThingDef thingDef)
		{
			foreach (Precept item in ideo.PreceptsListForReading)
			{
				...
				if ((precept_Apparel = item as Precept_Apparel) != null && precept_Apparel.apparelDef == thingDef)
				{
					return true;
				}
			}
		...
		private bool IsValidApparel(ThingDef td)
		{
			...
			for (int i = 0; i < def.comps.Count; i++)
			{
				...
				if ((preceptComp_Apparel = def.comps[i] as PreceptComp_Apparel) != null && !preceptComp_Apparel.CanApplyToApparel(td))
				{
					return false;
				}
			}
