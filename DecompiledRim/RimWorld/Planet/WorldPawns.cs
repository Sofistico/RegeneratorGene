	...
	public class WorldPawns : IExposable
	{
		...
		private HediffDef DefPreventingMothball(Pawn p)
		{
			...
			for (int i = 0; i < hediffs.Count; i++)
			{
				...
				if (!hediffs[i].def.AlwaysAllowMothball && !hediffs[i].IsPermanent() && ((hediff_MissingPart = hediffs[i] as Hediff_MissingPart) == null || hediff_MissingPart.Bleeding) && (!hediffs[i].def.allowMothballIfLowPriorityWorldPawn || !lowPrioritySituations.Contains(situation)))
				{
