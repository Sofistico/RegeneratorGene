	...
	public static class PlantUtility
	{
		...
		public static bool CanNowPlantAt(this ThingDef plantDef, IntVec3 c, Map map, bool canWipePlantsExceptTree = false)
		{
			...
			foreach (Thing thing in c.GetThingList(map))
			{
				...
				Building building;
				if ((building = thing as Building) != null && map.listerBuildings.TryGetReinstallBlueprint(building, out var _))
				{
					return false;
				}
			}
		...
		{
			IPlantToGrowSettable settable;
			if ((settable = obj as IPlantToGrowSettable) != null && !PollutionUtility.CanPlantAt(plantDef, settable))
			{
				return false;
			}
			if (obj is Zone)
