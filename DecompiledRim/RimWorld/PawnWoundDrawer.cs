	...
	public class PawnWoundDrawer : PawnOverlayDrawer
	{
		...
		protected override void WriteCache(CacheKey key, List<DrawCall> writeTarget)
		{
			...
			if (debugDrawPart != null)
			{
				...
				if (list.Count > 0)
				{
					foreach (BodyTypeDef.WoundAnchor item2 in list)
					{
						if (AnchorUseable(item2))
						{
							Material overlayMat = MaterialPool.MatFrom(new MaterialRequest(BaseContent.WhiteTex, ShaderDatabase.SolidColor, item2.debugColor));
							CalcAnchorData(item2, out var anchorOffset2, out var range2);
			...
			for (int i = 0; i < pawn.health.hediffSet.hediffs.Count && (pawn.RaceProps.Humanlike || writeTarget.Count < 5); i++)
			{
				...
				if (hediff is Hediff_AddedPart && pawn.apparel != null)
				{
					...
					{
						if (item3.def.apparel.blocksAddedPartWoundGraphics && item3.def.apparel.CoversBodyPart(hediff.Part))
						{
							flag = true;
							break;
						}
				...
				try
				{
					...
					Hediff_Injury hd;
					if (resolvedWound != null && (resolvedWound.wound.displayPermanent || (hd = hediff as Hediff_Injury) == null || !hd.IsPermanent()))
					{
