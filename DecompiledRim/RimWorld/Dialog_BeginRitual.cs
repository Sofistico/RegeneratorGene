...
namespace RimWorld
{
	...
	public class Dialog_BeginRitual : Window
	{
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			void Start()
			{
				if (action?.Invoke(assignments) ?? false)
				{
					Close();
				}
			}
		...
		private void CalculatePawnPortraitIcons(Pawn pawn)
		{
			...
			Ideo ideo = pawn.Ideo;
			PawnPortraitIcon item;
			if (assignments.Required(pawn))
			{
				List<PawnPortraitIcon> list = tmpPortraitIcons;
				item = new PawnPortraitIcon
				{
					color = Color.white,
			...
			{
				List<PawnPortraitIcon> list2 = tmpPortraitIcons;
				item = new PawnPortraitIcon
				{
					color = ideo.Color,
					icon = ideo.Icon,
				...
				if (role != null)
				{
					List<PawnPortraitIcon> list3 = tmpPortraitIcons;
					item = new PawnPortraitIcon
					{
						color = ideo.Color,
						icon = role.Icon,
			...
			{
				List<PawnPortraitIcon> list4 = tmpPortraitIcons;
				item = new PawnPortraitIcon
				{
			...
			if (pawn.IsSlave)
			{
				List<PawnPortraitIcon> list5 = tmpPortraitIcons;
				item = new PawnPortraitIcon
				{
					color = Color.white,
					icon = SlaveryIcon,
			...
			if (pawn.IsPrisoner)
			{
				List<PawnPortraitIcon> list6 = tmpPortraitIcons;
				item = new PawnPortraitIcon
				{
					color = Color.white,
					icon = prisonerIcon,
			...
			if (!pawn.Awake())
			{
				List<PawnPortraitIcon> list7 = tmpPortraitIcons;
				item = new PawnPortraitIcon
				{
					color = Color.white,
					icon = sleepingIcon,
		...
		protected void DrawPawnList(Rect viewRect, Rect listRect)
		{
			...
			try
			{
				...
				void DrawRoleGroup(IEnumerable<Pawn> selectedPawns, string headline, int maxPawns, Action<Pawn, Vector2> assignAction, object dropAreaContext, string extraInfo, bool locked, Texture2D extraInfoIcon, Action<Pawn> clickHandler, Action<Pawn> rightClickHandler, bool grayOutIfNoAssignableRole)
				{
					...
					try
					{
						...
						IGrouping<string, RitualRole> grouping2 = dropAreaContext as IGrouping<string, RitualRole>;
						if (grouping2 != null && Mouse.IsOver(rect6))
						{
							highlightedRole = grouping2.First();
						}
						if (locked)
						{
			...
			static string PostProcessFloatLabel(string label, string unavailableReason, Pawn replacing)
			{
				string text4 = label;
		...
		protected IEnumerable<string> BlockingIssues()
		{
			...
			{
				if (!participant.SafeTemperatureRange().IncludesEpsilon(target.Cell.GetTemperature(target.Map)))
				{
