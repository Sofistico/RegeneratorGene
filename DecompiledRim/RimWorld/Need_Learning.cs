		...
		public override void DrawOnGUI(Rect rect, int maxThresholdMarkers = int.MaxValue, float customMargin = -1f, bool drawArrows = true, bool doTooltip = true, Rect? rectForTooltip = null, bool drawLabel = true)
		{
			...
			for (int i = 0; i < activeLearningDesires.Count; i++)
			{
				...
				if (Mouse.IsOver(rect2))
				{
					...
					TooltipHandler.TipRegion(rect2, () => ("LearningDesireTooltip".Translate() + "\n\n" + "LearningDesireTooltip_CurrentDesire".Translate(pawn).Colorize(ColoredText.TipSectionTitleColor).ResolveTags() + "\n\n" + learningDesire.LabelCap + ": " + learningDesire.description).Resolve(), learningDesire.GetHashCode());
					doTooltip = false;
				}
			}
