	...
	public class LordToil_ReimplantXenogerm : LordToil
	{
		...
		public override void UpdateAllDuties()
		{
			...
			for (int i = 0; i < lord.ownedPawns.Count; i++)
			{
				...
				{
					lord.ownedPawns[i].mindState.duty = new PawnDuty(DutyDefOf.WanderClose, lordToilData_ReimplantXenogerm.waitSpot, 10f);
				}
			}
		}
		...
		public override IEnumerable<FloatMenuOption> ExtraFloatMenuOptions(Pawn requester, Pawn current)
		{
			...
			if (current.genes.Xenotype == lordToilData_ReimplantXenogerm.target.genes.Xenotype)
			{
