...
namespace RimWorld
{
	public class QuestPart_SpawnWorldObject : QuestPart
	{
		...
		public override void Notify_QuestSignalReceived(Signal signal)
		{
			...
			else if (Find.WorldObjects.AnyWorldObjectAt(tile) && !TileFinder.TryFindPassableTileWithTraversalDistance(tile, 1, 50, out tile, (int x) => !Find.WorldObjects.AnyWorldObjectAt(x), ignoreFirstTilePassability: false, TileFinderMode.Near))
			{
				tile = -1;
			}
			if (tile != -1)
