	...
	public class Dialog_ChooseThingsForNewColony : Window
	{
		...
		public int SlaveCount
		{
			get
			{
				...
				{
					Pawn pawn;
					if (selected.Contains(colonists[i]) && (pawn = colonists[i] as Pawn) != null && pawn.IsSlaveOfColony)
					{
						num++;
					}
				}
		...
		public override void PostOpen()
		{
			...
			foreach (Precept item2 in Faction.OfPlayer.ideos.PrimaryIdeo.PreceptsListForReading)
			{
				...
				if ((precept_Relic = item2 as Precept_Relic) != null && precept_Relic.RelicInPlayerPossession)
				{
		...
		private void ConfirmArchonexusSettlementConsequences(List<Thing> selectedList, bool onlySlavesSelected)
		{
			...
			void CloseAction()
			{
				...
				if (onlySlavesSelected)
				{
					...
					{
						Pawn pawn;
						if ((pawn = selected as Pawn) != null && pawn.IsSlaveOfColony)
						{
		...
		private void DoRow(Rect rect, Thing t, int index, bool isItem)
		{
			...
			if (isItem)
			{
				...
				Text.Anchor = TextAnchor.MiddleRight;
				Widgets.Label(label: (MoveColonyUtility.IsDistinctArchonexusItem(t.def) ? t.stackCount : itemArchonexusAllowedStackCount[t]).ToString(), rect: rect6.LeftPartPixels(rect6.width - 10f));
				Text.Anchor = anchor;
				Text.WordWrap = true;
			}
