...
namespace RimWorld
{
	public class Dialog_FormCaravan : Window
	{
		...
		private void TrySend()
		{
			...
			{
				IEnumerable<Pawn> source = pawns.Where(delegate(Pawn x)
				{
					Hediff_PsychicBond hediff_PsychicBond = x.health.hediffSet.GetFirstHediffOfDef(HediffDefOf.PsychicBond) as Hediff_PsychicBond;
					return (hediff_PsychicBond != null && ThoughtWorker_PsychicBondProximity.NearPsychicBondedPerson(x, hediff_PsychicBond) && !pawns.Contains(hediff_PsychicBond.target)) ? true : false;
				});
				if (source.Any())
				{
		...
		private bool ShouldShowWarningForUndesirableFood()
		{
			...
			foreach (TransferableOneWay transferable in transferables)
			{
				...
				{
					tmpFood.Add(new ThingCount(transferable.AnyThing, transferable.CountToTransfer));
		...
		private void Notify_TransferablesChanged()
		{
			...
			if (ModsConfig.BiotechActive)
			{
				foreach (TransferableOneWay transferable in transferables)
				{
					...
					{
						Pawn pawn;
						if (!transferable.IsThing || (pawn = transferable.things[i] as Pawn) == null || !pawn.IsColonyMech)
						{
							continue;
						}
						Pawn overseer = pawn.GetOverseer();
