...
namespace RimWorld
{
	public class Precept_Building : Precept_ThingStyle
	{
		...
		public override List<Thought_Situational> SituationThoughtsToAdd(Pawn pawn, List<Thought_Situational> activeThoughts)
		{
			...
			if (mapHeld != null)
			{
				...
				if (!presenceDemand.BuildingPresent(mapHeld))
				{
					...
					if (!activeThoughts.Any((Thought_Situational t) => (thought_IdeoMissingBuilding2 = t as Thought_IdeoMissingBuilding) != null && thought_IdeoMissingBuilding2.demand == presenceDemand))
					{
				...
				else if (!presenceDemand.RequirementsSatisfied(mapHeld) && !activeThoughts.Any((Thought_Situational t) => (thought_IdeoDisrespectedBuilding2 = t as Thought_IdeoDisrespectedBuilding) != null && thought_IdeoDisrespectedBuilding2.demand == presenceDemand))
				{
		...
		public override void CopyTo(Precept precept)
		{
			...
			precept_Building.presenceDemand = presenceDemand;
