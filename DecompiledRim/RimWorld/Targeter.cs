...
namespace RimWorld
{
	public class Targeter
	{
		...
		public void BeginTargeting(ITargetingSource source, ITargetingSource parent = null, bool allowNonSelectedTargetingSource = false, Func<LocalTargetInfo, ITargetingSource> extraSourceGetter = null, Action actionWhenFinished = null)
		{
		...
		private void ConfirmStillValid()
		{
			...
			if (targetingSource.Caster.Map != Find.CurrentMap || targetingSource.Caster.Destroyed || (!allowNonSelectedTargetingSource && !selector.IsSelected(targetingSource.Caster)) || (targetingSource.GetVerb != null && !targetingSource.GetVerb.Available()) || ((compAbilityEffect_WithDest = targetingSource as CompAbilityEffect_WithDest) != null && compAbilityEffect_WithDest.SelectedTargetInvalidated()) || ((verb_CastAbility = targetingSource as Verb_CastAbility) != null && !verb_CastAbility.ability.CanQueueCast))
			{
				StopTargeting();
				return;
			}
		...
		private void OrderVerbForceTarget()
		{
			...
			Building_Turret building_Turret2;
			if (targetingSource != null && (building_Turret2 = targetingSource.Caster as Building_Turret) != null && building_Turret2 != null && building_Turret2.Map == Find.CurrentMap)
			{
		...
		private LocalTargetInfo CurrentTargetUnderMouse(bool mustBeHittableNowIfNotMelee)
		{
			...
			LocalTargetInfo localTargetInfo = GenUI.TargetsAtMouse(targetingParameters, thingsOnly: false, targetingSource).FirstOrFallback(LocalTargetInfo.Invalid);
