...
namespace Verse
{
	public class Room
	{
		...
		public string GetRoomRoleLabel()
		{
			...
			TaggedString taggedString = ((pawn == null) ? ((TaggedString)Role.PostProcessedLabelCap) : ((pawn2 != null) ? "CouplesRoom".Translate(pawn.LabelShort, pawn2.LabelShort, Role.label, pawn.Named("PAWN1"), pawn2.Named("PAWN2")) : "SomeonesRoom".Translate(pawn.LabelShort, Role.label, pawn.Named("PAWN"))));
			return taggedString;
		}
		...
		private string DebugRolesString()
		{
			...
			foreach (var (num, arg) in from x in DefDatabase<RoomRoleDef>.AllDefs
				select (x.Worker.GetScore(this), x) into tuple
