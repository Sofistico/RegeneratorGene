...
namespace Verse
{
	...
	public static class GenUI
	{
		...
		public static readonly Color FillableBar_Green = new Color(103f / 255f, 179f / 255f, 73f / 255f);
		...
		public static IEnumerable<LocalTargetInfo> TargetsAt(Vector3 clickPos, TargetingParameters clickParams, bool thingsOnly = false, ITargetingSource source = null)
		{
			...
			if (!thingsOnly)
			{
				...
				if (intVec.InBounds(Find.CurrentMap) && clickParams.CanTarget(new TargetInfo(intVec, Find.CurrentMap), source))
				{
					yield return intVec;
				}
			}
