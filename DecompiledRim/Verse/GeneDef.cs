	...
	public class GeneDef : Def
	{
		...
		public string DescriptionFull
		{
			get
			{
				...
				{
					cachedDescription = GetDescriptionFull();
				}
		...
		public bool HasGraphic
		{
			get
			{
				if (graphicData != null)
				{
					if (graphicData.graphicPath == null && graphicData.graphicPathFemale == null)
					{
						return !graphicData.graphicPaths.NullOrEmpty();
					}
					return true;
				}
				return false;
		...
		{
			if (graphicData == null || graphicData.layer == GeneDrawLayer.None)
			{
				return false;
			}
			if (rotation == Rot4.North)
			{
				if (!graphicData.visibleNorth)
				{
					return false;
				}
				if (graphicData.drawNorthAfterHair)
				{
					return drawLayer != GeneDrawLayer.PostHair;
				}
			}
			return drawLayer == graphicData.layer;
		}
		...
		private string GetDescriptionFull()
		{
			...
			if (biostatMet != 0)
			{
				sb.AppendLineTagged("Metabolism".Translate().Colorize(GeneUtility.METColor) + ": " + biostatMet.ToStringWithSign());
				flag2 = true;
			}
			if (biostatArc != 0)
			{
			...
			if (addictionChanceFactor != 1f && chemical != null)
			{
		...
		public override IEnumerable<StatDrawEntry> SpecialDisplayStats(StatRequest req)
		{
			...
			if (capMods != null)
			{
				...
				{
					PawnCapacityModifier capMod = capMods[k];
					if (capMod.offset != 0f)
					{
					...
					{
						yield return new StatDrawEntry(StatCategoryDefOf.CapacityEffects, capMod.capacity.GetLabelFor(isFlesh: true, isHumanlike: true).CapitalizeFirst(), "max".Translate().CapitalizeFirst() + " " + capMod.EvaluateSetMax(req.Pawn).ToStringPercent(), capMod.capacity.description, 4060);
					}
				}
			}
			if (biostatCpx != 0)
			{
				yield return new StatDrawEntry(StatCategoryDefOf.Genetics, "Complexity".Translate(), biostatCpx.ToString(), "ComplexityDesc".Translate(), 998);
			}
			if (biostatMet != 0)
			{
				yield return new StatDrawEntry(StatCategoryDefOf.Genetics, "Metabolism".Translate(), biostatMet.ToString(), "MetabolismDesc".Translate(), 997);
			}
			if (biostatArc != 0)
			{
				yield return new StatDrawEntry(StatCategoryDefOf.Genetics, "ArchitesRequired".Translate(), biostatArc.ToString(), "ArchitesRequiredDesc".Translate(), 995);
			}
		}
		...
		public override IEnumerable<string> ConfigErrors()
		{
			...
			foreach (string item in graphicData.ConfigErrors())
			{
				yield return "graphicData - " + item;
			}
		}
	}
}
