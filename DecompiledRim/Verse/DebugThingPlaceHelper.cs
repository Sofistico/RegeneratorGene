...
namespace Verse
{
	public static class DebugThingPlaceHelper
	{
		public static bool IsDebugSpawnable(ThingDef def, bool allowPlayerBuildable = false)
		{
			...
			if (def.thingClass == typeof(Corpse) || def.IsBlueprint || def.IsFrame || def == ThingDefOf.ActiveDropPod || def.thingClass == typeof(MinifiedThing) || def.thingClass == typeof(MinifiedTree) || def.thingClass == typeof(UnfinishedThing) || def.thingClass.IsSubclassOf(typeof(SignalAction)) || def.destroyOnDrop)
			{
				return false;
			}
