...
namespace Verse
{
	public class PawnRenderer
	{
		...
		private const float YOffset_OnHeadDirect = 0.0260617733f;
		...
		private void DrawHeadHair(Vector3 rootLoc, Vector3 headOffset, float angle, Rot4 bodyFacing, Rot4 headFacing, RotDrawMode bodyDrawType, PawnRenderFlags flags, bool bodyDrawn)
		{
			...
			Quaternion quat = Quaternion.AngleAxis(angle, Vector3.up);
			bool num = pawn.DevelopmentalStage.Baby() || bodyDrawType == RotDrawMode.Dessicated || flags.FlagSet(PawnRenderFlags.HeadStump);
			bool flag = !num || pawn.story?.hairDef == null || pawn.story.hairDef.noGraphic;
			bool flag2 = !num && bodyFacing != Rot4.North && pawn.DevelopmentalStage.Adult() && (pawn.style?.beardDef ?? BeardDefOf.NoBeard) != BeardDefOf.NoBeard;
			...
			if (flag2)
			{
				Vector3 loc2 = OffsetBeardLocationForHead(pawn.style.beardDef, pawn.story.headType, headFacing, rootLoc + headOffset);
			...
			void DrawGene(GeneGraphicRecord geneRecord, GeneDrawLayer layer)
			{
				...
				{
					Vector3 loc4 = HeadGeneDrawLocation(geneRecord.sourceGene.def, pawn.story.headType, headFacing, rootLoc + headOffset, layer);
