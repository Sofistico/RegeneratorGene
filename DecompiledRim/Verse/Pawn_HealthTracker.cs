...
namespace Verse
{
	public class Pawn_HealthTracker : IExposable
	{
		...
		private void ForceDeathrestOrComa(DamageInfo? dinfo, Hediff hediff)
		{
			...
			else if (!hediffSet.HasHediff(HediffDefOf.RegenerationComa))
			{
				AddHediff(HediffDefOf.RegenerationComa);
				if (!Downed)
				{
					forceDowned = true;
					MakeDowned(dinfo, hediff);
		...
		private void MakeDowned(DamageInfo? dinfo, Hediff hediff)
		{
			...
			if (this.pawn.Spawned)
			{
				this.pawn.DropAndForbidEverything(keepInventoryAndEquipmentIfInBed: true, rememberPrimary: true);
				this.pawn.stances.CancelBusyStanceSoft();
			}
			if (!this.pawn.DutyActiveWhenDown(onlyInBed: true))
			{
				this.pawn.ClearMind(ifLayingKeepLaying: true, clearInspiration: false, clearMentalState: false);
			}
			if (Current.ProgramState == ProgramState.Playing)
			{
				this.pawn.GetLord()?.Notify_PawnLost(this.pawn, PawnLostCondition.Incapped, dinfo);
			}
			...
			this.pawn.mechanitor?.Notify_Downed();
		}
