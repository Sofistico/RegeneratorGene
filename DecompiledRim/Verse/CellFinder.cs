...
namespace Verse
{
	public static class CellFinder
	{
		...
		{
			return TryFindRandomReachableCellNear(root, map, radius, TraverseParms.For(TraverseMode.NoPassClosedDoors).WithFenceblocked(forceFenceblocked: true), (IntVec3 c) => c.Standable(map) && (extraValidator == null || extraValidator(c)), null, out result);
		}
		...
		public static IntVec3 RandomClosewalkCellNearNotForbidden(Pawn pawn, int radius)
		{
			...
			Map map = pawn.Map;
			if (!TryFindRandomReachableCellNear(position, map, radius, TraverseParms.For(TraverseMode.NoPassClosedDoors).WithFenceblockedOf(pawn), (IntVec3 c) => !c.IsForbidden(pawn) && c.Standable(map), null, out var result))
			{
				return RandomClosewalkCellNear(position, map, radius);
			}
			return result;
		}
