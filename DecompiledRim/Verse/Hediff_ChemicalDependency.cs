...
namespace Verse
{
	public class Hediff_ChemicalDependency : HediffWithComps
	{
			...
			get
			{
				if (cachedDependencyGene == null && pawn.genes != null)
				{
					...
					for (int i = 0; i < genesListForReading.Count; i++)
					{
						...
						if ((gene_ChemicalDependency = genesListForReading[i] as Gene_ChemicalDependency) != null && gene_ChemicalDependency.def.chemical == chemical)
						{
		...
		public override bool TryMergeWith(Hediff other)
		{
			...
			if ((hediff_ChemicalDependency = other as Hediff_ChemicalDependency) == null)
			{
				return false;
			}
