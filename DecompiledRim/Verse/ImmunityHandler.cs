...
namespace Verse
{
	public class ImmunityHandler : IExposable
	{
		...
		public float GetImmunity(HediffDef def, bool naturalImmunityOnly = false)
		{
			...
			if (!naturalImmunityOnly && (AnyHediffMakesFullyImmuneTo(def, out var _) || AnyGeneMakesFullyImmuneTo(def)) && num < 0.650000036f)
			{
				num = 0.650000036f;
			}
			return num;
		...
		public ImmunityRecord GetImmunityRecord(HediffDef def)
		{
			...
			if (AnyHediffMakesFullyImmuneTo(def, out var sourceHediff) && (immunityRecord == null || immunityRecord.immunity < 0.650000036f))
			{
