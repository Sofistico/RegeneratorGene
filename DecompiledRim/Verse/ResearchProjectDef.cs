...
namespace Verse
{
	public class ResearchProjectDef : Def
	{
		...
		public bool CanStartNow
		{
			get
			{
				if (!IsFinished && PrerequisitesCompleted && TechprintRequirementMet && (requiredResearchBuilding == null || PlayerHasAnyAppropriateResearchBench) && PlayerMechanitorRequirementMet)
				{
					return StudiedThingsRequirementsMet;
				}
				return false;
			}
		}
		...
		public int RequiredStudiedThingCount
		{
			get
			{
				...
				return requiredStudied?.Count ?? 0;
			}
		}
		...
		public int StudiedThingsCompleted
		{
			get
			{
				if (requiredStudied.NullOrEmpty())
				{
					return 0;
				}
				int num = 0;
				for (int i = 0; i < requiredStudied.Count; i++)
				{
					if (Find.StudyManager.StudyComplete(requiredStudied[i]))
					{
						num++;
					}
				}
		...
		public bool StudiedThingsRequirementsMet
		{
			get
			{
				if (RequiredStudiedThingCount > 0)
				{
					return StudiedThingsCompleted >= RequiredStudiedThingCount;
				}
				return true;
			}
		}
		...
		public override IEnumerable<string> ConfigErrors()
		{
			...
			if (!ModLister.BiotechInstalled && !requiredStudied.NullOrEmpty())
			{
			...
			foreach (ThingDef item2 in requiredStudied)
			{
				if (!item2.HasComp(typeof(CompStudiable)))
				{
					yield return "requires studying " + item2.label + " but " + item2.label + " cannot be studied.";
				}
			}
		}
		...
		public string GetTip()
		{
			if (cachedTip == null)
			{
				cachedTip = LabelCap.Colorize(ColoredText.TipSectionTitleColor) + "\n" + description;
				if (TechprintCount > 0)
				{
				...
				if (RequiredStudiedThingCount > 0)
				{
					cachedTip = cachedTip + "\n\n" + "StudyRequirementTip".Translate().ToString() + ": " + requiredStudied.Select((ThingDef t) => t.label).ToCommaList(useAnd: true).CapitalizeFirst();
				}
