...
namespace Verse
{
	public static class GenColor
	{
		...
		private const float redScaleFactor = 1.29293621f;

		private const float redPowerFactor = -0.133204758f;

		private const float blueScaleFactor = 0.5432068f;

		private const float blueOffset = 1.19625413f;

		private const float coolGreenScale = 0.390081584f;

		private const float coolGreenOffset = (float)Math.E * 139f / 598f;

		private const float warmGreenScale = 1.12989092f;

		private const float warmGreenPower = -0.0755148456f;
		...
		public static Color FromColorTemperature(float temperatureKelvin)
		{
			...
			{
				value = 1f;
				value2 = 0.390081584f * Mathf.Log(num) - (float)Math.E * 139f / 598f;
				value3 = ((!(num <= 19f)) ? (0.5432068f * Mathf.Log(num - 10f) - 1.19625413f) : 0f);
			}
			else
			{
				num -= 60f;
				value = 1.29293621f * Mathf.Pow(num, -0.133204758f);
				value2 = 1.12989092f * Mathf.Pow(num, -0.0755148456f);
				value3 = 1f;
			}
		...
		public static float? ColorTemperature(this Color color)
		{
			...
			if (num4 < 1f)
			{
				...
				float num5 = Mathf.Exp((num3 + (float)Math.E * 139f / 598f) / 0.390081584f);
				float num6 = ((num4 != 0f) ? (Mathf.Exp((num4 + 1.19625413f) / 0.5432068f) + 10f) : Mathf.Min(19f, num5));
			...
			{
				float num8 = Mathf.Exp(Mathf.Log(num3 / 1.12989092f) / -0.0755148456f) + 60f;
				float num9 = ((num2 != 1f) ? (Mathf.Exp(Mathf.Log(num2 / 1.29293621f) / -0.133204758f) + 60f) : Mathf.Min(66.98f, Mathf.Max(66f, num8)));
