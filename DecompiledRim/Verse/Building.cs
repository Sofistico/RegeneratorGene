...
namespace Verse
{
	public class Building : ThingWithComps
	{
		...
		public override void DeSpawn(DestroyMode mode = DestroyMode.Vanish)
		{
			...
			{
				int itemCount = item2.GetItemCount(map);
				if (itemCount <= 1 || itemCount <= item2.GetMaxItemsAllowedInCell(map))
				{
					continue;
				}
				for (int k = 0; k < itemCount - 1; k++)
				{
					Thing firstItem = item2.GetFirstItem(map);
					if (firstItem == null)
					{
						break;
					}
					firstItem.DeSpawn();
					GenPlace.TryPlaceThing(firstItem, item2, map, ThingPlaceMode.Near);
					if (item2.GetItemCount(map) <= item2.GetMaxItemsAllowedInCell(map))
					{
						break;
					}
				}
		...
		public override IEnumerable<Gizmo> GetGizmos()
		{
			...
			Command command = BuildCopyCommandUtility.BuildCopyCommand(def, base.Stuff, base.StyleSourcePrecept as Precept_Building, base.StyleDef, styleOverridden: true, glowerColorOverride);
			if (command != null)
			{
		...
		public override string GetInspectStringLowPriority()
		{
			...
			if (def.IsNonDeconstructibleAttackableBuilding)
			{
				if (!text.NullOrEmpty())
				{
					text += "\n";
