	...
	public class Pawn_AgeTracker : IExposable
	{
		...
		public float PercentToNextGrowthTier
		{
			get
			{
				...
				return GrowthUtility.GrowthTierPointsRequirements[growthTier] / GrowthUtility.GrowthTierPointsRequirements[growthTier + 1];
			}
		}
		...
		public float GrowthPointsPerDay
		{
			get
			{
				...
				if (learning != null)
				{
					return GrowthPointsPerDayAtLearningLevel(learning.CurLevel);
				}
				return 0f;
			}
		}
		...
		private void BirthdayBiological(int birthdayAge)
		{
			...
			bool flag4 = PawnUtility.ShouldSendNotificationAbout(pawn) && pawn.Faction == Faction.OfPlayer && !pawn.IsQuestLodger();
			if (pawn.RaceProps.Humanlike && !flag4)
			{
				if (passionGainsCount > 0)
				{
					SkillDef skillDef = ChoiceLetter_GrowthMoment.PassionOptions(pawn, passionGainsCount).FirstOrFallback();
					if (skillDef != null)
					{
			...
			if (pawn.RaceProps.Humanlike && flag4 && (tmpHediffsGained.Count > 0 || flag3))
			{
				...
				if (ModsConfig.BiotechActive && flag3)
				{
					...
					if ((nameTriple = pawn.Name as NameTriple) != null && !nameTriple.NickSet)
					{
		...
		{
			if (pawn.Ideo?.HasPrecept(PreceptDefOf.AgeReversal_Demanded) ?? false)
			{
