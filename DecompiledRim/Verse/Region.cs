...
namespace Verse
{
	public sealed class Region
	{
		...
		public Danger DangerFor(Pawn p)
		{
			...
			{
				if (cachedDangersForFrame != RealTime.frameCount)
				{
					cachedDangers.Clear();
					cachedDangersForFrame = RealTime.frameCount;
				}
				else
				{
			...
			if (Current.ProgramState == ProgramState.Playing)
			{
				if (cachedSafeTemperatureRangesForFrame != RealTime.frameCount)
				{
					cachedSafeTemperatureRanges.Clear();
					cachedSafeTemperatureRangesForFrame = RealTime.frameCount;
				}
		...
		public override string ToString()
		{
			...
			return string.Concat("Region(id=", id, ", mapIndex=", mapIndex, ", center=", extentsClose.CenterCell, ", links=", links.Count, ", cells=", CellCount, (door != null) ? (", portal=" + text) : null, ")");
		}
