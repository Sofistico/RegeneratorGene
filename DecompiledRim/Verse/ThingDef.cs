...
namespace Verse
{
	public class ThingDef : BuildableDef
	{
		...
		public bool IsNonDeconstructibleAttackableBuilding
		{
			get
			{
				if (IsBuildingArtificial && !building.IsDeconstructible && destroyable && !mineable)
				{
		...
		public bool IsStudiable => HasComp(typeof(CompStudiable));
		...
		public override IEnumerable<StatDrawEntry> SpecialDisplayStats(StatRequest req)
		{
			...
			if (!verbs.NullOrEmpty())
			{
				...
				if (verb.defaultProjectile != null && verb.defaultProjectile.projectile.damageDef.harmsHealth)
				{
					...
					stringBuilder.AppendLine();
					yield return new StatDrawEntry(valueString: ((float)verb.defaultProjectile.projectile.GetDamageAmount(req.Thing, stringBuilder)).ToString(), category: statCat, label: "Damage".Translate(), reportText: stringBuilder.ToString(), displayPriorityWithinCategory: 5500);
			...
			if (isTechHediff)
			{
				...
				foreach (StatDrawEntry medicalStatsFromRecipeDef in MedicalRecipesUtility.GetMedicalStatsFromRecipeDefs(enumerable))
				{
			...
			for (int k = 0; k < comps.Count; k++)
			{
				foreach (StatDrawEntry item6 in comps[k].SpecialDisplayStats(req))
				{
					yield return item6;
				}
			}
			...
			if (IsMeat)
			{
				...
				string valueString3 = string.Join(", ", list.Select((ThingDef p) => p.label).ToArray()).CapitalizeFirst();
				yield return new StatDrawEntry(StatCategoryDefOf.BasicsPawn, "Stat_SourceSpecies_Name".Translate(), valueString3, "Stat_SourceSpecies_Desc".Translate(), 1200, null, Dialog_InfoCard.DefsToHyperlinks(list));
			}
			if (IsLeather)
			{
				...
				string valueString4 = string.Join(", ", list2.Select((ThingDef p) => p.label).ToArray()).CapitalizeFirst();
				yield return new StatDrawEntry(StatCategoryDefOf.BasicsPawn, "Stat_SourceSpecies_Name".Translate(), valueString4, "Stat_SourceSpecies_Desc".Translate(), 1200, null, Dialog_InfoCard.DefsToHyperlinks(list2));
			}
			if (!equippedStatOffsets.NullOrEmpty())
			{
				for (int k = 0; k < equippedStatOffsets.Count; k++)
				{
					...
					float num = equippedStatOffsets[k].value;
					StringBuilder stringBuilder4 = new StringBuilder(stat.description);
					if (req.HasThing && stat.Worker != null)
					{
						stringBuilder4.AppendLine();
						stringBuilder4.AppendLine();
						stringBuilder4.AppendLine("StatsReport_BaseValue".Translate() + ": " + stat.ValueToString(num, ToStringNumberSense.Offset, stat.finalizeEquippedStatOffset));
						num = StatWorker.StatOffsetFromGear(req.Thing, stat);
						...
						stringBuilder4.AppendLine();
						stringBuilder4.AppendLine("StatsReport_FinalValue".Translate() + ": " + stat.ValueToString(num, ToStringNumberSense.Offset, !stat.formatString.NullOrEmpty()));
					}
					yield return new StatDrawEntry(StatCategoryDefOf.EquippedStatOffsets, equippedStatOffsets[k].stat, num, StatRequest.ForEmpty(), ToStringNumberSense.Offset, null, forceUnfinalizedMode: true).SetReportText(stringBuilder4.ToString());
				}
			}
			if (!IsDrug)
			{
