...
namespace Verse
{
	public abstract class Verb : ITargetingSource, IExposable, ILoadReferenceable
	{
		...
		protected virtual float EffectiveRange => verbProps.AdjustedRange(this, CasterPawn);
		...
		public void VerbTick()
		{
			...
			{
				Pawn pawn;
				if (!caster.Spawned || ((pawn = caster as Pawn) != null && pawn.stances.stunner.Stunned))
				{
					Reset();
				}
				else
		...
		public virtual bool Available()
		{
			...
			if (CasterIsPawn && EquipmentSource != null && EquipmentUtility.RolePreventsFromUsing(CasterPawn, EquipmentSource, out var _))
			{
				return false;
			}
			return true;
		...
		public virtual bool ValidateTarget(LocalTargetInfo target, bool showMessages = true)
		{
			...
			Pawn victim;
			if (CasterIsPawn && (victim = target.Thing as Pawn) != null && HistoryEventUtility.IsKillingInnocentAnimal(CasterPawn, victim) && !new HistoryEvent(HistoryEventDefOf.KilledInnocentAnimal, CasterPawn.Named(HistoryEventArgsNames.Doer)).Notify_PawnAboutToDo())
			{
				return false;
			}
			Pawn pawn;
			if (CasterIsPawn && (pawn = target.Thing as Pawn) != null && CasterPawn.Ideo != null && CasterPawn.Ideo.IsVeneratedAnimal(pawn) && !new HistoryEvent(HistoryEventDefOf.HuntedVeneratedAnimal, CasterPawn.Named(HistoryEventArgsNames.Doer)).Notify_PawnAboutToDo())
			{
				return false;
			}
			return true;
