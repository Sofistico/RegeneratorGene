...
namespace Verse
{
	public class ThingOwner<T> : ThingOwner, IList<T>, ICollection<T>, IEnumerable<T>, IEnumerable where T : Thing
	{
		...
		public override void ExposeData()
		{
			...
			if (Scribe.mode == LoadSaveMode.PostLoadInit)
			{
				...
				int num = innerList.RemoveAll((T x) => x == null || ((minifiedThing = x as MinifiedThing) != null && minifiedThing.InnerThing == null));
				if (num > 0)
				{
	...
	public abstract class ThingOwner : IExposable, IList<Thing>, ICollection<Thing>, IEnumerable<Thing>, IEnumerable
	{
		...
		protected void NotifyAdded(Thing item)
		{
			...
			if (((thing = owner as Thing) != null && thing.Faction.IsPlayerSafe()) || ((worldObject = owner as WorldObject) != null && worldObject.Faction.IsPlayerSafe()) || ((pawn_InventoryTracker = owner as Pawn_InventoryTracker) != null && pawn_InventoryTracker.pawn.Faction.IsPlayerSafe()))
			{
				item.EverSeenByPlayer = true;
			}
			(owner as CompTransporter)?.Notify_ThingAdded(item);
			(owner as Caravan)?.Notify_PawnAdded((Pawn)item);
			Pawn_ApparelTracker pawn_ApparelTracker = owner as Pawn_ApparelTracker;
			...
			Pawn_EquipmentTracker pawn_EquipmentTracker = owner as Pawn_EquipmentTracker;
