...
namespace Verse
{
	public static class GenTemperature
	{
		...
		public static void EqualizeTemperaturesThroughBuilding(Building b, float rate, bool twoWay)
		{
			...
			for (int k = 0; k < num; k++)
			{
				if (!beqRooms[k].UsesOutdoorTemperature)
				{
					float temperature = beqRooms[k].Temperature;
					float num5 = (num3 - temperature) * rate;
					float num6 = num5 / (float)beqRooms[k].CellCount;
					float num7 = beqRooms[k].Temperature + num6;
					...
					float num8 = Mathf.Abs((num7 - temperature) * (float)beqRooms[k].CellCount / num5);
					if (num8 < num4)
					{
						num4 = num8;
					}
			...
			for (int l = 0; l < num; l++)
			{
				if (!beqRooms[l].UsesOutdoorTemperature)
				{
					float temperature2 = beqRooms[l].Temperature;
					float num9 = (num3 - temperature2) * rate * num4 / (float)beqRooms[l].CellCount;
					beqRooms[l].Temperature += num9;
				}
			}
			for (int m = 0; m < beqRooms.Length; m++)
			{
				beqRooms[m] = null;
			}
		}
