...
namespace Verse
{
	public static class DebugOutputsPawns
	{
		...
		public static void AnimalCombatBalance()
		{
			...
			static float combatPowerCalculated(PawnKindDef pawnKind)
			{
		...
		{
			Pawn pawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(pawnKind, null, PawnGenerationContext.NonPlayer, -1, forceGenerateNewPawn: false, allowDead: false, allowDowned: false, canGeneratePawnRelations: true, mustBeCapableOfViolence: true));
			...
			DebugTables.MakeTablesDialog(DefDatabase<ThoughtDef>.AllDefs.Where(delegate(ThoughtDef td)
			{
				foreach (PreceptDef item in DefDatabase<PreceptDef>.AllDefsListForReading)
				{
					foreach (PreceptComp comp in item.comps)
					{
						...
						if ((preceptComp_Thought = comp as PreceptComp_Thought) != null && preceptComp_Thought.thought == td)
						{
							return true;
						}
					}
