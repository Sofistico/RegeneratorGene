...
namespace Verse
{
	...
	public static class Widgets
	{
		...
		static Widgets()
		{
			...
			colorComponentLabels = new string[6] { "Red", "Green", "Blue", "Hue", "Saturation", "Value" };
		...
		public static void DefIcon(Rect rect, Def def, ThingDef stuffDef = null, float scale = 1f, ThingStyleDef thingStyleDef = null, bool drawPlaceholder = false, Color? color = null, Material material = null, int? graphicIndexOverride = null)
		{
			...
			else if ((pawnKindDef = def as PawnKindDef) != null)
			{
		...
		public static void ThingIcon(Rect rect, Thing thing, float alpha = 1f, Rot4? rot = null, bool stackOfOne = false)
		{
			...
			Color color;
			Texture resolvedIcon = GetIconFor(thing, new Vector2(rect.width, rect.height), rot, stackOfOne, out scale, out angle, out iconProportions, out color);
			GUI.color = color;
			...
			else if (thing is Pawn || thing is Corpse)
			{
				...
				{
					resolvedIcon = PortraitsCache.Get(pawn, new Vector2(rect.width, rect.height), rot ?? Rot4.South);
				}
			}
			if (alpha != 1f)
			{
			...
			ThingIconWorker(rect, thing.def, resolvedIcon, angle);
			GUI.color = Color.white;
		}
		...
		public static Texture GetIconFor(Thing thing, Vector2 size, Rot4? rot, bool stackOfOne, out float scale, out float angle, out Vector2 iconProportions, out Color color)
		{
			...
			else if ((pawn = thing as Pawn) != null)
			{
				...
				else
				{
					...
					r.y += 3f;
					r = r.Rounded();
					result = PortraitsCache.Get(pawn, new Vector2(r.width, r.height), Rot4.South, default(Vector3), 1.8f);
				}
			}
			else if (stackOfOne)
			{
				Graphic_StackCount graphic_StackCount = thing.Graphic as Graphic_StackCount;
				result = ((graphic_StackCount == null) ? thing.Graphic.ExtractInnerGraphicFor(thing).MatAt(thing.def.defaultPlacingRot).mainTexture : graphic_StackCount.SubGraphicForStackCount(1, thing.def).MatSingleFor(thing).mainTexture);
			}
			else
			{
				result = thing.Graphic.ExtractInnerGraphicFor(thing).MatAt(thing.def.defaultPlacingRot).mainTexture;
			}
			return result;
		}
		...
		public static void DrawBox(Rect rect, int thickness = 1, Texture2D lineTexture = null)
		{
			...
			GUI.DrawTexture(AdjustRectToUIScaling(new Rect(vector.x, vector.y, thickness, vector3.y)), lineTexture ?? BaseContent.WhiteTex);
			GUI.DrawTexture(AdjustRectToUIScaling(new Rect(vector2.x - (float)thickness, vector.y, thickness, vector3.y)), lineTexture ?? BaseContent.WhiteTex);
			GUI.DrawTexture(AdjustRectToUIScaling(new Rect(vector.x + (float)thickness, vector.y, vector3.x - (float)(thickness * 2), thickness)), lineTexture ?? BaseContent.WhiteTex);
			GUI.DrawTexture(AdjustRectToUIScaling(new Rect(vector.x + (float)thickness, vector2.y - (float)thickness, vector3.x - (float)(thickness * 2), thickness)), lineTexture ?? BaseContent.WhiteTex);
		}
		...
		public static bool LabelFit(Rect rect, string label)
		{
			...
			else
			{
				...
				{
					Label(rect, label.Truncate(rect.width));
					result = true;
				}
				Text.Font = GameFont.Small;
			}
		...
		public static void HyperlinkWithIcon(Rect rect, Dialog_InfoCard.Hyperlink hyperlink, string text = null, float iconMargin = 2f, float textOffsetX = 6f, Color? color = null, bool truncateLabel = false, string textSuffix = null)
		{
			...
			Text.Anchor = TextAnchor.MiddleLeft;
			Text.WordWrap = false;
			ButtonText(rect3, truncateLabel ? text2.Truncate(rect3.width) : text2, drawBackground: false, doMouseoverSound: false, color ?? NormalOptionColor, active: false);
		...
		public static void CheckboxLabeled(Rect rect, string label, ref bool checkOn, bool disabled = false, Texture2D texChecked = null, Texture2D texUnchecked = null, bool placeCheckboxNearText = false)
		{
			...
			CheckboxDraw(rect.x + rect.width - 24f, rect.y + (rect.height - 24f) / 2f, checkOn, disabled);
			Text.Anchor = anchor;
		}
		...
		public static bool ColorBox(Rect rect, ref Color color, Color boxColor, int colorSize = 22, int colorPadding = 2)
		{
			...
			DrawBoxSolid(new Rect(rect.x + (float)colorPadding, rect.y + (float)colorPadding, colorSize, colorSize), boxColor);
			bool result = false;
		...
		public static bool ColorSelector(Rect rect, ref Color color, List<Color> colors, out float height, Texture icon = null, int colorSize = 22, int colorPadding = 2)
		{
			...
			for (int i = 0; i < colors.Count; i++)
			{
				...
				if (ColorBox(rect2, ref color, colors[i], colorSize, colorPadding))
				{
					result = true;
				}
		...
		public static void ColorTemperatureBar(Rect rect, ref Color color, ref bool dragging, float? colorValueOverride = null)
		{
			...
			if (Event.current.button == 0)
			{
				...
				else if (ClickedInsideRect(rectDivider) || (dragging && Event.current.type == EventType.MouseDrag))
				{
					dragging = true;
					Event.current.Use();
		...
		public static void DraggableBar(Rect barRect, Texture2D barTexture, Texture2D barHighlightTexture, Texture2D emptyBarTex, Texture2D dragBarTex, ref bool draggingBar, float barValue, ref float targetValue, List<float> bandPercentages = null, int increments = 20)
		{
			...
			if (!bandPercentages.NullOrEmpty())
			{
				for (int i = 1; i < bandPercentages.Count - 1; i++)
				{
					DrawDraggableBarThreshold(barRect, bandPercentages[i], barValue);
				}
			}
		...
		private static void DrawDraggableBarTarget(Rect rect, float percent, Texture2D targetTex)
		{
			...
			float num2 = AdjustCoordToUIScalingFloor(rect.x + 2f + num);
			float xMax = AdjustCoordToUIScalingCeil(num2 + 4f);
