...
namespace Verse.AI
{
	public class JobDriver_Wait : JobDriver
	{
		...
		{
			if (base.pawn.Downed || base.pawn.stances.FullBodyBusy || base.pawn.IsCarryingPawn())
			{
				return;
			}
			...
			for (int i = 0; i < 9; i++)
			{
				...
				for (int j = 0; j < thingList.Count; j++)
				{
					if (flag)
					{
						...
						if (pawn != null && !pawn.Downed && base.pawn.HostileTo(pawn) && !base.pawn.ThreatDisabledBecauseNonAggressiveRoamer(pawn) && GenHostility.IsActiveThreatTo(pawn, base.pawn.Faction))
						{
