...
namespace Verse.AI
{
	public class JobDriver_DoBill : JobDriver
	{
		...
		{
			Bill_Mech bill;
			if (ModsConfig.BiotechActive && (bill = job.bill as Bill_Mech) != null)
			{
		...
		protected override IEnumerable<Toil> MakeNewToils()
		{
			...
			foreach (Toil item in CollectIngredientsToils(TargetIndex.B, TargetIndex.A, TargetIndex.C, subtractNumTakenFromJobCount: false, failIfStackCountLessThanJobCount: true, BillGiver is Building_MechGestator))
			{
				yield return item;
			}
