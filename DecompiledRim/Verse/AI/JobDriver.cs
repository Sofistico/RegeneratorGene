...
namespace Verse.AI
{
	public abstract class JobDriver : IExposable, IJobEndable
	{
		...
		{
			return job.reportStringOverride ?? ReportStringProcessed(job.def.reportString);
		}
		...
		public void DriverTick()
		{
			try
			{
				...
				else
				{
					...
					else if (CurToil.defaultCompleteMode == ToilCompleteMode.Delay)
					{
					...
					{
						if (CurToil.defaultCompleteMode != ToilCompleteMode.FinishedBusy || pawn.stances.FullBodyBusy)
						{
							goto IL_00bc;
						}
						ReadyForNextToil();
		...
		{
			if (CurToil.defaultCompleteMode == ToilCompleteMode.PatherArrival)
			{
				ReadyForNextToil();
			}
		}
