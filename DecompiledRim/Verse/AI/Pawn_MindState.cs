...
namespace Verse.AI
{
	public class Pawn_MindState : IExposable
	{
		...
		public void MindStateTick()
		{
			...
			if (pawn.Spawned && pawn.IsWildMan() && !WildManEverReachedOutside && pawn.GetDistrict() != null && pawn.GetDistrict().TouchesMapEdge)
			{
			...
			if (Find.TickManager.TicksGame % 123 == 0 && pawn.Spawned && pawn.RaceProps.IsFlesh && pawn.needs.mood != null)
			{
				...
				if (pawn.Position.GasDentity(pawn.Map, GasType.RotStink) > 0)
				{
		...
		public void Notify_ClamorImpact(Thing instigator)
		{
			...
			if (pawn.RaceProps.Animal && instigator is Projectile && ((Projectile)instigator).AnimalsFleeImpact && (pawn.playerSettings == null || pawn.playerSettings.Master == null) && Rand.Chance(0.4f) && CanStartFleeingBecauseOfPawnAction(pawn))
			{
		...
		public void StartFleeingBecauseOfPawnAction(Thing instigator)
		{
			...
			if (fleeDest != pawn.Position)
			{
				...
				if (pawn.RaceProps.Animal && pawn.Faction == null && flag && Rand.Chance(0.5f) && CellFinderLoose.GetFleeExitPosition(pawn, 10f, out position))
				{
