...
namespace Verse
{
	public static class BackCompatibility
	{
		...
		{
			if (Scribe.mode != 0 && VersionControl.CurrentBuild == ScribeMetaHeaderUtility.loadedGameVersionBuild)
			{
				return defName;
			}
		...
		public static void FactionManagerPostLoadInit()
		{
			...
			{
				Faction faction = FactionGenerator.NewGeneratedFaction(new FactionGeneratorParms(FactionDefOf.Empire));
