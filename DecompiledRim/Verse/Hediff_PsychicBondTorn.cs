...
namespace Verse
{
	public class Hediff_PsychicBondTorn : HediffWithTarget
	{
		...
		public override void Notify_Resurrected()
		{
			bool flag = false;
			Pawn pawn;
			if ((pawn = target as Pawn) != null && !pawn.Dead && !pawn.Destroyed && creationTick >= 0 && Find.TickManager.TicksGame - creationTick <= ThoughtDefOf.PsychicBondTorn.DurationTicks)
			{
