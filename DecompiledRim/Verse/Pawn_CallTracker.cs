...
namespace Verse
{
	public class Pawn_CallTracker
	{
		...
		{
			if (Find.CameraDriver.CurrentViewRect.ExpandedBy(10).Contains(pawn.Position) && !pawn.Downed && pawn.Awake() && !pawn.Position.Fogged(pawn.Map) && (!pawn.IsColonyMech || !pawn.IsCharging()))
			{
				DoCall();
			}
		}
		...
		public void DoCall()
		{
			...
			if (PawnAggressive)
			{
				LifeStageUtility.PlayNearestLifestageSound(pawn, (LifeStageAge ls) => ls.soundAngry, null);
				return;
			}
			LifeStageUtility.PlayNearestLifestageSound(pawn, (LifeStageAge ls) => ls.soundCall, (GeneDef g) => g.soundCall, IdleCallVolumeFactor);
		}
