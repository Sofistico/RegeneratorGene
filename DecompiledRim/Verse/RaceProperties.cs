...
namespace Verse
{
	public class RaceProperties
	{
		...
		public IEnumerable<StatDrawEntry> SpecialDisplayStats(ThingDef parentDef, StatRequest req)
		{
			...
			if (!ModsConfig.BiotechActive || !Humanlike || pawn?.genes == null || pawn.genes.Xenotype == XenotypeDefOf.Baseliner)
			{
			...
			Pawn pawn2;
			if ((pawn2 = req.Thing as Pawn) != null && pawn2.needs?.food != null)
			{
				yield return new StatDrawEntry(StatCategoryDefOf.BasicsPawn, "FoodConsumption".Translate(), NutritionEatenPerDay(pawn2), NutritionEatenPerDayExplanation(pawn2), 1600);
			}
			...
			if (parentDef.race.Animal)
			{
				...
				Pawn pawn3;
				if ((pawn3 = req.Thing as Pawn) != null && pawn3.gender != 0)
				{
