	...
	public static class DebugOutputsEcology
	{
		...
		public static void BiomeRanching()
		{
			...
			foreach (Quadrum quadrum2 in QuadrumUtility.Quadrums)
			{
				...
				{
					list2.Add(new TableDataGetter<BiomeDef>(quadrum2.LabelShort() + "\nnutrition\ndaily\n/10x10\n" + terrain2.defName, (BiomeDef b) => (!b.terrainsByFertility.Any((TerrainThreshold tbf) => tbf.terrain == terrain2)) ? "" : (GetAverageNutritionPerDay(quadrum2, b, terrain2) * 10f * 10f).ToString("F3")));
				}
			}
			foreach (Quadrum quadrum in QuadrumUtility.Quadrums)
			{
				...
				{
					list2.Add(new TableDataGetter<BiomeDef>(quadrum.LabelShort() + "\ncows\n/10x10\n" + terrain.defName, (BiomeDef b) => (!b.terrainsByFertility.Any((TerrainThreshold tbf) => tbf.terrain == terrain)) ? "" : (CowsFeedPerDay(quadrum, b, terrain) * 10f * 10f).ToString("F3")));
				}
			}
