...
namespace Verse
{
	public static class CellInspectorDrawer
	{
		...
		private static void DrawMapInspector()
		{
			...
			float value = Find.CurrentMap.glowGrid.GameGlowAt(intVec);
			...
			DrawRow(info: BeautyUtility.AverageBeautyPerceptible(intVec, Find.CurrentMap).ToString("F1"), label: "Beauty_Label".Translate());
			if (zone != null)
			{
		...
		private static void DrawWorldInspector()
		{
			...
			using (List<WorldObject>.Enumerator enumerator = list.GetEnumerator())
			{
				for (; enumerator.MoveNext(); DrawDivider())
				{
					...
					if ((settlement = worldObject as Settlement) != null)
					{
					...
					Caravan caravan;
					if ((caravan = worldObject as Caravan) == null)
					{
						continue;
					}
					...
					if (caravan2.pather.Moving)
					{
						...
						DrawRow(info: ((float)CaravanArrivalTimeEstimator.EstimatedTicksToArrive(caravan2, allowCaching: true) / 60000f).ToString("0.#"), label: "CaravanTTD_Label".Translate());
					}
					else
					{
			...
			{
				string info2 = (WorldPathGrid.CalculatedMovementDifficultyAt(num, perceivedStatic: false) * Find.WorldGrid.GetRoadMovementDifficultyMultiplier(num, -1)).ToString("0.#");
				DrawRow("MovementDifficulty_Label".Translate(), info2);
			}
