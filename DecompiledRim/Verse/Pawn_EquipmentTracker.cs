		...
		public IEnumerable<Gizmo> GetGizmos()
		{
			...
			try
			{
				...
				if (primaryMelee != null)
				{
					...
					foreach (Gizmo item in YieldGizmos(primaryMelee, misc))
					{
						yield return item;
					}
				}
				if (primaryRanged != null)
				{
					...
					foreach (Gizmo item2 in YieldGizmos(primaryRanged, misc2))
					{
						yield return item2;
					}
				}
				for (int i = 0; i < list.Count; i++)
				{
					...
					foreach (Gizmo item3 in YieldGizmos(thingWithComps, null))
					{
						yield return item3;
					}
				}
