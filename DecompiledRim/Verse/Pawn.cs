...
namespace Verse
{
	public class Pawn : ThingWithComps, IStrippable, IBillGiver, IVerbOwner, ITrader, IAttackTarget, ILoadReferenceable, IAttackTargetSearcher, IThingHolder
	{
		...
		public bool IsColonyMechPlayerControlled
		{
			get
			{
				if (base.Spawned && IsColonyMech)
				{
					CompOverseerSubject compOverseerSubject = this.TryGetComp<CompOverseerSubject>();
					if (compOverseerSubject != null)
					{
						return compOverseerSubject.State == OverseerSubjectState.Overseen;
					}
					return false;
				}
				return false;
		...
		public override void Tick()
		{
			...
			drawer?.renderer.EffectersTick(suspended);
		}
		...
		private void DropBeforeDying(DamageInfo? dinfo, ref Map map, ref bool spawned)
		{
			...
			if (pawn_CarryTracker != null && holdingOwner.TryDrop(this, pawn_CarryTracker.pawn.Position, pawn_CarryTracker.pawn.Map, ThingPlaceMode.Near, out var _))
			{
			...
			try
			{
				...
				if (Current.ProgramState == ProgramState.Playing && map != null)
				{
					...
					foreach (Lord lord in map.lordManager.lords)
					{
						...
						if ((lordJob_Ritual = lord.LordJob as LordJob_Ritual) != null && lordJob_Ritual.pawnsDeathIgnored.Contains(this))
						{
							flag4 = true;
							break;
						}
		...
		public void ExitMap(bool allowedToJoinOrCreateCaravan, Rot4 exitDir)
		{
			...
			bool flag = !this.IsCaravanMember() && !teleporting && !PawnUtility.IsTravelingInTransportPodWorldObject(this) && (!IsPrisoner || base.ParentHolder == null || base.ParentHolder is CompShuttle || (guest != null && guest.Released));
			if (flag)
			{
			...
			{
				SlaveFaction.Notify_MemberExitedMap(this, flag);
			}
		...
		public override string GetInspectString()
		{
			...
			if (guest != null && !guest.Recruitable && base.Faction != Faction.OfPlayer)
			{
		...
		public override IEnumerable<Gizmo> GetGizmos()
		{
			...
			LordJob_Ritual lordJob_Ritual;
			if ((IsColonistPlayerControlled || IsColonyMech) && (lord2 == null || (lordJob_Ritual = lord2.LordJob as LordJob_Ritual) == null || !lordJob_Ritual.BlocksDrafting))
			{
				if (drafter != null)
				{
		...
		public override IEnumerable<StatDrawEntry> SpecialDisplayStats()
		{
			...
			if (RaceProps.lifeStageAges.Count > 1)
			{
		...
		public bool Sterile(bool forEmbryoImplantation = false)
		{
			...
			if (!forEmbryoImplantation && health.hediffSet.HasHediff(HediffDefOf.Lactating))
			{
				return true;
			}
