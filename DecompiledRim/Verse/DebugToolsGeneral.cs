	...
	public static class DebugToolsGeneral
	{
		...
		private static List<DebugActionNode> Explosion()
		{
			if (explosionDatas.NullOrEmpty())
			{
				...
				ExplosionData item = new ExplosionData
				{
					damageDef = DamageDefOf.Bomb
				};
				list.Add(item);
				item = new ExplosionData
				{
				...
				{
					List<ExplosionData> list2 = explosionDatas;
					item = new ExplosionData
					{
					...
					list2.Add(item);
					List<ExplosionData> list3 = explosionDatas;
					item = new ExplosionData
					{
			...
			for (int i = 0; i < explosionDatas.Count; i++)
			{
				...
				list4.Add(new DebugActionNode(data.damageDef.LabelCap, DebugActionType.ToolMap, delegate
				{
					GenExplosion.DoExplosion(UI.MouseCell(), Find.CurrentMap, 4.9f, data.damageDef, null, -1, -1f, null, null, null, null, data.thingDef, data.thingChance, data.thingCount, data.gasType, data.applyToNeighbors);
				}));
			}
			return list4;
		}
		...
		private static List<DebugActionNode> PushHeat()
		{
			...
			for (int i = 0; i < HeatPushOptions.Length; i++)
			{
				...
				list.Add(new DebugActionNode(t.ToString(), DebugActionType.ToolMap, delegate
				{
					...
					{
						_ = allCell;
						GenTemperature.PushHeat(UI.MouseCell(), Find.CurrentMap, t);
					}
				}));
			}
			return list;
		...
		private static void ForceSleep()
		{
			foreach (Thing item in Find.CurrentMap.thingGrid.ThingsAt(UI.MouseCell()).ToList())
			{
				...
				else if ((pawn = item as Pawn) != null)
				{
