	...
	public class Thing : Entity, IExposable, ISelectable, ILoadReferenceable, ISignalReceiver
	{
			...
			foreach (Ideo ideo in Faction.OfPlayer.ideos.AllIdeos)
			{
				for (int i = 0; i < ideo.PreceptsListForReading.Count; i++)
				{
					...
					else if (ritual.isAnytime && ritual.ShouldShowGizmo(this))
					{
						...
						yield return new FloatMenuOption(beginRitualText, (text2.NullOrEmpty() && ritualTargetUseReport.failReason.NullOrEmpty()) ? action2 : null, ritual.Icon, (!ritual.def.mergeRitualGizmosFromAllIdeos) ? ideo.Color : Color.white);
						ShowingGizmosForRitualsTmp.Add(ritual.sourcePattern);
					}
				}
			}
		...
		public DamageWorker.DamageResult TakeDamage(DamageInfo dinfo)
		{
			...
			if (dinfo.Def.ExternalViolenceFor(this))
			{
				...
				{
					(dinfo.Instigator as Pawn)?.records.AddTo(RecordDefOf.DamageDealt, damageResult.totalDamageDealt);
					if (dinfo.Instigator.Faction == Faction.OfPlayer)
					{
						QuestUtility.SendQuestTargetSignals(questTags, "TookDamageFromPlayer", this.Named("SUBJECT"));
					}
				}
			}
		...
		public float Ingested(Pawn ingester, float nutritionWanted)
		{
			...
			if (ingester.needs.mood != null)
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					Thought_FoodEaten thought_FoodEaten;
					if ((thought_FoodEaten = thought_Memory as Thought_FoodEaten) != null)
					{
			...
			if (def.ingestible.outcomeDoers != null)
			{
				...
				{
					def.ingestible.outcomeDoers[l].DoIngestionOutcome(ingester, this);
				}
			}
			if (flag8)
			{
