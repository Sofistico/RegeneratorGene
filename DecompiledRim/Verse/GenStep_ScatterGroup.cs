	...
	public class GenStep_ScatterGroup : GenStep_Scatterer
	{
		...
		private bool CalculateScatterInformation(IntVec3 loc, Map map, out CellRect rect, List<ThingSpawn> outThingSpawns)
		{
			...
			if (group.spawnAtCenter != null)
			{
				...
				List<ThingSpawn> list = outThingSpawns;
				item = new ThingSpawn
				{
				...
				{
					List<ThingSpawn> list2 = outThingSpawns;
					item = new ThingSpawn
					{
			...
			foreach (IntVec3 item2 in rect)
			{
				...
				foreach (ThingWeight tmpWeightedThing in tmpWeightedThings)
				{
					...
					{
						if (CanSpawn(tmpWeightedThing.thing, item2, tmpPossibleRotation, GenAdj.OccupiedRect(item2, tmpPossibleRotation, tmpWeightedThing.thing.size)))
						{
							num++;
							flag = true;
							break;
			...
			while (num4 < num3)
			{
				...
				foreach (ThingWeight tmpWeightedThing3 in tmpWeightedThings)
				{
					...
					foreach (IntVec3 item3 in rect.Cells.InRandomOrder(tmpCellsRandomOrderWorkingList))
					{
						foreach (Rot4 tmpPossibleRotation2 in tmpPossibleRotations)
						{
							CellRect occupiedRect2 = GenAdj.OccupiedRect(item3, tmpPossibleRotation2, tmpWeightedThing3.thing.size);
							if (CanSpawn(tmpWeightedThing3.thing, item3, tmpPossibleRotation2, occupiedRect2))
							{
								List<ThingSpawn> list3 = outThingSpawns;
								item = new ThingSpawn
								{
			...
			bool CanSpawn(ThingDef def, IntVec3 cell, Rot4 rot, CellRect occupiedRect)
			{
				...
				foreach (IntVec3 item4 in occupiedRect)
				{
					foreach (Thing thing in item4.GetThingList(map))
					{
