...
public static class MechanitorUtility
{
	...
	public static bool IsOverseerSubject(Pawn pawn)
	{
		if (pawn.Faction.IsPlayerSafe())
		{
			return pawn.GetComp<CompOverseerSubject>() != null;
		}
		return false;
	}
	...
	{
		if (IsMechanitor(pawn) && TryGetMechsLostFromDroppingThing(pawn, thing, out var lostMechs, out var bandwidthLost))
		{
	...
	public static string GetMechGestationJobString(JobDriver_DoBill job, Pawn mechanitor, Bill_Mech bill)
	{
		...
		case FormingCycleState.Gathering:
			...
			goto case FormingCycleState.Preparing;
	...
	{
		return MechsInPlayerFaction().Any();
	}
