...
namespace System.Collections.Generic
{
	...
	public class PriorityQueue<TElement, TPriority>
	{
		...
		public void Enqueue(TElement element, TPriority priority)
		{
			...
			if (_comparer == null)
			{
				MoveUpDefaultComparer((element, priority), num);
			}
			else
			{
				MoveUpCustomComparer((element, priority), num);
			}
		}
		...
		public void EnqueueRange(IEnumerable<TElement> elements, TPriority priority)
		{
			...
			ICollection<(TElement, TPriority)> collection;
			int count;
			if ((collection = elements as ICollection<(TElement, TPriority)>) != null && (count = collection.Count) > _nodes.Length - _size)
			{
				Grow(_size + count);
			}
			if (_size == 0)
